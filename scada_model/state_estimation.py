...
class WLSEstimator:
    ...
    def estimate(
        self, Ybus: np.ndarray, measurements: dict, bus_ids: list[str], slack_bus_idx: int = 0,  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        ...
        for iteration in range(1, self.max_iterations + 1):
            ...
            H_reduced = H[:, keep_cols]  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability

            # Gain matrix G = H^T W H
            try:
                G = H_reduced.T @ W @ H_reduced
                G_inv = np.linalg.inv(G)  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        ...
        H_final = self._compute_jacobian(x, Ybus, h_indices, n)  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        r_final = z - h_x_final
        objective = float(r_final.T @ W @ r_final)

        # Covariance matrix (use reduced Jacobian to avoid singularity)
        try:
            H_final_reduced = H_final[:, keep_cols]  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
            G_final = H_final_reduced.T @ W @ H_final_reduced  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
            G_inv_final = np.linalg.inv(G_final)  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        ...
        if covariance is not None:
            ...
            Omega = np.eye(m) - S  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
            try:
                Omega_inv = np.linalg.inv(np.diag(np.diag(Omega)) + np.eye(m) * 1e-10)  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
    ...
    def _build_measurement_vectors(  # NOSONAR: cognitive complexity; scheduled for refactoring sprint (extract helpers / early returns)
        self, measurements: dict, n: int, slack_idx: int,  # NOSONAR: unused param kept for API compatibility
        ...
        for bus_idx, (P, Q, sigma_P, sigma_Q) in measurements.get("power_injection", {}).items():  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
    ...
    def _compute_h(self, x: np.ndarray, Ybus: np.ndarray, h_indices: list, n: int) -> np.ndarray:  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        ...
        for k, idx_info in enumerate(h_indices):
            if idx_info[0] == "V":
                ...
                _, i, j = idx_info
                Gij = Ybus[i, j].real  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
                Bij = Ybus[i, j].imag  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
                Pij = V[i] ** 2 * Gij - V[i] * V[j] * (  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
                ...
                Gij = Ybus[i, j].real
                Bij = Ybus[i, j].imag
                Qij = -(V[i] ** 2) * Bij - V[i] * V[j] * (  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
    ...
    def _compute_jacobian(  # NOSONAR: cognitive complexity; scheduled for refactoring sprint (extract helpers / early returns)
        self, x: np.ndarray, Ybus: np.ndarray, h_indices: list, n: int,  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
    ) -> np.ndarray:
