...
class WLSEstimator:
    ...
    def estimate(self, Ybus: np.ndarray, measurements: dict,
                 bus_ids: List[str], slack_bus_idx: int = 0) -> StateEstimationResult:
        """
        ...
        z, h_indices, weights = self._build_measurement_vectors(
            measurements, n, slack_bus_idx
        )
    ...
    def _build_measurement_vectors(self, measurements: dict, n: int,
                                    slack_idx: int) -> Tuple[np.ndarray, list, np.ndarray]:
    ...
    def _compute_h(self, x: np.ndarray, Ybus: np.ndarray,
                   h_indices: list, n: int) -> np.ndarray:
        ...
        for k, idx_info in enumerate(h_indices):
            if idx_info[0] == 'V':
                ...
                Pi = 0.0
                for j in range(n):
                    Pi += V[i] * V[j] * (Ybus[i, j].real * np.cos(theta[i] - theta[j]) +
                ...
                Qi = 0.0
                for j in range(n):
                    Qi += V[i] * V[j] * (Ybus[i, j].real * np.sin(theta[i] - theta[j]) -
                ...
                Gij = Ybus[i, j].real
                Bij = Ybus[i, j].imag
                Pij = V[i]**2 * Gij - V[i] * V[j] * (Gij * np.cos(theta[i] - theta[j]) +
                                                         Bij * np.sin(theta[i] - theta[j]))
                h[k] = Pij
            elif idx_info[0] == 'Qij':
                _, i, j = idx_info
                Gij = Ybus[i, j].real
                Bij = Ybus[i, j].imag
                Qij = -V[i]**2 * Bij - V[i] * V[j] * (Gij * np.sin(theta[i] - theta[j]) -
                                                          Bij * np.cos(theta[i] - theta[j]))
                h[k] = Qij
        return h

    def _compute_jacobian(self, x: np.ndarray, Ybus: np.ndarray,
                          h_indices: list, n: int) -> np.ndarray:
        ...
        for k, idx_info in enumerate(h_indices):
            if idx_info[0] == 'V':
                ...
                for j in range(n):
                    ...
                    Bij = Ybus[i, j].imag
                    # dPi/dtheta_j
                    H[k, j] = V[i] * V[j] * (Gij * np.sin(theta[i] - theta[j]) -
                                               Bij * np.cos(theta[i] - theta[j]))
                # dPi/dtheta_i
                H[k, i] = -sum(
                    V[i] * V[j] * (Ybus[i, j].real * np.sin(theta[i] - theta[j]) -
                                    Ybus[i, j].imag * np.cos(theta[i] - theta[j]))
                    for j in range(n) if j != i
                )
                # dPi/dV_i
                H[k, n + i] = sum(
                    V[j] * (Ybus[i, j].real * np.cos(theta[i] - theta[j]) +
                             Ybus[i, j].imag * np.sin(theta[i] - theta[j]))
                    for j in range(n)
                )
                # dPi/dV_j
                for j in range(n):
                    if i == j:
                        continue
                    H[k, n + j] = V[i] * (Ybus[i, j].real * np.cos(theta[i] - theta[j]) +
                                           Ybus[i, j].imag * np.sin(theta[i] - theta[j]))
                ...
                for j in range(n):
                    ...
                    Gij = Ybus[i, j].real
                    Bij = Ybus[i, j].imag
                    H[k, j] = -V[i] * V[j] * (Gij * np.cos(theta[i] - theta[j]) +
                                                Bij * np.sin(theta[i] - theta[j]))
                H[k, i] = sum(
                    V[i] * V[j] * (Ybus[i, j].real * np.cos(theta[i] - theta[j]) +
                                    Ybus[i, j].imag * np.sin(theta[i] - theta[j]))
                    for j in range(n) if j != i
                )
                H[k, n + i] = sum(
                    V[j] * (Ybus[i, j].real * np.sin(theta[i] - theta[j]) -
                             Ybus[i, j].imag * np.cos(theta[i] - theta[j]))
                    for j in range(n)
                )
                for j in range(n):
                    if i == j:
                        continue
                    H[k, n + j] = V[i] * (Ybus[i, j].real * np.sin(theta[i] - theta[j]) -
                                           Ybus[i, j].imag * np.cos(theta[i] - theta[j]))
                ...
                Gij = Ybus[i, j].real
                Bij = Ybus[i, j].imag
                H[k, i] = V[i] * V[j] * (Gij * np.sin(theta[i] - theta[j]) +
                                           Bij * np.cos(theta[i] - theta[j]))
                H[k, j] = -V[i] * V[j] * (Gij * np.sin(theta[i] - theta[j]) +
                                            Bij * np.cos(theta[i] - theta[j]))
                H[k, n + i] = 2 * V[i] * Gij - V[j] * (Gij * np.cos(theta[i] - theta[j]) +
                                                          Bij * np.sin(theta[i] - theta[j]))
                H[k, n + j] = -V[i] * (Gij * np.cos(theta[i] - theta[j]) +
                                        Bij * np.sin(theta[i] - theta[j]))
                ...
                Gij = Ybus[i, j].real
                Bij = Ybus[i, j].imag
                H[k, i] = -V[i] * V[j] * (Gij * np.cos(theta[i] - theta[j]) -
                                            Bij * np.sin(theta[i] - theta[j]))
                H[k, j] = V[i] * V[j] * (Gij * np.cos(theta[i] - theta[j]) -
                                          Bij * np.sin(theta[i] - theta[j]))
                H[k, n + i] = -2 * V[i] * Bij - V[j] * (Gij * np.sin(theta[i] - theta[j]) -
                                                           Bij * np.cos(theta[i] - theta[j]))
                H[k, n + j] = -V[i] * (Gij * np.sin(theta[i] - theta[j]) -
                                        Bij * np.cos(theta[i] - theta[j]))
