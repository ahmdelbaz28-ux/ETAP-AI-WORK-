 ...
 *
 * V214: Exposes the backend /api/v1/api-keys endpoints:
 *   GET    /api-keys          — List all API keys
 *   POST   /api-keys          — Create new API key
 *   DELETE /api-keys/{hash}   — Delete API key
 *   PUT    /api-keys/{hash}   — Update API key
 *   GET    /api-keys/roles    — List available roles
 */
...
export function ApiKeysPage() {
	...
	const fetchKeys = async () => {
		...
		try {
			...
			const resp = await fetch("/api/v1/api-keys", { headers });
	...
	const handleDelete = async (keyHash: string) => {
		...
		try {
			...
			const headers: Record<string, string> = {};
			if (apiKey) headers["X-API-Key"] = apiKey;
			const resp = await fetch(`/api/v1/api-keys/${keyHash}`, {
				method: "DELETE",
				headers,
			});
			if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
