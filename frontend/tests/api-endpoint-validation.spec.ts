...
test.describe('API Endpoint Validation Tests', () => {
  ...
  test('should validate dashboard API calls', async ({ page }) => {
    ...
    const dashboardCalls = apiCallLogs.filter(call => 
      call.url.includes('/api/') && 
      (call.url.includes('/projects') || call.url.includes('/stats') || call.url.includes('/health'))
    );
    ...
    for (const call of dashboardCalls) {
      console.log(`Dashboard API Call: ${call.method} ${call.url} -> ${call.statusCode}`);
  ...
  test('should validate AutoCAD connect API call', async ({ page }) => {
    ...
    if (await connectButton.count() > 0) {
      ...
      console.log(`AutoCAD Connect API: ${response.request().method()} ${response.url()} -> ${response.status()}`);
    } else {
  ...
  test('should validate Revit connect API call', async ({ page }) => {
    ...
    if (await connectButton.count() > 0) {
      ...
      console.log(`Revit Connect API: ${response.request().method()} ${response.url()} -> ${response.status()}`);
    } else {
  ...
  test('should validate project creation API call', async ({ page }) => {
    ...
    if (await createButton.count() > 0) {
      ...
      console.log(`Project Creation API: ${response.request().method()} ${response.url()} -> ${response.status()}`);
    } else {
  ...
  test('should validate digital twin conversion API call', async ({ page }) => {
    ...
    if (await convertButton.count() > 0) {
      // Wait for the click to trigger the API call
      const responsePromise = page.waitForResponse('**/api/v*/digital-twin/convert');
      ...
      console.log(`Digital Twin Conversion API: ${response.request().method()} ${response.url()} -> ${response.status()}`);
    } else {
  ...
  test('should validate element operations API calls', async ({ page }) => {
    ...
    if (await filterButton.count() > 0) {
      ...
      console.log(`Elements API: ${response.request().method()} ${response.url()} -> ${response.status()}`);
    } else {
      // If no filter button, try other element-related buttons
      const otherButtons = page.locator('button:has-text("Refresh"), button:has-text("Load")');
  ...
  test('should validate connection operations API calls', async ({ page }) => {
    ...
    const actionButtons = page.locator('button:has-text("Validate"), button:has-text("Test"), button:has-text("Sync")');
    ...
    if (await actionButtons.count() > 0) {
      ...
      console.log(`Connections API: ${response.request().method()} ${response.url()} -> ${response.status()}`);
    } else {
  ...
  test('should validate conflict resolution API calls', async ({ page }) => {
    ...
    if (await resolveButton.count() > 0) {
      ...
      console.log(`Conflicts API: ${response.request().method()} ${response.url()} -> ${response.status()}`);
    } else {
  ...
  test('should validate report generation API calls', async ({ page }) => {
    ...
    if (await generateButton.count() > 0) {
      ...
      console.log(`Reports API: ${response.request().method()} ${response.url()} -> ${response.status()}`);
    } else {
  ...
  test('should validate export operations API calls', async ({ page }) => {
    ...
    if (await exportButton.count() > 0) {
      ...
      console.log(`Export API: ${response.request().method()} ${response.url()} -> ${response.status()}`);
    } else {
...
test.describe('API Call Validation Summary', () => {
  test('should have valid API responses for all button interactions', async ({ page }) => {
    ...
    for (const call of apiCalls) {
      console.log(`${call.method} ${new URL(call.url).pathname} -> ${call.statusCode}`);
      
      // Validate each API call meets requirements
      expect(call.statusCode, `API call failed: ${call.method} ${call.url}`).toBeGreaterThanOrEqual(200);
      expect(call.statusCode, `API call resulted in error: ${call.method} ${call.url}`).toBeLessThan(400);
    }
    
    // Ensure we had at least some API calls
    expect(apiCalls.length, 'Should have captured at least one API call').toBeGreaterThan(0);
  });
});