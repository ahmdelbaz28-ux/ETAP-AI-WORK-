 ...
 */
function logTestResult(testName: string, action: string, status: number, statusText: string, duration: number, error?: string, details: any = {}) {
...
test.describe('Dashboard Page Button Tests', () => {
  test('should test dashboard refresh button and verify backend connection', async ({ page }) => {
    await page.goto('/dashboard');
    await page.waitForLoadState('networkidle');
    
    // Wait for the refresh button to be available
    const refreshButton = page.locator('button:has-text("Refresh"), button:has-text("refresh"), button:has(svg)').filter({ hasText: 'Activity' });
  ...
  test('should test dashboard report generator button and verify backend connection', async ({ page }) => {
    await page.goto('/dashboard');
    await page.waitForLoadState('networkidle');
    
    // Find the report generator button
    const reportButton = page.locator('button:has-text("Open Report Generator"), button:has-text("Generate Report")');
...
test.describe('Projects Page Button Tests', () => {
  test('should test projects page buttons and verify backend connection', async ({ page }) => {
...
test.describe('AutoCAD Page Button Tests', () => {
  test('should test AutoCAD connect button and verify backend connection', async ({ page }) => {
    await page.goto('/autocad');
    await page.waitForLoadState('networkidle');
    
    const connectButton = page.locator('button:has-text("Connect"), button:has-text("Connect to AutoCAD")');
  ...
  test('should test AutoCAD upload button and verify backend connection', async ({ page }) => {
    await page.goto('/autocad');
    await page.waitForLoadState('networkidle');
    
    const uploadButton = page.locator('button:has-text("Upload"), button:has-text("Import DWG")');
...
test.describe('Revit Page Button Tests', () => {
  test('should test Revit connect button and verify backend connection', async ({ page }) => {
    await page.goto('/revit');
    await page.waitForLoadState('networkidle');
    
    const connectButton = page.locator('button:has-text("Connect"), button:has-text("Connect to Revit")');
  ...
  test('should test Revit upload button and verify backend connection', async ({ page }) => {
    await page.goto('/revit');
    await page.waitForLoadState('networkidle');
    
    const uploadButton = page.locator('button:has-text("Upload"), button:has-text("Import RVT")');
...
test.describe('Digital Twin Page Button Tests', () => {
  test('should test digital twin conversion button and verify backend connection', async ({ page }) => {
    await page.goto('/digital-twin');
    await page.waitForLoadState('networkidle');
    
    const convertButton = page.locator('button:has-text("Convert"), button:has-text("Start Conversion")');
    
    if (await convertButton.count() > 0) {
      // Intercept the conversion API call
      const responsePromise = page.waitForResponse('**/api/**/digital-twin/convert');
...
test.describe('Elements Page Button Tests', () => {
  test('should test elements filter buttons and verify backend connection', async ({ page }) => {
    await page.goto('/elements');
    await page.waitForLoadState('networkidle');
    
    // Test filter and action buttons
    const filterButtons = page.locator('button:has-text("Filter"), button:has-text("Search"), button:has-text("Clear")');
...
test.describe('Connections Page Button Tests', () => {
  test('should test connections create button and verify backend connection', async ({ page }) => {
    await page.goto('/connections');
    await page.waitForLoadState('networkidle');
    
    const createButton = page.locator('button:has-text("New Connection"), button:has-text("Create Connection")');
...
test.describe('Conflicts Page Button Tests', () => {
  test('should test conflicts resolve button and verify backend connection', async ({ page }) => {
    await page.goto('/conflicts');
    await page.waitForLoadState('networkidle');
    
    const resolveButton = page.locator('button:has-text("Resolve"), button:has-text("Fix Conflicts")');
...
test.describe('Reports Page Button Tests', () => {
  test('should test reports generate button and verify backend connection', async ({ page }) => {
  ...
  test('should test reports export buttons and verify backend connection', async ({ page }) => {
    await page.goto('/reports');
    await page.waitForLoadState('networkidle');
    
    // Test export buttons
    const exportButtons = page.locator('button:has-text("Export"), button:has-text("Download"), button:has-text("Print")');
...
test.describe('Settings Page Button Tests', () => {
  test('should test settings save button and verify backend connection', async ({ page }) => {
...
test.describe('Engineering Page Button Tests', () => {
  test('should test engineering page buttons and verify backend connection', async ({ page }) => {
    ...
    if (buttonCount > 0) {
      ...
      for (let i = 0; i < Math.min(buttonCount, 5); i++) {
        ...
        if (buttonText && buttonText.trim() !== '') {
          ...
          try {
            const responsePromise = page.waitForResponse('**/api/**', { timeout: 5000 });
            await button.click();
            const response = await responsePromise;
...
test.describe('Fire Alarm Page Button Tests', () => {
  test('should test fire alarm page buttons and verify backend connection', async ({ page }) => {
...
test.describe('CAD Settings Page Button Tests', () => {
  test('should test CAD settings page buttons and verify backend connection', async ({ page }) => {
    await page.goto('/settings/cad');
    await page.waitForLoadState('networkidle');
    
    // Test connection test buttons
    const testButtons = page.locator('button:has-text("Test Connection"), button:has-text("Verify"), button:has-text("Test")');
    ...
    if (await testButtons.count() > 0) {
      for (let i = 0; i < Math.min(await testButtons.count(), 3); i++) {
        ...
        try {
          const responsePromise = page.waitForResponse('**/api/**/settings/test*', { timeout: 5000 });
          await button.click();
          const response = await responsePromise;
...
test.describe('Navigation Button Tests', () => {
  test('should test all navigation buttons and verify they lead to correct pages', async ({ page }) => {
    await page.goto('/');
    await page.waitForLoadState('networkidle');
    ...
    for (let i = 0; i < Math.min(linkCount, 15); i++) { // Limit to avoid too many tests
      const link = navLinks.nth(i);
      const href = await link.getAttribute('href');
      
      if (href && !href.startsWith('http') && !href.startsWith('mailto')) { // Internal links only
...
test.afterAll(async () => {
  ...
  const report = {
    summary: {
      ...
      passedTests: testResults.filter(r => r.status >= 200 && r.status < 300).length,
      failedTests: testResults.filter(r => r.status < 200 || r.status >= 300).length,
      totalDuration: testResults.reduce((sum, r) => sum + r.duration, 0),
      averageDuration: testResults.length > 0 ? testResults.reduce((sum, r) => sum + r.duration, 0) / testResults.length : 0
    },
    results: testResults,
    timestamp: new Date().toISOString()
  };
  ...
  for (const result of testResults) {
    console.log(`${result.testName} - ${result.action}: [${result.status}] ${result.statusText} (${result.duration}ms)`);
  }
  ...
  // Final verification
  expect(report.summary.totalTests, 'Should have tested at least some buttons').toBeGreaterThan(0);
  expect(report.summary.passedTests, 'Majority of tests should pass').toBeGreaterThanOrEqual(Math.max(1, report.summary.totalTests * 0.5)); // At least 50% or 1 test should pass
});