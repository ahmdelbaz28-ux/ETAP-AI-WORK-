...
class SecurityMiddleware:
    ...
    def validate_request(
        ...
        # Check rate limits
        rate_limit_ok, rate_limit_error = self._check_rate_limits(
            request.security.get("auth_token"), source_node
        )
        ...
        is_auth, auth_context = self.auth_provider.authenticate_request(
            request.security, source_node
        )
        if not is_auth:
        ...
        policy_check_ok, policy_errors = self._apply_security_policy(
            request, risk_level, source_node
        )
    ...
    def _check_payload_size(self, params: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            ...
            if payload_size > self.payload_size_limit:
                return (
                    False,
                    f"Payload exceeds size limit ({payload_size} > {self.payload_size_limit})",
                )
            return True, ""
        except Exception as e:
    ...
    def _check_rate_limits(self, token: str, source_node: str) -> Tuple[bool, str]:
        ...
        if user_id in self.rate_limits:
            self.rate_limits[user_id] = [
                (ts, count)
                for ts, count in self.rate_limits[user_id]
        ...
        max_requests = (
            1000 if source_node and "l2" in source_node else 100
        )  # More permissive for orchestrator nodes
    ...
    def _apply_security_policy(
        self, request: FACPRequest, risk_level: str, source_node: str
    ) -> Tuple[bool, List[str]]:
        ...
        if risk_level == RiskLevel.HIGH.value:
            ...
            if request.method.startswith("admin.") or request.method.startswith(
                "system."
                ...
                if not source_node or not any(n in source_node for n in ["l2", "orchestrator"]):
                    errors.append(
                        "High-risk administrative methods only allowed from orchestrator nodes"
                    )
...
class ValidationFirewall:
    ...
    def process_request(
        ...
        is_valid, errors, validated_data = self.middleware.validate_request(
            request_data, source_node
        )
    ...
    def validate_target_access(
        self, auth_context: Dict[str, Any], target: str, source_node: str
    ) -> bool:
        """
        ...
        if target == "engine":
            return (
                "engine_access" in user_permissions
                or "admin" in user_permissions
                or ("operator" in user_roles and source_node and "l2" in source_node)
            )
    ...
    def log_security_event(
        self, event_type: str, details: Dict[str, Any], source_node: Optional[str] = None
    ):
    ...
    def cleanup_expired_entries(self):
        ...
        expired_keys = [
            key
            for key, value in self.distributed_idempotency_store.items()
