...
class Orchestrator:
    ...
    def _register_default_agents(self):
        ...
        self.agent_registry.register_agent(
            "planner_agent",
            {"type": "planner", "capabilities": ["plan.*", "schedule.*"], "node_affinity": None},
        )

        self.agent_registry.register_agent(
            "executor_agent",
            {"type": "executor", "capabilities": ["execute.*", "task.*"], "node_affinity": None},
        )

        self.agent_registry.register_agent(
            "validator_agent",
            {"type": "validator", "capabilities": ["validate.*", "check.*"], "node_affinity": None},
        )

        self.agent_registry.register_agent(
            "optimizer_agent",
            {"type": "optimizer", "capabilities": ["optimize.*", "tune.*"], "node_affinity": None},
        )
    ...
    def process_request(
        ...
        try:
            ...
            if not allowed:
                self.logger.warning(
                    "Orchestrator[%s]: Request %s denied by permission check: %s",
                    self.node_id,
                    request_id,
                    permission_reason,
                )
                ...
                error_response = FACPResponse(
                    id=request_id,
                    status="error",
                    error={"code": "PERMISSION_DENIED", "message": permission_reason},
                    trace={
                        "execution_path": ["L1", "L2_orchestrator"],
                        "latency_ms": (time.time() - self.active_tasks[request_id]["received_at"])
                        * 1000,
            ...
            if self._should_route_to_engine(method):
                ...
                if not target_worker:
                    error_response = FACPResponse(
                        ...
                        trace={
                            "execution_path": ["L1", "L2_orchestrator"],
                            "latency_ms": (
                                time.time() - self.active_tasks[request_id]["received_at"]
                            )
                            * 1000,
                            "node_id": self.node_id,
                ...
                self.task_scheduler.schedule_task(method, request_data, target_worker, source_node)
                ...
                if "trace" in response:
                    response["trace"]["execution_path"] = [
                        "L1",
                        "L2_orchestrator",
                        f"L3_engine@{target_worker}",
                    ]
                    response["trace"]["orchestrator_node"] = self.node_id
                    response["trace"]["orchestration_time_ms"] = (
                        time.time() - self.active_tasks[request_id]["received_at"]
                    ) * 1000
                else:
                    response["trace"] = {
            ...
            self.logger.info(
                "Orchestrator[%s]: Processing request %s with agent for method %s",
                self.node_id,
                request_id,
                method,
            )
            ...
            self.logger.error(
                "Orchestrator[%s]: Unexpected error processing request %s: %s",
                self.node_id,
                request_id,
                str(e),
            )
    ...
    def _should_route_to_engine(
        self, method: str
    ) -> bool:  # NOSONAR — S3516: always True is a CONSERVATIVE SAFETY DEFAULT (route to engine when in doubt)
    ...
    def _simulate_engine_forwarding(
        self, request_data: Dict[str, Any], target_worker: str
    ) -> Dict[str, Any]:
    ...
    def _process_with_agent_info(
        self, agent_info: Dict[str, Any], request_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _calculate_tasks_per_minute(self) -> float:
        ...
        time_span_minutes = (
            time.time() - min(t.get("received_at", time.time()) for t in recent_tasks)
        ) / 60
    ...
    def update_policy(
        self, policy_name: str, _policy_config: Dict[str, Any]
    ...
    def enforce_distributed_idempotency(
    ...
    def release_idempotency_lock(self, idempotency_key: str, response: Dict[str, Any]):
        ...
        if idempotency_key in self.distributed_locks:
            self.distributed_locks[idempotency_key].update(
                {"status": "completed", "response": response, "released_at": time.time()}
            )
