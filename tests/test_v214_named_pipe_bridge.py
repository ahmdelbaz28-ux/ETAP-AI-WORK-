...
Verifies that:
  ...
  5. ETAPApplication.cs exists with IExternalApplication
...
class TestV214RevitAddinProject:
    ...
    def test_csproj_exists(self):
        """ETAPRevitAddin.csproj must exist."""
        csproj = ADDIN_DIR / "ETAPRevitAddin.csproj"
    ...
    def test_sln_exists(self):
        """ETAPRevitAddin.sln must exist."""
        sln = ADDIN_DIR / "ETAPRevitAddin.sln"
        assert sln.exists(), f"Missing: {sln}"

    def test_addin_registration_file_exists(self):
        """ETAPRevitAddin.addin must exist for Revit registration."""
        addin = ADDIN_DIR / "ETAPRevitAddin.addin"
        assert addin.exists(), f"Missing: {addin}"
        content = addin.read_text(encoding="utf-8")
        assert "ETAPApplication" in content, "Must reference ETAPApplication class"
    ...
    def test_named_pipe_server_cs_exists(self):
        ...
        assert "ETAPRevitPipe" in content, "Must use the ETAPRevitPipe name"
    ...
    def test_application_cs_exists(self):
        """ETAPApplication.cs must implement IExternalApplication."""
        app = ADDIN_DIR / "ETAPApplication.cs"
...
class TestV214PythonNamedPipeClient:
    ...
    def test_module_exists(self):
        """named_pipe_client.py must exist in etap/mcp_server/."""
        module = REPO_ROOT / "etap" / "mcp_server" / "named_pipe_client.py"
        assert module.exists(), f"Missing: {module}"

    def test_pipe_name_matches_cs(self):
        """The Python pipe name must match the C# pipe name."""
        py_module = REPO_ROOT / "etap" / "mcp_server" / "named_pipe_client.py"
        ...
        # Both must contain "ETAPRevitPipe"
        assert "ETAPRevitPipe" in py_content, "Python must reference ETAPRevitPipe"
        assert "ETAPRevitPipe" in cs_content, "C# must reference ETAPRevitPipe"
    ...
    def test_get_stats_returns_pipe_info(self):
        ...
        assert "ETAPRevitPipe" in stats["pipe_name"]
