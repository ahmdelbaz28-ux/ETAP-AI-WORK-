        ...
        for fpath in frontend_paths:
            ...
            if "{id}" in fpath:
                ...
                if not found:
                    ...
                    found = any(
                        fpath.replace("{id}", "").rstrip("/") in bpath for bpath in backend_paths
                    )
            else:
...
class TestOpenApiCompleteness:
    ...
    def test_all_get_endpoints_reachable(self, client, openapi_paths, auth_headers):
        ...
        get_endpoints = [
            ...
            if "get" in methods and not any(ind in path for ind in param_indicators)
        ]
        assert len(get_endpoints) > 40, (
            f"Expected 40+ param-less GET endpoints, got {len(get_endpoints)}"
        )
        ...
        assert len(failures) == 0, f"These GET endpoints return 404 (not registered): {failures}"
