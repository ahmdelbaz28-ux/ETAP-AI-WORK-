...
def multimachine_data():
    ...
    Ybus_red = G + 1j * B  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
...
class TestStabilityScenario:
    ...
    def test_transient_stability_analysis(self, agent, multimachine_data):
        ...
        d = multimachine_data
        fault_Ybus = d["Ybus_red"].copy()  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        fault_Ybus[0, 0] += 1.0 / 1e-6  # Three-phase fault at bus 0

        post_fault_Ybus = d["Ybus_red"].copy()  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
