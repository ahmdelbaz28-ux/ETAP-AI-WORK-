...
async function runStressTest() {
  ...
  console.log(
    `  ${agentOverload.degradedGracefully ? '✓ Degraded gracefully' : '✗ Did not degrade gracefully'}`,
  );
  await new Promise((r) => setTimeout(r, 1000));

  // Scenario 3: Queue Saturation — 300 study submissions
  console.log('\n▶ Scenario 3: Queue Saturation — 300 study submissions');
  const queueSat = await runBurst(300, '/api/v1/studies/run', 'POST', {
    studyType: 'load_flow',
    parameters: { base_mva: 100 },
  });
  results.push({ ...queueSat, scenario: 'Queue Saturation (300 studies)' });
  console.log(
    `  ✅ ${queueSat.successCount} ok | ❌ ${queueSat.errorCount} err | 🚫 ${queueSat.rateLimitedCount} rate-limited`,
  );
  console.log(
