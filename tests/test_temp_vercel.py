...
async def test_probe_providers():
    ...
    async with httpx.AsyncClient() as client:
        ...
        headers = {
            "Authorization": f"Bearer {keys['openmodel']}",
            "Content-Type": "application/json",
        }
        body = {"model": "qwen3.6-plus", "input": "How many r-s are in strawberry?"}
        try:
        ...
        headers_nv = {
            "Authorization": f"Bearer {keys['nvidia_prefix']}",
            "Content-Type": "application/json",
        }
        try:
            ...
            if resp_nv.status_code == 200:
                ...
                for target_model in nv_models[:3]:
                    ...
                    resp_chat = await client.post(
                        url_nv_chat, headers=headers_nv, json=body_nv, timeout=15
                    )
