  ...
  return {
    ...
    async doGenerate(options: any) {
      ...
      for (const model of models) {
        try {
          console.log(
            `[Failover] Attempting doGenerate with model: ${model.provider}:${model.modelId}`,
          );
          return await model.doGenerate(options);
        } catch (err: any) {
          lastError = err;
          console.warn(
            `[Failover] Model ${model.modelId} failed: ${err.message || err}. Trying next...`,
          );
        }
      }
      throw lastError;
    ...
    async doStream(options: any) {
      ...
      for (const model of models) {
        try {
          console.log(
            `[Failover] Attempting doStream with model: ${model.provider}:${model.modelId}`,
          );
...
async function runTest() {
  const failoverModel = createFailoverModel([openModelLanguageModel, modalModel, nvidiaModel]);
