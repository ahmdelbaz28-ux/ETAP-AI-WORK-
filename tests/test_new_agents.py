...
class TestStabilityAgent:
    ...
    def test_transient_stability_smib(self):
        ...
        H = np.array([5.0])
        D = np.array([2.0])
        Pm = np.array([0.8])  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        Ybus_red = np.array([[5.0 - 20j]])  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
    ...
    def test_small_signal_stability(self):
        ...
        H = np.array([4.0, 5.0])
        D = np.array([2.0, 2.0])
        Pm = np.array([0.7, 0.5])  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
...
class TestCableSizingAgent:
    ...
    def test_voltage_drop_ac_3phase(self):
        ...
        assert isinstance(result, dict)
        vd_pct = result.get(
            "voltage_drop_percent", result.get("voltage_drop_pct", result.get("percentage", 0))
        )
        assert vd_pct > 0
        assert vd_pct < 10.0
    ...
    def test_cable_recommendation(self):
        ...
        xsec = result.get("recommended_cross_section_mm2", result.get("cross_section_mm2", 0))
...
class TestEarthGridAgent:
    ...
    def test_surface_derating_factor(self):
        ...
        Cs = agent._surface_derating_factor(rho_s=2000, rho_b=100, hs=0.1)  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        assert 0 < Cs <= 1.0
    ...
    def test_mesh_voltage_calculation(self):
        ...
        has_voltage = any(k in result for k in ["E_mesh", "mesh_voltage_V", "Em_V", "mesh_voltage"])
...
class TestRenewableAgent:
    ...
    def test_hosting_capacity(self):
        ...
        assert isinstance(result, dict)
        hc = result.get("hosting_capacity_kw", result.get("max_der_kw", 0))
        assert hc >= 0
