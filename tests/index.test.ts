...
describe('Cloudflare Worker API Gateway', () => {
  // 32-byte test keys (>= API_KEY_MIN_LENGTH = 32) so the
  // security-hardened auth layer accepts them.
  const TEST_KEY = 'test-secret-0123456789-abcdefghijklmnopqrstuvwxyz'; // 44 chars
  const TEST_KV_KEY = 'kv-test-key-0123456789-abcdefghijklmnopqrstuvwxyz'; // 44 chars
  // 'bad-key' and 'some-key' tests below intentionally use SHORT keys
  // to verify the min-length rejection — those stay below 32 chars.
  ...
  it('returns 401 with invalid API key', async () => {
    const res = await makeRequest('/api/v1/agents', {
      // 32 chars but NOT the configured secret — must be rejected
      headers: { 'x-api-key': 'bad-key-0123456789-abcdefghijklmnopqrstuv' },
    });
    expect(res.status).toBe(401);
    const body = (await res.json()) as Record<string, unknown>;
  ...
  it('returns 401 when API_KEY_SECRET is not configured', async () => {
    const res = await makeRequest('/api/v1/agents', {
      headers: { 'x-api-key': 'some-key-0123456789-abcdefghijklmnopqrstuv' },
    }, {});
    expect(res.status).toBe(401);
    const body = (await res.json()) as Record<string, unknown>;
  ...
  it('returns 200 and agent list with valid API key (KV-backed)', async () => {
    const kv = mockKV();
    await kv.put(`api-key:${TEST_KV_KEY}`, JSON.stringify({ createdAt: Date.now(), name: 'test-key' }));
    const env = {
      API_KEY_SECRET: TEST_KEY,
      RATE_LIMIT_KV: kv,
      TASK_STORE_KV: kv,
  ...
  it('returns 401 with revoked KV-backed API key', async () => {
    const kv = mockKV();
    await kv.put(`api-key:${TEST_KV_KEY}`, JSON.stringify({ createdAt: Date.now(), revoked: true, name: 'revoked' }));
    const env = {
      API_KEY_SECRET: TEST_KEY,
      RATE_LIMIT_KV: kv,
      TASK_STORE_KV: kv,
  ...
  it('returns CORS headers on preflight', async () => {
    // Use a trusted origin (one of DEFAULT_TRUSTED_ORIGINS) so the
    // security-hardened corsHeaders() actually reflects it.
    const trustedOrigin = 'https://etap-ai-work.vercel.app';
    const res = await makeRequest('/api/v1/agents', {
      method: 'OPTIONS',
  ...
  it('returns 400 for invalid JSON on chat endpoint', async () => {
    const res = await makeRequest('/api/v1/agents/load-flow-agent/chat', {
      ...
      headers: {
        ...
        'x-api-key': TEST_KEY,
        'origin': 'https://etap-ai-work.vercel.app',
      },
      body: 'not-json',
    });
  ...
  it('enforces rate limiting via mock KV', async () => {
    const kv = mockKV();
    const env = { API_KEY_SECRET: TEST_KEY, RATE_LIMIT_KV: kv, RATE_LIMIT_REQUESTS_PER_MINUTE: '3' };
