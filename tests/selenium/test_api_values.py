...
def test_healthz():
    ...
    if not assert_eq(status, 200, "status"):
        return False
    if not assert_in("status", data, "response"):
        return False
    if not assert_eq(data["status"], "ok", "status value"):
        return False
    return True
...
def test_health():
    ...
    if not assert_in("status", data, "response"):
        return False
    if not assert_eq(data["status"], "healthy", "status value"):
        return False
    if not assert_in("uptime_seconds", data, "response"):
        return False
    if not assert_type(data["uptime_seconds"], (int, float), "uptime_seconds type"):
        return False
    if not assert_in("version", data, "response"):
        return False
    if not assert_type(data["version"], str, "version type"):
        return False
    return True
...
def test_ready():
    ...
    if not assert_eq(status, 200, "status"):
        return False
    if not assert_eq(data["status"], "ready", "status value"):
        return False
    if not assert_in("uptime", data, "response"):
        return False
    return True
...
def test_info():
    ...
    if not assert_eq(status, 200, "status"):
        return False
    if not assert_eq(data["name"], "AhmedETAP", "name"):
        return False
    if not assert_in("version", data, "response"):
        return False
    if not assert_in("description", data, "response"):
        return False
    if not assert_in("agents", data, "response"):
        return False
    if not assert_type(data["agents"], int, "agents type"):
        return False
    if data["agents"] < 1:
...
def test_agents_list():
    ...
    if not assert_eq(status, 200, "status"):
        return False
    if not assert_in("count", data, "response"):
        return False
    if not assert_in("agents", data, "response"):
        return False
    if not assert_type(data["agents"], list, "agents type"):
        return False
...
def test_agent_by_id():
    ...
    if not assert_eq(status, 200, "status"):
        return False
    if not assert_eq(data["id"], valid_id, "id matches"):
        return False
    if not assert_in("name", data, "response"):
        return False
    return True
...
def test_study_types():
    ...
    if not assert_eq(status, 200, "status"):
        return False
    if not assert_in("study_types", data, "response"):
        return False
    if not assert_type(data["study_types"], list, "study_types type"):
        return False
...
def test_knowledge():
    ...
    if not assert_eq(status, 200, "status"):
        return False
    if not assert_in("etap", data, "response"):
        return False
    if not assert_type(data["etap"], dict, "etap type"):
        return False
...
def test_ml_capabilities():
    ...
    if not assert_eq(status, 200, "status"):
        return False
    if not assert_in("success", data, "response"):
        return False
    if not assert_eq(data["success"], True, "success"):
        return False
    if not assert_in("data", data, "response"):
        return False
    if not assert_in("sklearn", data["data"], "data.sklearn"):
        return False
    return True
...
def test_settings_health():
    ...
    if not assert_in("data", data, "response"):
        return False
    if not assert_in("crypto_available", data["data"], "crypto_available"):
        return False
    if not assert_type(data["data"]["crypto_available"], bool, "crypto_available type"):
        return False
    return True
...
def test_settings_keys():
    ...
    if not assert_in("success", data, "response"):
        return False
    if not assert_in("providers", data, "response"):
        return False
    if not assert_type(data["providers"], list, "providers type"):
        return False
    return True
...
def test_chat_expert():
    ...
    if len(response_text) < 100:
        print(
            f"  ✗ FAIL: response too short ({len(response_text)} chars), expected substantive content"
        )
        return False
...
def test_chat_gui():
    status, data = request(
        "POST", "/api/v1/agents/etap-gui/chat", {"question": "Open load flow study"}
    )
    if not assert_eq(status, 200, "status"):
        return False
    if not assert_in("success", data, "response"):
...
def test_study_load_flow():
    ...
    if not assert_eq(status, 200, "status"):
        return False
    if not assert_in("study_type", data, "response"):
        return False
    if not assert_eq(data["study_type"], "load_flow", "study_type value"):
        return False
    if not assert_in("status", data, "response"):
        return False
    return True
...
def test_predict_anomaly():
    status, data = request(
        ...
        {"data": [100, 105, 110, 500, 115, 120], "threshold": 3.0},
    )
    if not assert_eq(status, 200, "status"):
        return False
    if not assert_in("success", data, "response"):
    ...
    if not assert_in("data", data, "response"):
        return False
    if not assert_in("anomalies", data["data"], "anomalies"):
        return False
    if not assert_type(data["data"]["anomalies"], list, "anomalies type"):
        return False
...
def test_context_retrieve():
    status, data = request(
        "POST", "/api/v1/context/retrieve", {"query": "load flow analysis", "top_k": 5}
    )
    if not assert_eq(status, 200, "status"):
        return False
    if not assert_in("success", data, "response"):
    ...
    if not assert_in("count", data, "response"):
        return False
    if not assert_in("chunks", data, "response"):
        return False
    if not assert_type(data["chunks"], list, "chunks type"):
        return False
    return True
...
def test_agent_not_found():
    ...
    if not assert_eq(status, 404, "status should be 404"):
        return False
    if not assert_in("error", data, "response"):
        return False
    return True
...
def test_chat_missing_message():
    ...
    if not assert_eq(status, 422, "status should be 422"):
        return False
    return True
...
def test_study_invalid_type():
    ...
    status, data = request(
        "POST", "/api/v1/studies/run", {"study_type": "invalid_study_type", "parameters": {}}
    )
    if not assert_eq(status, 400, "status should be 400"):
        return False
    return True
...
def test_predict_load_insufficient_data():
    ...
    status, data = request(
        "POST",
        "/api/v1/predict/load",
        {"historical_data": [100, 110, 105, 115, 120], "horizon": 24},
    )
    if not assert_eq(status, 400, "status should be 400 (not 500, not 200)"):
        return False
    if not assert_in("error", data, "response"):
        return False
...
def test_siem_events_not_configured():
    ...
    if not assert_eq(status, 400, "status should be 400 (logging not configured)"):
        return False
    if not assert_in("error", data, "response"):
        return False
    if not assert_eq(data["error"], "logging_only_mode_not_active", "error code"):
        return False
    return True
...
def test_settings_test_key_not_found():
    ...
    if not assert_eq(status, 404, "status should be 404 (key not found)"):
        return False
    if not assert_in("error", data, "response"):
        return False
    if not assert_eq(data["error"], "key_not_found", "error code"):
        return False
    return True
...
if __name__ == "__main__":
    ...
    run_test(
        "NEGATIVE: POST /agents/etap-expert/chat without message → 422", test_chat_missing_message
    )
    run_test("NEGATIVE: POST /studies/run invalid type → 400", test_study_invalid_type)
    run_test(
        "NEGATIVE: POST /predict/load < 48 points → 400 (not 500)",
        test_predict_load_insufficient_data,
    )
    ...
    run_test(
        "NEGATIVE: POST /settings/keys/nonexistent/test → 404", test_settings_test_key_not_found
    )
