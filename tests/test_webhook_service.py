...
class TestSubscriptionValidation:
    ...
    def test_http_rejected_in_production_mode(self):
        ...
        with pytest.raises(
            ValueError, match="HTTP webhooks are not allowed"
        ):  # NOSONAR — S5778: re-raise inside except is intentional (context-specific)
            prod_service.subscribe(
                WebhookSubscription(
                    id="sub-1",
                    url="http://insecure.com/hook",  # NOSONAR: HTTP/WS in test  # NOSONAR — S7632: test function documented via class name / module path
                    secret=os.getenv(
                        "SECRET_KEY"
                    ),  # NOSONAR: hard-coded secret in test fixture  # NOSONAR — S7632: test function documented via class name / module path
                )
    ...
    def test_short_secret_rejected(self, service):
        ...
        with pytest.raises(
            ValueError, match="at least 32 characters"
        ):  # NOSONAR — S5778: re-raise inside except is intentional (context-specific)
            service.subscribe(
                WebhookSubscription(
                    id="sub-1",
    ...
    def test_non_http_scheme_rejected(self, service):
        with pytest.raises(
            ValueError, match="http or https scheme"
        ):  # NOSONAR — S5778: re-raise inside except is intentional (context-specific)
            service.subscribe(
                WebhookSubscription(
                    id="sub-1",
    ...
    def test_host_allowlist_enforced(self):
        ...
        # Disallowed host
        with pytest.raises(
            ValueError, match="not in allowlist"
        ):  # NOSONAR — S5778: re-raise inside except is intentional (context-specific)
            service.subscribe(
                WebhookSubscription(
                    id="sub-2",
                    url="https://forbidden.com/hook",
                    secret=os.getenv(
                        "SECRET_KEY"
                    ),  # NOSONAR: hard-coded secret in test fixture  # NOSONAR — S7632: test function documented via class name / module path
                )
