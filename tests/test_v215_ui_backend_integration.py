...
class TestV215ApiKeysEndpoints:
    ...
    def test_list_keys_endpoint_exists(self, client, admin_headers):
        ...
        assert resp.status_code != 404, "/api/v1/admin/keys not registered — frontend will 404"
        if resp.status_code == 200:
            data = resp.json()
...
class TestV215RouterRegistration:
    ...
    def test_all_four_routers_registered(self, client):
        ...
        assert len(export_paths) >= 1, f"Exports router not registered — found {export_paths}"
...
class TestV215FrontendBackendPathMatch:
    ...
    def test_self_healing_api_uses_correct_path(self):
        ...
        with open(sh_api_path) as f:
            content = f.read()
        assert "/self-healing/" in content, "selfHealingApi.ts must call /self-healing/* paths"
    ...
    def test_sidebar_includes_all_four_pages(self):
        ...
        assert "/self-healing" in content, "Sidebar.tsx must include nav entry for /self-healing"
        # Reports must be accessible (BOQ & Reports in WORKSPACE section)
        assert "/reports" in content, "Sidebar.tsx must include nav entry for /reports"
