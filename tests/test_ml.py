...
class TestFaultPredictor:
    def test_predict_after_train(self):
        ...
        features = np.random.randn(50, 4)  # NOSONAR: numpy.random.Generator migration; API change required
        labels = np.random.randint(0, 4, size=50)  # NOSONAR: numpy.random.Generator migration; API change required
...
class TestAnomalyDetector:
    def test_train_and_detect(self):
        ...
        normal_data = np.random.randn(100, 3)  # NOSONAR: numpy.random.Generator migration; API change required
        detector.train(normal_data)
        result = detector.detect(np.random.randn(10, 3))  # NOSONAR: numpy.random.Generator migration; API change required
    ...
    def test_detect_single_sample(self):
        detector = AnomalyDetector(contamination=0.1)
        normal_data = np.random.randn(50, 3)  # NOSONAR: numpy.random.Generator migration; API change required
        detector.train(normal_data)
