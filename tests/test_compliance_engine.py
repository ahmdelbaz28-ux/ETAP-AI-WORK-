...
class TestDetectorSpacingRule:
    ...
    def test_compliant_spacing_passes(self, engine):
        ...
        spacing_violations = [
            v for v in violations if "17.6.3.1.2" in v and "sloped" not in v.lower()
        ]
    ...
    def test_excessive_spacing_fails(self, engine):
        ...
        assert len(spacing_violations) > 0, "Excessive spacing should be flagged as non-compliant"
...
class TestSlopedCeilingRule:
    ...
    def test_sloped_ceiling_within_limit(self, engine):
        ...
        violations = engine.validate(context)
        sloped_violations = [v for v in violations if "17.6.3.1.2(a)" in v]
    ...
    def test_sloped_ceiling_exceeds_limit(self, engine):
        ...
        violations = engine.validate(context)
        sloped_violations = [v for v in violations if "17.6.3.1.2(a)" in v]
        assert len(sloped_violations) > 0, "Sloped ceiling at 9.1m spacing should be flagged"
    ...
    def test_flat_ceiling_not_affected_by_sloped_rule(self, engine):
        ...
        violations = engine.validate(context)
        sloped_violations = [v for v in violations if "17.6.3.1.2(a)" in v]
        assert len(sloped_violations) == 0, "Flat ceiling should not trigger sloped rule"
...
class TestCoverageRule:
    ...
    def test_sufficient_coverage_passes(self, engine):
        ...
        violations = engine.validate(context)
        coverage_violations = [
            v for v in violations if "17.6.3.1.1" in v and "coverage" in v.lower()
        ]
    ...
    def test_insufficient_coverage_fails(self, engine):
        ...
        assert len(coverage_violations) > 0, "95% coverage should be flagged as non-compliant"
...
class TestBranchVoltageDropRule:
    ...
    def test_compliant_voltage_drop(self, engine):
        ...
        assert len(branch_violations) == 0, f"2.5% drop should pass, got: {branch_violations}"
    ...
    def test_excessive_voltage_drop(self, engine):
        ...
        violations = engine.validate(context)
        branch_violations = [v for v in violations if "210.19" in v]
        assert len(branch_violations) > 0, "4% drop should be flagged as non-compliant"
...
class TestTotalVoltageDropRule:
    ...
    def test_compliant_total_drop(self, engine):
        ...
        assert len(total_violations) == 0, f"4% total drop should pass, got: {total_violations}"
    ...
    def test_excessive_total_drop(self, engine):
        ...
        violations = engine.validate(context)
        total_violations = [v for v in violations if "215.2" in v]
        assert len(total_violations) > 0, "6% total drop should be flagged"
...
class TestTerminalVoltageRule:
    ...
    def test_insufficient_terminal_voltage(self, engine):
        ...
        violations = engine.validate(context)
        terminal_violations = [v for v in violations if "10.14" in v]
        assert len(terminal_violations) > 0, "14V terminal should be flagged as non-compliant"
...
class TestStairwellMaxPressureRule:
    ...
    def test_pressure_within_limit(self, engine):
        ...
        assert len(pressure_violations) == 0, f"50 Pa should pass, got: {pressure_violations}"
    ...
    def test_excessive_pressure(self, engine):
        ...
        violations = engine.validate(context)
        pressure_violations = [v for v in violations if "6.4.2" in v]
        assert len(pressure_violations) > 0, "100 Pa should be flagged (door entrapment risk)"
...
class TestStairwellMinPressureRule:
    ...
    def test_sufficient_min_pressure(self, engine):
        ...
        min_violations = [v for v in violations if "NFPA92:6.4" in v and "6.4.2" not in v]
        assert len(min_violations) == 0, f"30 Pa should pass, got: {min_violations}"
    ...
    def test_insufficient_min_pressure(self, engine):
        ...
        violations = engine.validate(context)
        min_violations = [v for v in violations if "NFPA92:6.4" in v and "6.4.2" not in v]
        assert len(min_violations) > 0, "15 Pa should be flagged (smoke infiltration risk)"
    ...
    def test_no_pressurization_not_flagged(self, engine):
        ...
        violations = engine.validate(context)
        min_violations = [v for v in violations if "NFPA92:6.4" in v and "6.4.2" not in v]
        assert len(min_violations) == 0, "No pressurization should not trigger min pressure rule"
...
class TestValidateAndReport:
    ...
    def test_compliance_percentage_calculation(self, engine):
        ...
        context = {}
        report = engine.validate_and_report(context)
        expected_pct = round((1 - report["violation_count"] / len(engine.rules)) * 100, 1)
...
class TestEngineConstruction:
    ...
    def test_engine_has_rules(self, engine):
        ...
        assert len(engine.rules) >= 10, f"Engine should have >=10 rules, got {len(engine.rules)}"
