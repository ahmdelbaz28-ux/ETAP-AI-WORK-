...
class WeatherAgent(BaseAgent):
    ...
    def analyze_wind_impact(
        ...
        """
        T_c = conductor_max_temp_c  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        T_a = ambient_temp_c  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        ...
        T_film = (T_c + T_a) / 2.0 + 273.15  # K  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        k_air = 0.0242 + 7.0e-5 * (T_film - 300.0)  # Thermal conductivity W/(m·K)
        nu_air = 1.516e-5 + 4.0e-8 * (T_film - 300.0)  # Kinematic viscosity m²/s

        # Reynolds number
        Re = wind_speed_ms * D / nu_air if nu_air > 0 else 0.0  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability

        # Forced convection coefficient (simplified IEEE 738)
        if Re > 0:
            Nu = 0.3 + 0.62 * Re**0.5 * 0.71 ** (1.0 / 3.0)  # Simplified  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
            h_conv = Nu * k_air / D
        else:
        ...
        T_c_K = T_c + 273.15  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        T_a_K = T_a + 273.15  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        ...
        I_dynamic = float(np.sqrt(q_joule / R)) if q_joule > 0 and R > 0 else 0.0  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        ...
        I_dlr_simple = static_rating_a * dlr_factor  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
