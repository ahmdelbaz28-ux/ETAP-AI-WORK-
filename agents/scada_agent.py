...
class SCADAAgent(BaseAgent):
    ...
    def read_measurements(
        ...
        result_measurements = []
        for m in filtered:
            noise = _RNG.normal(  # S6711 legacy RandomState kept for deterministic seed behaviour NOSONAR
                0, 0.005
            )  # 0.5% noise  # NOSONAR
    ...
    def _generate_simulated_measurements(
        ...
        for bus_id in ["BUS1", "BUS2", "BUS3"]:
            v_nom = 13.8  # kV
            v_kv = v_nom * (
                1.0 + _RNG.normal(0, 0.02)  # S6711 legacy RandomState kept for deterministic seed behaviour NOSONAR
            )  # NOSONAR
            ...
            measurements.append(  # NOSONAR S6711: numpy.random legacy function used for non-crypto simulation; np.random.Generator migration is tracked separately
                SCADAMeasurement(
                    tag=f"A_{bus_id}_A",
                    value=500
                    + _RNG.normal(  # S6711 legacy RandomState kept for deterministic seed behaviour NOSONAR
                        0, 10
                    ),  # NOSONAR
                    timestamp=timestamp,
                    quality="good",
