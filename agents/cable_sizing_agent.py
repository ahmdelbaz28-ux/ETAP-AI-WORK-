...
class CableSizingAgent(BaseAgent):
    ...
    def validate_result(self, result: AgentResult) -> bool:
        ...
        if (
            amp is not None and amp.get("derated_ampacity_A", 0) <= 0
        ):  # NOSONAR — S1066: not collapsible; `and` short-circuits the .get() on None
            errors.append("Derated ampacity is zero or negative")

        vd = result.data.get("voltage_drop")
        if vd is not None and not vd.get(
            "compliant_5pct", True
        ):  # NOSONAR — S1066: not collapsible; `and` short-circuits the .get() on None
            errors.append(
        ...
        if sc is not None and not sc.get(
            "adequate", True
        ):  # NOSONAR — S1066: not collapsible; `and` short-circuits the .get() on None
            errors.append(
        ...
        if rec is not None and "No standard cable" in rec.get(
            "recommendation", ""
        ):  # NOSONAR — S1066: not collapsible; `and` short-circuits the .get() on None
