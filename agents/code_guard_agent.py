...
class CodeGuardAgent(BaseAgent):
    ...
    def _initialize_guards(self) -> None:
        ...
        try:
            ...
            self.logger.warning("Guards module not available: %s. Agent will operate in fallback mode.", e)
    ...
    async def execute(self, task: EngineeringTask) -> AgentResult:
        ...
        try:
            ...
            must_fix_total = sum(r.get('must_fix', 0) for r in results.values() if isinstance(r, dict))
            should_fix_total = sum(r.get('should_fix', 0) for r in results.values() if isinstance(r, dict))
