...
class BaseAgent:
    ...
    async def execute(self, task: EngineeringTask) -> AgentResult:
        ...
        self.log_execution(
            f"BaseAgent.execute invoked on {self.agent_name} (no override). "
            f"Task={task.task_id}",
            level="ERROR",
        )
        return AgentResult(
    ...
    async def execute(self, task: EngineeringTask) -> AgentResult:
        ...
        try:
            ...
            if isinstance(system_data, dict):
                raise TypeError("system_data must be a System object instance, not a dictionary. Ensure a valid System object is passed in task parameters.")
            ...
            self.log_execution(f"Load flow completed in {execution_time:.2f}s, "
    ...
    async def execute(self, task: EngineeringTask) -> AgentResult:
        ...
        try:
            ...
            analyzer = FaultAnalyzer(Ybus_pos, Ybus_neg, Ybus_zero,
                                    base_mva=base_mva, base_kv=base_kv)
...
class OptimalPowerFlowAgent(BaseAgent):
    ...
    def validate_result(self, result: AgentResult) -> bool:
        ...
        if balance_error > 1.0:  # Allow 1 MW tolerance
            result.validation_errors.append(
                f"Power balance error: {balance_error:.2f} MW"
            )
            return False
...
class ETAPExecutionAgent(BaseAgent):
    ...
    async def execute(self, task: EngineeringTask) -> AgentResult:
        ...
        try:
            ...
            self.log_execution(f"Executing ETAP task {task.task_id} via {type(self.provider).__name__}")
...
class ValidationAgent(BaseAgent):
    ...
    async def execute(self, task: EngineeringTask) -> AgentResult:
        ...
        try:
            ...
            self.log_execution(f"Validation completed: {validation_summary['passed']}/{validation_summary['total_checks']} passed")
            return result
...
class ReportGenerationAgent(BaseAgent):
    ...
    def _generate_executive_summary(self, report: Dict) -> str:
        ...
        if lf:
            ...
            summary_lines.append(f"Load Flow Analysis: {'Converged' if converged else 'Did Not Converge'}")
    ...
    def _generate_recommendations(self, report: Dict) -> List[str]:
        ...
        if harm.get('resonance_detected'):
            recommendations.append(
                "Install passive harmonic filters to mitigate resonance"
            )
        ...
        for study_type in dependent_studies:
            ...
            if agent:
                ...
                if not result.validation_status:
                    self.logger.warning(
                        f"Validation failed for {study_type.value}: "
                ...
                for pr in parallel_results:
                    if isinstance(pr, Exception):
                        ...
                        if not pr.validation_status:
                            self.logger.warning(
                                f"Validation failed: {pr.validation_errors}"
                            )
        ...
        *benchmark* mode also executes the same studies sequentially
        ...
        for study_str in study_types:
            ...
            if agent_key is None:
                self.logger.warning(
                    "Unknown study type '%s' – skipping", study_str
                )
                continue
        ...
        task_id = (
            f"parallel_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}"
        )
        ...
        def _make_task(study_str: str, agent_key: str) -> EngineeringTask:
            return EngineeringTask(
                ...
                study_types=[
                    s
                    for s in StudyType
                    if s.value == study_str or s.value == agent_key
                ][:1],  # best-effort StudyType match
        ...
        async def _run_with_semaphore(
            ...
            async with semaphore:
                ...
                try:
                    ...
                    self.logger.error(
                        "[parallel] Failed %s: %s", study_str, exc
                    )
        ...
        for item in parallel_raw:
            if isinstance(item, Exception):
                self.logger.error(
                    "[parallel] Unexpected exception: %s", item
                )
                continue
        ...
        if benchmark:
            ...
            for study_str, agent_key, agent in resolved:
                ...
                try:
                    ...
                    self.logger.error(
                        "[sequential] Failed %s: %s", study_str, exc
                    )
            ...
            self.logger.info(
                "Benchmark complete – parallel: %.4fs, sequential: %.4fs, "
                "speedup: %.2fx",
                parallel_time,
                sequential_time,
                speedup,
        ...
        self.logger.info(
            "Parallel studies completed: task_id=%s, studies=%d, "
            "parallel_time=%.4fs",
            task_id,
