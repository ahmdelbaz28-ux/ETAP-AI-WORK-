...
class PredictiveAgent(BaseAgent):
    ...
    def forecast_short_term(
        ...
        for t in range(season_length, n):
            L_new = alpha * (y[t] - S[t % season_length]) + (1 - alpha) * (L + T)  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
            T_new = beta * (L_new - L) + (1 - beta) * T  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        ...
        fitted = []
        L_f = np.mean(y[:season_length])  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        T_f = (  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
            np.mean(y[season_length : 2 * season_length]) - np.mean(y[:season_length])
        ) / season_length
        S_f = y[:season_length] - L_f  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        for t in range(season_length, n):
        ...
        try:
            ...
            if analysis_type in ("short_term_forecast", "full"):
                ...
                if not hist_load:
                    ...
                    hours = 168  # 1 week
                    hist_load = [
                        100.0 + 30.0 * np.sin(2 * np.pi * h / 24) + 5.0 * np.random.randn()  # NOSONAR: numpy.random.Generator migration; API change required
