...
class ValidationCampaign:
    ...
    def validate_ieee_3bus(self):
        ...
        if converged:
            for bid in sorted(system.buses.keys()):
                ...
                self._record("3-Bus LF", f"Bus {bid} Voltage", in_range,
            ...
            self._record("3-Bus LF", "Power Balance", True,
                         f"Total Load={total_load:.4f}, Total Gen={total_gen:.4f}, Losses={P_loss:.4f} pu")

            # Ybus symmetry check
            Ybus = solver.Ybus
            sym = np.allclose(Ybus, Ybus.conj().T)
            self._record("3-Bus LF", "Ybus Symmetry", sym,
                         f"Ybus is symmetric: {sym}")
    ...
    def validate_ieee_5bus(self):
        ...
        system = System(base_mva=100.0)
        buses = {}
        bus_data = {1: ('slack', 1.06), 2: ('pv', 1.045), 3: ('pq', 1.0),
                    4: ('pq', 1.0), 5: ('pq', 1.0)}
        ...
        gen1 = Generator(generator_id=1, bus=buses[1],
                         impedance={'1': complex(0,0.25), '2': complex(0,0.25), '0': complex(0,0.1)})
        gen2 = Generator(generator_id=2, bus=buses[2],
        ...
        line_data = [
            (1, 2, complex(0.02, 0.06)), (1, 3, complex(0.08, 0.24)),
            (2, 3, complex(0.06, 0.18)), (2, 4, complex(0.06, 0.18)),
            (2, 5, complex(0.04, 0.12)), (3, 4, complex(0.01, 0.03)),
        ...
        if converged:
            for bid in sorted(system.buses.keys()):
                ...
                self._record("5-Bus LF", f"Bus {bid} Voltage", in_range,
                             f"|V|={v:.6f} pu")
    ...
    def validate_ieee_14bus(self):
        ...
        line_data = [
            (1,2,complex(0.01938,0.05917)), (1,5,complex(0.05403,0.22304)),
            (2,3,complex(0.04699,0.19797)), (2,4,complex(0.05811,0.17632)),
            (2,5,complex(0.05695,0.17388)), (3,4,complex(0.06701,0.17103)),
            (4,5,complex(0.01335,0.04211)), (4,7,complex(0.0,0.20912)),
            (4,9,complex(0.0,0.55618)), (5,6,complex(0.0,0.25202)),
            (6,11,complex(0.09498,0.19890)), (6,12,complex(0.12291,0.25581)),
            (6,13,complex(0.06615,0.13027)), (7,8,complex(0.0,0.17615)),
            (7,9,complex(0.0,0.11001)), (9,10,complex(0.03181,0.08450)),
            (9,14,complex(0.12711,0.27038)), (10,11,complex(0.08205,0.19207)),
            (12,13,complex(0.22092,0.19988)), (13,14,complex(0.17093,0.34802)),
        ]
        for lid, (fb, tb, z1) in enumerate(line_data, 1):
            system.add_line(Line(line_id=lid, from_bus=buses[fb], to_bus=buses[tb], z1=z1))
        ...
        if converged:
            ...
            self._record("14-Bus LF", "All Voltages in Range", all_ok,
    ...
    def validate_ieee_30bus(self):
        ...
        for gid, bid in enumerate(gen_buses, 1):
            gen = Generator(generator_id=gid, bus=system.buses[bid],
                            impedance={'1': complex(0,0.2), '2': complex(0,0.2), '0': complex(0,0.1)})
            system.add_generator(gen)
        ...
        # Loads (simplified)
        load_data = {
            2: complex(0.217, 0.127), 3: complex(0.024, 0.012), 4: complex(0.076, 0.016),
            5: complex(0.942, 0.19), 7: complex(0.228, 0.109), 8: complex(0.30, 0.30),
            10: complex(0.058, 0.02), 12: complex(0.112, 0.075), 14: complex(0.062, 0.016),
            15: complex(0.082, 0.019), 16: complex(0.035, 0.018), 17: complex(0.09, 0.058),
            18: complex(0.032, 0.009), 19: complex(0.095, 0.034), 20: complex(0.022, 0.007),
            21: complex(0.175, 0.112), 23: complex(0.032, 0.016), 24: complex(0.087, 0.067),
            26: complex(0.035, 0.023), 29: complex(0.024, 0.009), 30: complex(0.106, 0.019),
        }
        for lid, (bid, lp) in enumerate(load_data.items(), 1):
        ...
        line_data = [
            (1,2,complex(0.0192,0.0575)), (1,3,complex(0.0452,0.1852)),
            (2,4,complex(0.0570,0.1739)), (2,5,complex(0.0132,0.0379)),
            (2,6,complex(0.0472,0.1498)), (3,4,complex(0.0587,0.1672)),
            (4,6,complex(0.0119,0.0414)), (5,7,complex(0.0460,0.1460)),
            (6,7,complex(0.0267,0.0820)), (6,8,complex(0.0120,0.0420)),
            (6,9,complex(0.0,0.2080)), (6,10,complex(0.0,0.5560)),
            (9,10,complex(0.0,0.1100)), (9,11,complex(0.0,0.2080)),
        ]
        for lid, (fb, tb, z1) in enumerate(line_data, 1):
            system.add_line(Line(line_id=lid, from_bus=system.buses[fb], to_bus=system.buses[tb], z1=z1))
        ...
        if converged:
            ...
            for bid in sorted(system.buses.keys()):
                ...
                if not (0.85 <= v <= 1.15):
                    all_ok = False
            self._record("30-Bus LF", "All Voltages in Range", all_ok,
    ...
    def validate_short_circuit(self):
        ...
        self._record("SC", "DLG Fault Currents > 0", Ib > 0 and Ic > 0,
    ...
    def validate_arc_flash(self):
        ...
        try:
            ...
            self._record("ArcFlash", "Arc Current > 0 (4.16kV, 20kA)", Iarc > 0,
                         f"Iarc={Iarc:.4f} kA, Iarc_red={Iarc_red:.4f} kA")
            # Arc current should be less than bolted fault current
            self._record("ArcFlash", "Iarc < Ibf", Iarc < 20.0,
                         f"Iarc={Iarc:.4f} < Ibf=20.0 kA")
            # Reduced should be 85% of full
            ratio = Iarc_red / Iarc if Iarc > 0 else 0
            self._record("ArcFlash", "Reduced = 85% of Full", abs(ratio - 0.85) < 0.01,
        ...
        try:
            ...
            self._record("ArcFlash", "Incident Energy > 0", E_final > 0,
                         f"E={E_final:.4f} cal/cm2")
            self._record("ArcFlash", "E_full > 0 and E_red > 0", E_full > 0 and E_red > 0,
        ...
        try:
            ...
            self._record("ArcFlash", "Boundary > Working Distance", D_boundary > 0,
        ...
        try:
            result = engine.calculate(4.16, 20.0, 0.5, 610.0, ElectrodeConfig.VCB, EnclosureType.BOX)
            self._record("ArcFlash", "Complete Analysis", True,
        ...
        try:
            ...
            self._record("ArcFlash", "Ralph Lee Method", result.incident_energy_cal_cm2 > 0,
        ...
        for config in ElectrodeConfig:
            try:
                ...
                self._record("ArcFlash", f"Arc Current {config.value}", Iarc > 0,
                             f"Iarc={Iarc:.4f} kA")
            except Exception as e:
    ...
    def validate_protection_coordination(self):
        ...
        self._record("ProtCoord", "IEC 60255 SI Curve", t_si > 0,
                     f"t={t_si:.4f}s at TMS=1.0, I/Ip=10")

        t_vi = curves.very_inverse(1.0, 10.0, 1.0)
        self._record("ProtCoord", "IEC 60255 VI Curve", t_vi > 0 and t_vi < t_si,
                     f"t={t_vi:.4f}s (should be < SI)")

        t_ei = curves.extremely_inverse(1.0, 10.0, 1.0)
        self._record("ProtCoord", "IEC 60255 EI Curve", t_ei > 0 and t_ei < t_vi,
                     f"t={t_ei:.4f}s (should be < VI)")

        # Coordination check
        fault_currents = [2.0, 5.0, 10.0, 20.0]
        results = coord_engine.check_coordination_range(relay_upstream, relay_downstream, fault_currents)

        all_coordinated = all(r['coordinated'] for r in results)
        self._record("ProtCoord", "51 Relay Coordination", all_coordinated,
                     f"All coordinated: {all_coordinated}")

        # Verify selectivity (downstream trips first)
        for i, r in enumerate(results):
            self._record("ProtCoord", f"Selectivity at If={fault_currents[i]}",
        ...
        for i, r in enumerate(results):
            self._record("ProtCoord", f"Margin at If={fault_currents[i]}",
                         r['margin'] >= 0.2,
                         f"margin={r['margin']:.4f}s (required >= 0.2)")

        # TMS adjustment suggestion
        suggested_tms = coord_engine.suggest_tms_adjustment(relay_upstream, relay_downstream, fault_currents)
        self._record("ProtCoord", "TMS Suggestion", suggested_tms is not None,
                     f"Suggested TMS={suggested_tms}")

        # Test 67 Relay (Directional)
        from relays.relay import DirectionalRelay
        dir_relay = DirectionalRelay(relay_id=3, name='Dir-67', voltage_threshold=0.1, angle_offset=30)
        ...
        self._record("ProtCoord", "67 Relay Forward Direction", picked_up,
        ...
        self._record("ProtCoord", "21 Relay Impedance Check", picked_up_dist == (abs(Z_measured) < 0.5),
        ...
        self._record("ProtCoord", "87 Relay Internal Fault", picked_up_int,
        ...
        self._record("ProtCoord", "87 Relay External Fault Restraint", not picked_up_ext,
    ...
    def validate_numerical_stability(self):
        ...
        solver = LoadFlowSolver(system)
        converged = solver.solve(max_iter=200, tol=1e-6)
        self._record("Stability", "High R/X Convergence", converged,
        ...
        self._record("Stability", "Weak Grid Convergence", converged2,
        ...
        for lid in range(1, 10):
            system3.add_line(Line(line_id=lid,
        ...
        self._record("Stability", "10-Bus Radial Convergence", converged3,
        ...
        if converged3:
            ...
            self._record("Stability", "Radial Voltage Drop", v10 < v1,
        ...
        gen = Generator(generator_id=1, bus=system4.buses[2],
                        impedance={'1': complex(0,0.2), '2': complex(0,0.2), '0': complex(0,0.1)})
        ...
        mesh_lines = [(1,2), (1,3), (1,4), (1,5), (2,3), (2,4), (2,5), (3,4), (3,5), (4,5)]
        for lid, (fb, tb) in enumerate(mesh_lines, 1):
            system4.add_line(Line(line_id=lid,
        ...
        self._record("Stability", "5-Bus Meshed Convergence", converged4,
                     f"Converged={converged4}")
    ...
    def run_full_campaign(self):
        ...
        print(f"  Pass Rate: {self.passed/total_tests*100:.1f}%" if total_tests > 0 else "  No tests run")
        print("="*70)
