...
function logTestResult(testName, action, result, details = {}) {
  ...
  // Log to console
  console.log(`[${result.status}] ${testName}: ${action} (${result.duration}ms)`);
  if (result.error) {
...
async function testDashboardPage(page) {
  ...
  for (const test of dashboardTests) {
    try {
      ...
      await page.waitForSelector(test.selector, { timeout: 3000 }).catch(() => {});
...
async function testProjectsPage(page) {
  ...
  await page.waitForSelector('[data-testid="projects-list"]', { timeout: 5000 }).catch(() => {});
  ...
  try {
    await page.waitForSelector(createProjectTest.selector, { timeout: 3000 }).catch(() => {});
  ...
  try {
    await page.waitForSelector(loadProjectsTest.selector, { timeout: 3000 }).catch(() => {});
...
async function testAutoCADPage(page) {
  ...
  await page.waitForSelector('[data-testid="autocad-panel"]', { timeout: 5000 }).catch(() => {});
  ...
  const connectAutoCADTest = {
    ...
    apiCall: () => makeApiRequest('/api/v1/autocad/connect', { method: 'POST', body: {} }),
    action: 'Click connect AutoCAD button'
  };
  
  try {
    await page.waitForSelector(connectAutoCADTest.selector, { timeout: 3000 }).catch(() => {});
    const result = await connectAutoCADTest.apiCall();
    logTestResult(connectAutoCADTest.testName, connectAutoCADTest.action, result);
  } catch (error) {
  ...
  try {
    await page.waitForSelector(loadDocumentsTest.selector, { timeout: 3000 }).catch(() => {});
...
async function testRevitPage(page) {
  ...
  await page.waitForSelector('[data-testid="revit-panel"]', { timeout: 5000 }).catch(() => {});
  
  // Test connecting to Revit
  const connectRevitTest = {
    testName: 'Connect Revit Button',
    selector: '[data-testid="connect-revit-btn"]',
    apiCall: () => makeApiRequest('/api/v1/revit/connect', { method: 'POST', body: {} }),
    action: 'Click connect Revit button'
  };
  
  try {
    await page.waitForSelector(connectRevitTest.selector, { timeout: 3000 }).catch(() => {});
  ...
  try {
    await page.waitForSelector(loadElementsTest.selector, { timeout: 3000 }).catch(() => {});
...
async function testDigitalTwinPage(page) {
  ...
  await page.waitForSelector('[data-testid="digital-twin-panel"]', { timeout: 5000 }).catch(() => {});
  
  // Test conversion
  const convertTest = {
    testName: 'Digital Twin Convert Button',
    selector: '[data-testid="convert-btn"]',
    apiCall: () => makeApiRequest('/api/v1/digital-twin/convert', {
      method: 'POST',
      body: {
        sourceFormat: 'dwg',
        targetFormat: 'rvt',
  ...
  try {
    await page.waitForSelector(convertTest.selector, { timeout: 3000 }).catch(() => {});
  ...
  try {
    await page.waitForSelector(historyTest.selector, { timeout: 3000 }).catch(() => {});
...
async function testElementsPage(page) {
  ...
  await page.waitForSelector('[data-testid="elements-grid"]', { timeout: 5000 }).catch(() => {});
  ...
  try {
    await page.waitForSelector(elementsTest.selector, { timeout: 3000 }).catch(() => {});
...
async function testConnectionsPage(page) {
  ...
  await page.waitForSelector('[data-testid="connections-table"]', { timeout: 5000 }).catch(() => {});
  ...
  try {
    await page.waitForSelector(connectionsTest.selector, { timeout: 3000 }).catch(() => {});
...
async function testConflictsPage(page) {
  ...
  await page.waitForSelector('[data-testid="conflicts-list"]', { timeout: 5000 }).catch(() => {});
  ...
  try {
    await page.waitForSelector(conflictsTest.selector, { timeout: 3000 }).catch(() => {});
...
async function testReportsPage(page) {
  ...
  await page.waitForSelector('[data-testid="reports-section"]', { timeout: 5000 }).catch(() => {});
  
  // Test generating report
  const reportTest = {
    testName: 'Generate Report',
    selector: '[data-testid="generate-report-btn"]',
    apiCall: () => makeApiRequest('/api/v1/reports/generate', {
      method: 'POST',
      body: {
        reportType: 'summary',
        filters: {}
  ...
  try {
    await page.waitForSelector(reportTest.selector, { timeout: 3000 }).catch(() => {});
...
## Failed Tests
${testReport.errors.length ? testReport.errors.map(error => `
### ${error.testName}
- **Action:** ${error.action}
