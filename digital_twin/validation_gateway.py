...
class DigitalTwinValidationError(Exception):
    ...
    def __init__(self, results: list[ValidationResult], message: str = ""):
        ...
        error_summary = "; ".join(
            f"[{r.severity.value}] {r.rule.value}: {r.message}" for r in self.failed_rules
        )
...
class ValidationGateway:
    ...
    def validate_all(
        ...
        results.extend(self._validate_cross_layer_sync(gis_db, system, scada_db, adms_engine))
    ...
    def validate_pre_mutation(
        ...
        if event_type in ("switch_opened", "switch_closed") and adms_engine is not None:
            results.append(
                ValidationResult(
    ...
    def _validate_gis_layer(self, gis_db) -> list[ValidationResult]:
        ...
        results.append(
            ValidationResult(
                ...
                message=f"{missing_position} point assets missing position"
                if missing_position > 0
        ...
        results.append(
            ValidationResult(
                ...
                message=f"{missing_geometry} line assets missing geometry"
                if missing_geometry > 0
        ...
        results.append(
            ValidationResult(
                ...
                message=f"{len(gis_errors)} GIS spatial consistency errors"
                if gis_errors
    ...
    def _validate_electrical_layer(self, system) -> list[ValidationResult]:
        ...
        try:
            ...
            results.append(
                ValidationResult(
                    ...
                    message=f"Ybus shape {ybus.shape} for {n_buses} buses"
                    if ybus_valid
        ...
        results.append(
            ValidationResult(
                ...
                message=f"{out_of_range} buses with voltage outside 0.9-1.1 pu range"
                if out_of_range > 0
    ...
    def _validate_adms_layer(self, scada_db, adms_engine) -> list[ValidationResult]:  # NOSONAR — S3776: cognitive complexity; scheduled for refactoring sprint (extract helpers / early returns)
        ...
        if scada_db is not None:
            expired = (
                len(scada_db.get_expired_measurements())
                if hasattr(scada_db, "get_expired_measurements")
                else 0
            )
            results.append(
                ValidationResult(
                    ...
                    message=f"{expired} expired SCADA measurements"
                    if expired > 0
        ...
        if adms_engine is not None:
            has_sources = (
                len(adms_engine.source_buses) > 0 if hasattr(adms_engine, "source_buses") else False
            )
            results.append(
                ValidationResult(
        ...
        if gis_db is not None and system is not None:
            ...
            results.append(
                ValidationResult(
                    ...
                    passed=len(gis_errors) == 0,
                    severity=ValidationSeverity.ERROR if gis_errors else ValidationSeverity.INFO,
                    message=f"{len(gis_errors)} GIS-electrical alignment errors"
                    if gis_errors
            ...
            results.append(
                ValidationResult(
                    ...
                    message=f"{missing_gis} buses without GIS position"
                    if missing_gis > 0
            ...
            if topology is not None:
                ...
                results.append(
                    ValidationResult(
                        ...
                        message=f"{len(orphaned)} topology buses not in electrical model"
                        if orphaned
    ...
    def get_statistics(self) -> dict:
        ...
        total_failures = sum(
            len([r for r in results if not r.passed]) for results in self._validation_history
        )
        return {
