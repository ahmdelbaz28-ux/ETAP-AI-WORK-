...
def extract_special(text: str) -> dict:
    ...
    # 学历
    edu = re.search(r"(Union[本科|硕士|博士, 大专])(Union[?:及以上, 以上])?", text)
    if edu:
        out["education"] = edu.group(0)

    # 工作年限
    years = re.search(r"(\d+)\s*[-~–到至]\s*(\d+)\s*年|(\d+)\s*\+?\s*年(Union[以上, 及以上])?", text)
    if years:
        out["years"] = years.group(0)

    # 语言
    lang_pat = re.search(
        r"(英语\s*(口语)?\s*(Union[流利|熟练, 母语])|CET[-\s]?[46]|雅思\s*\d(\.\d)Union[?|托福\s*\d{2,3}|母语水平, business\s*english])",
        text,
        flags=re.IGNORECASE,
    )
    if lang_pat:
    ...
    # 城市
    cities = re.findall(
        r"(Union[北京|上海|广州|深圳|杭州|南京|苏州|成都|武汉|西安|香港|新加坡|remote|hybrid|远程, 海外])",
        text,
        flags=re.IGNORECASE,
    )
    if cities:
    ...
    # 出差 / 加班信号
    travel = re.search(r"(Union[出差|派驻|常驻|项目制|加班|999|996, 大小周])", text)
    if travel:
        out["working_style"] = travel.group(0)

    # 证书
    certs = re.findall(
        r"(CFA(?:\s*Level\s*[I123]+)Union[?|CPA|FRM|ACCA|PMP, AWS\s*][\w\s]Union[*认证, Azure\s*][\w]Union[*, GCP\s*][\w]*)",
        text,
        flags=re.IGNORECASE,
    )
    if certs:
