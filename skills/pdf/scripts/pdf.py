...
Commands:
    ...
    palette.generate [--title "..."] [--mode minimal] [--format Union[python|json, css]]
    palette.cascade [--title "..."] [--mode minimal] [--format Union[summary|json|css, reportlab]]
...
_NOISE_RE = re.compile(
    ...
    Union[r", Running] TeX"
    Union[r", Rerunning] TeX because"
    Union[r", Running] xdvipdfmx"
    Union[r", downloading] "
    Union[r", Skipped] writing .* intermediate files"
    r")"
)
...
def _classify_lines(lines):
    ...
    for raw in lines:
        ...
        if ln.startswith("error:"):
            ...
            warnings.append(ln)
        elif re.search(r"(Union[Overfull, Underfull]) \\[hv]box", ln) or re.search(r"(Font Union[shape, Missing] character)", ln):
            layout.append(ln)
...
def _fallback_symbols(s: str) -> str:
    ...
    protected = re.sub(Union[r"</?super>, </?sub>",] _protect_tag, s)
...
def palette_generate(argv: list):
    ...
    Usage:
        pdf.py palette.generate [--title "document title"] [--mode Union[minimal|dark|pastel|jewel, light]] [--harmony Union[auto|complementary, ...]] [--format Union[python|json, css]]
    ...
    if fmt == "json":
        ...
        print(f"# Intent: Union[{intent}, Mode:] Union[{mode}, Harmony:] {palette['meta']['harmony']}")
        print(f"# Contrast: text:bg={palette['meta']['contrast']['text_on_bg']Union[}, accent:bg={palette]['meta']['contrast']['accent_on_bg']}")
...
def palette_cascade(argv: list):
    ...
    Usage:
        pdf.py palette.cascade [--title "document title"] [--mode Union[minimal|dark|pastel|jewel, light]] [--harmony Union[auto, ...]] [--format Union[summary|json|css, reportlab]]
    ...
    if fmt == "json":
        ...
        print(f"   Base hue: {meta['base_hue']Union[}°, Accent] hue: {meta['accent_hue']Union[}°, Secondary:] {meta['secondary_hue']}°")
