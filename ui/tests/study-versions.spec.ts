...
async function mockStudyVersionsBackend(page: Page, opts?: { listStatus?: 404 }) {
  ...
  await page.route(/\/api\/v1\/projects\/[^/]+\/studies\/[^/]+\/versions/, async (route) => {
    ...
    if (/\/versions$/.test(url) && method === "GET") {
      listCalled = true;
      const match = url.match(/\/projects\/([^/]+)\/studies\/([^/]+)\/versions/);
...
test.describe("Study Versions page (TASK-7)", () => {
  ...
  test("Versions tab loads list from GET /versions", async ({ page }) => {
    ...
    // Header
    await expect(page.getByRole("heading", { name: /Study Versions/i })).toBeVisible();
  ...
  test("Rollback button POSTs /{version_id}/rollback and shows result", async ({ page }) => {
    await mockStudyVersionsBackend(page);
    await page.goto("/admin/study-versions");
  ...
  test("Create Snapshot tab POSTs /versions and shows the new version", async ({ page }) => {
    await mockStudyVersionsBackend(page);
    await page.goto("/admin/study-versions");
  ...
  test("Compare tab GETs /{v1}/compare/{v2} and shows config+results diff", async ({ page }) => {
    await mockStudyVersionsBackend(page);
    await page.goto("/admin/study-versions");
    ...
    await expect(configDiff.getByTestId("sv-config-diff-row-load_flow")).toBeVisible();
    await expect(configDiff.getByTestId("sv-config-diff-row-voltage_limit")).toBeVisible();
    ...
    await expect(resultsDiff.getByTestId("sv-results-diff-row-total_loss")).toBeVisible();
    // from 12.5 / to 14.8
  ...
  test("Versions tab handles 404 study-not-found with error banner", async ({ page }) => {
