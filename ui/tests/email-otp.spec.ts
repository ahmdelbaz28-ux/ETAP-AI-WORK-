...
let sendBody: { email?: string; purpose?: string; user_name?: string } | null =
	null;
let verifyCalled = false;
let verifyBody: { email?: string; purpose?: string; code?: string } | null =
	null;
...
test.describe("Email OTP page (TASK-8)", () => {
	...
	test("Send tab POSTs /send and shows success + test_code (test mode)", async ({
		page,
	}) => {
		...
		// Header
		await expect(
			page.getByRole("heading", { name: /Email OTP/i }),
		).toBeVisible();
		...
		await expect(
			page.getByText(/OTP sent to user@example\.com/).last(),
		).toBeVisible({
			timeout: 5_000,
		});
	});

	test("Send tab handles 429 rate_limited with error banner", async ({
		page,
	}) => {
	...
	test("Verify tab POSTs /verify and shows verified_email", async ({
		page,
	}) => {
		await mockEmailOtpBackend(page);
		await page.goto("/admin/email-otp");
		...
		await expect(
			page.getByText(/OTP verified for user@example\.com/).last(),
		).toBeVisible({
			timeout: 5_000,
		});
	});

	test("Verify tab handles 400 wrong_code with error banner", async ({
		page,
	}) => {
		await mockEmailOtpBackend(page);
		await page.goto("/admin/email-otp");
	...
	test("Invalidate tab POSTs /invalidate (query params) and shows success", async ({
		page,
	}) => {
		await mockEmailOtpBackend(page);
		await page.goto("/admin/email-otp");
		...
		await expect
			.poll(() => invalidateEmail, { timeout: 5_000 })
