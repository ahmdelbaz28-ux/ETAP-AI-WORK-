...
function write(level: LogLevel, source: string, message: string, data?: Record<string, unknown>) {
  ...
  if (typeof window !== 'undefined' && (window as any).__etapTelemetry) {
    ;(window as any).__etapTelemetry(entry)
  }
}
...
export const logger = {
  ...
  error: (source: string, message: string, data?: Record<string, unknown>) => write('error', source, message, data),
  getBuffer: (): readonly LogEntry[] => _buffer,
  clear: () => { _buffer.length = 0 },
}

export function installTelemetry(handler: (entry: LogEntry) => void) {
  (window as any).__etapTelemetry = handler
}
