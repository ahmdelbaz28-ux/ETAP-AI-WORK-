...
const DEMO_AGENTS = [
  {
    ...
    description: 'Orchestrates multi-agent engineering workflows',
    capabilities: ['load_flow', 'short_circuit', 'protection', 'arc_flash'],
  ...
  },
  {
    ...
    description: 'Newton-Raphson power flow analysis',
    capabilities: ['load_flow', 'voltage_profile', 'power_loss'],
    model: 'gpt-4o',
    provider: 'OpenAI',
  },
  {
    ...
    description: 'IEC 60909 fault analysis',
    capabilities: ['short_circuit', 'fault_current'],
    model: 'claude-3-5-sonnet',
    provider: 'Anthropic',
  },
  {
    ...
    description: 'IEEE 1584 incident energy calculation',
    capabilities: ['arc_flash', 'ppe_category'],
    model: 'gpt-4o',
    provider: 'OpenAI',
  },
  {
    ...
    description: 'IEC 60255 relay coordination',
    capabilities: ['protection_coordination', 'relay_curves'],
    model: 'glm-4',
    provider: 'ZhipuAI',
  },
  {
    id: 'harmonic-agent',
    name: 'Harmonic Agent',
    description: 'THD/TDD analysis per IEEE 519',
    capabilities: ['harmonic_analysis', 'thd_calc'],
    model: 'qwen-max',
    provider: 'Alibaba',
  },
  {
    ...
    description: 'Voltage drop & torque analysis',
    capabilities: ['motor_starting'],
    model: 'deepseek-coder',
    provider: 'DeepSeek',
  },
  {
    id: 'etap-expert-agent',
    name: 'ETAP Expert Agent',
    description: 'ETAP integration specialist',
    capabilities: ['etap_com', 'study_execution'],
    model: 'claude-3-5-sonnet',
    provider: 'Anthropic',
  },
];
...
const DEMO_AUDIT = [
  ...
  {
    timestamp: '2026-06-09T11:18:00Z',
    method: 'GET',
    path: '/api/v1/studies',
    statusCode: 200,
    action: 'list_studies',
...
const DEMO_METRICS = {
  ...
  circuits: {
    'provider-anthropic': { state: 'closed', consecutiveFailures: 0 },
    'provider-openai': { state: 'closed', consecutiveFailures: 0 },
  },
};
...
async function demoResponse<T>(path: string, options?: RequestInit): Promise<T> {
  ...
  if (path === '/api/v1/studies/run' && method === 'POST') {
    ...
    return {
      ...
      data: {
        voltage_profile: [
          { bus: 1, voltage_pu: 1.05 },
          { bus: 2, voltage_pu: 1.0 },
          { bus: 3, voltage_pu: 0.98 },
        ], // NOSONAR — S7748: number literal trailing zero; cosmetic
        power_flow: [
          { from: 1, to: 2, mw: 5.2, mvar: 1.1 },
          { from: 2, to: 3, mw: 4.8, mvar: 0.9 },
        ],
        ...
        code_guard: {
          ...
          violations: [
            ...
            },
            {
              ...
              description: 'Numeric literal without named constant',
              location: 'line 17',
      ...
      data: {
        guards: {
          code_guard: {
            ...
            description: '23 clean-code rules',
            rules_checked: 23,
          },
          test_guard: { name: 'Test Quality Guard', description: '9 test rules', rules_checked: 9 },
          docs_guard: {
            name: 'Docs Quality Guard',
            description: '10 documentation rules',
            rules_checked: 10,
          },
        },
        severity_levels: {
