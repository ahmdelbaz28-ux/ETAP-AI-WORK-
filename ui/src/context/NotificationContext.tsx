...
<<<<<<< HEAD
  ...
  // the identical error text, which confuses users. Backend-sourced
  // notifications are deduplicated by their backend id (tracked in
  // toastedBackendIds) and also by (type+message) as a second guard.
  const pushToast = useCallback(
    (type: ToastNotification["type"], message: string, backendId?: string) => {
      ...
      setNotifications((prev) => {
        ...
        const alreadyShown = prev.some((n) => n.type === type && n.message === message);
        if (alreadyShown) return prev;
        return [...prev, { id, type, message }];
      });
      setTimeout(() => {
        if (!isMountedRef.current) return;
        setNotifications((prev) => prev.filter((n) => n.id !== id)); // NOSONAR — S2004: auto-dismiss
      }, 5000);
    },
    [],
  );
  ...
  // Issue #7/#12: Strip any redundant prefix (e.g. "Error: " or "خطأ: ")
  // that the caller might have added — the toast icon already conveys the type.
