...
const supportedFormats: FormatInfo[] = [
  {
    ...
    desc: "IEC Common Information Model",
    standard: "IEC 61970",
    ext: ".xml, .cim",
  },
  {
    ...
    async (file: File) => {
      ...
      try {
        ...
        if (data.success) {
          notify(
            "success",
            `Imported ${file.name}: ${data.buses.length} buses, ${data.branches.length} branches`,
          );
        } else {
                ...
                <div className="bg-amber-500/5 border border-amber-500/20 rounded-lg p-3">
                  ...
                  <ul className="text-xs text-[var(--text-secondary)] space-y-0.5 max-h-32 overflow-y-auto">
                    {result.warnings.slice(0, 10).map((w) => {
                      ...
                      return (
                        <li key={warningKey} className="font-mono">
                          {w}
                        </li>
                      );
                    })}
                ...
                <div className="bg-red-500/5 border border-red-500/20 rounded-lg p-3">
                  ...
                  <ul className="text-xs text-[var(--text-secondary)] space-y-0.5">
                    {result.errors.map((e) => {
                      ...
                      return (
                        <li key={errorKey} className="font-mono">
                          {e}
                        </li>
                      );
                    })}
                        ...
                        {result.buses.slice(0, 100).map((b) => {
                          ...
                          return (
                            <tr key={busKey} className="border-t border-[var(--border-primary)]">
                              ...
                              <td className="p-2 mono-engineering text-[var(--text-primary)]">
                                {b.voltage_kv ?? "—"}
                              </td>
