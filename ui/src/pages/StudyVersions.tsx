...
function versionLabel(v: { readonly version_number: number; readonly label?: string | null }): string {
        const suffix = v.label ? ` — ${v.label}` : "";
        return `v${v.version_number}${suffix}`;
}
...
export default function StudyVersionsPage() {
        ...
        const [rollbackResult, setRollbackResult] = useState<RollbackResponse | null>(
                null,
        );
        ...
        const [createResult, setCreateResult] = useState<VersionResponse | null>(
                null,
        );
        ...
        const handleRollback = useCallback(
                async (version: VersionResponse) => {
                        ...
                        try {
                                ...
                                notify(
                                        "success",
                                        res.message || `Rolled back to version ${version.version_number}.`,
                                );
                        } catch (err) {
                                const msg = err instanceof Error ? err.message : String(err);
        ...
        const handleCreate = useCallback(async () => {
                if (!versionsBasePath) {
                        setCreateError(
                                "Load a study first (Project ID + Study ID) on the Versions tab.",
                        );
                        return;
                }
                ...
                try {
                        ...
                        notify(
                                "success",
                                `Snapshot created: ${versionLabel(res)}.`,
                        );
                        ...
                        <div className="flex items-start justify-between gap-4">
                                <div className="min-w-0">
                                        ...
                                        <p className="text-sm text-[var(--text-muted)] mt-1">
                                                Version control for studies — browse snapshots, create new ones,
                                                roll back to a previous state, and compare two versions
                                                side-by-side.
                                        </p>
                                </div>
                                <Button
                                ...
                                <CardSection className="p-4">
                                        ...
                                        {lockedProjectId && lockedStudyId && (
                                                <p
                                                        ...
                                                        data-testid="sv-target-summary"
                                                >
                                                        Target: project{" "}
                                                        <code className="font-mono text-zinc-300">{lockedProjectId}</code>{" "}
                                                        / study{" "}
                                                        <code className="font-mono text-zinc-300">{lockedStudyId}</code>
                                                </p>
                                        )}
                                </CardSection>
                        </Card>
                        ...
                        {tab === "versions" && (
                                <Card>
                                        ...
                                        <CardSection className="p-4">
                                                ...
                                                {versionsBasePath && versionsLoading && (
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                                ...
                                                <CardSection className="p-4 space-y-4">
                                                        {!versionsBasePath && (
                                                                <p className="text-xs text-amber-300">
                                                                        <span>Tip: load a study on the Versions tab first. The snapshot will be created for </span>
                                        ...
                                        <Card>
                                                <CardHeader
                                                        ...
                                                        subtitle="Backend response from POST /versions"
                                                />
                                                <CardSection className="p-4">
                                                        ...
                                                        {createResult && (
                                                                <div data-testid="sv-create-result" className="space-y-2">
                                                                        ...
                                                                        <StatRow
                                                                                label="Description"
                                                                                value={createResult.description ?? "—"}
                                                                        />
                                                                        <StatRow
                                                                                label="Created by"
                                                                                value={createResult.created_by || "—"}
                                                                        />
                                                                        <StatRow
                                                                                label="Created at"
                                                                                value={createResult.created_at ?? "—"}
                                                                        />
                                                                </div>
                                                        )}
                                                </CardSection>
                                ...
                                <div className="grid grid-cols-1 gap-6">
                                        <Card>
                                                ...
                                                <CardSection className="p-4 space-y-4">
                                                        {!versionsBasePath && (
                                                                <p className="text-xs text-amber-300">
                                                                        Load a study on the Versions tab first — the version dropdowns
                                                                        below populate from that study's history.
                                                                </p>
                                                        )}
                                                        {versionsBasePath && versions.length < 2 && (
                                                                <p className="text-xs text-amber-300">
                                                                        Only {versions.length} version(s) loaded — you need at least
                                                                        two to compare. Load more versions or create a new snapshot
                                                                        first.
                                                                </p>
                                                        )}
                                                        <div className="grid grid-cols-1 sm:grid-cols-[1fr_1fr_auto] gap-3 items-end">
                                                        ...
                                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                                <Card>
                                                                        ...
                                                                        <CardSection className="p-4">
                                                                                ...
                                                                                <StatRow
                                                                                        label="Created by"
                                                                                        value={compareResult.version_a.created_by || "—"}
                                                                                />
                                                                                <StatRow
                                                                                        label="Created at"
                                                                                        value={compareResult.version_a.created_at ?? "—"}
                                                                                />
                                                                        </CardSection>
                                                                </Card>
                                                                <Card>
                                                                        ...
                                                                        <CardSection className="p-4">
                                                                                ...
                                                                                <StatRow
                                                                                        label="Created by"
                                                                                        value={compareResult.version_b.created_by || "—"}
                                                                                />
                                                                                <StatRow
                                                                                        label="Created at"
                                                                                        value={compareResult.version_b.created_at ?? "—"}
                                                                                />
                                                                        </CardSection>
                                                                </Card>
                                                        </div>
                                                                        ...
                                                                        {Object.keys(compareResult.config_diff).length === 0 ? (
                                                                                ...
                                                                                >
                                                                                        <table className="w-full text-sm">
                                                                                                ...
                                                                                                <tbody>
                                                                                                        {Object.entries(compareResult.config_diff).map(
                                                                                                                ([key, diff]) => (
                                                                                                                        <tr
                                                                                                                                key={key}
                                                                                ...
                                                                                {Object.keys(compareResult.results_diff).length === 0 ? (
                                                                                        ...
                                                                                        >
                                                                                                <table className="w-full text-sm">
                                                                                                        ...
                                                                                                        <tbody>
                                                                                                                {Object.entries(compareResult.results_diff).map(
                                                                                                                        ([key, diff]) => (
                                                                                                                                <tr
                                                                                                                                        key={key}
                                                                                                                                        className="border-b border-[var(--border-primary)] last:border-0"
