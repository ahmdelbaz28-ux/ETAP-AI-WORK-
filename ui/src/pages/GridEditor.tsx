...
import {
  Play, CheckCircle, AlertTriangle, Trash2, Plus, MousePointer,
  Link2, Sliders, Zap, Sparkles, Download
} from 'lucide-react'
...
const CAIRO_TEMPLATE = {
  buses: [
    { id: 1, name: 'Cairo West Gen Bus', x: 150, y: 150, type: 'slack', baseKv: 13.8, voltageMagnitude: 1.05, voltageAngle: 0 },
    { id: 2, name: 'Giza Distribution Bus', x: 450, y: 150, type: 'pq', baseKv: 13.8, voltageMagnitude: 1.0, voltageAngle: 0 },
    { id: 3, name: 'Helwan Substation Bus', x: 300, y: 350, type: 'pq', baseKv: 13.8, voltageMagnitude: 1.0, voltageAngle: 0 }
  ] as Bus[],
  lines: [
...
export default function GridEditor() {
  ...
  const [activeMode, setActiveMode] = useState<'select' | 'bus' | 'line' | 'generator' | 'load'>('select')
  const [selectedElement, setSelectedElement] = useState<{ type: 'bus' | 'line' | 'generator' | 'load'; id: number } | null>(null)
  ...
  const loadCairoTemplate = () => {
    ...
    notify('success', isRtl ? 'تم تحميل نموذج شبكة القاهرة 13.8kV' : 'Cairo 13.8kV Grid Template Loaded')
  }
  ...
  const handleBusClick = (e: React.MouseEvent, bus: Bus) => {
    ...
    if (activeMode === 'line') {
      if (connectingBusId === null) {
        ...
        notify('info', isRtl ? 'اختر الناقل الثاني لإكمال الربط' : 'Select second bus to complete link')
        ...
        setConnectingBusId(null)
        setActiveMode('select')
        notify('success', isRtl ? 'تم توصيل خط النقل بنجاح' : 'Transmission line connected successfully')
      }
  ...
  const handleRunLoadFlow = async () => {
    ...
    try {
      ...
      if (body.success && results.converged) {
        notify('success', isRtl ? 'اكتمل حل سريان الحمل (نيوتن-رافسون)' : 'Load flow converged successfully (Newton-Raphson)')
        ...
        if (results.buses) {
          setBuses(prev => prev.map(b => {
            ...
            if (resBus) {
              return {
                ...b,
                voltageMagnitude: resBus.voltage_magnitude_pu || resBus.voltage_magnitude || b.voltageMagnitude,
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[var(--text-tertiary)] mt-1">
            {isRtl ? 'ارسم شبكتك الهندسية، تحقق من اتصالاتها، وشغل حسابات سريان الأحمال حياً.' : 'Design topological grids, validate structural links, and run live calculations.'}
          </p>
        </div>
          ...
          <Card padding="sm">
            ...
            <div className="flex flex-wrap items-center gap-2 border-b border-[var(--border-primary)] pb-3 mb-3">
              <Button
                ...
                size="sm"
                icon={MousePointer}
                onClick={() => { setActiveMode('select'); setConnectingBusId(null) }}
              >
                {isRtl ? 'تحديد' : 'Select'}
              </Button>
              <Button
                ...
                size="sm"
                icon={Plus}
                onClick={() => { setActiveMode('bus'); setConnectingBusId(null) }}
              >
              ...
              <Button
                ...
                size="sm"
                icon={Link2}
                onClick={() => { setActiveMode('line'); setConnectingBusId(null) }}
              >
              ...
              <Button
                ...
                size="sm"
                icon={Sparkles}
                onClick={() => { setActiveMode('generator'); setConnectingBusId(null) }}
              >
              ...
              <Button
                ...
                size="sm"
                icon={Zap}
                onClick={() => { setActiveMode('load'); setConnectingBusId(null) }}
              >
            ...
            <div className="relative border border-[var(--border-primary)] rounded-lg bg-[var(--bg-primary)] overflow-hidden">
              ...
              >
                ...
                <defs>
                  <pattern id="gridPattern" width="20" height="20" patternUnits="userSpaceOnUse">
                    <path d="M 20 0 L 0 0 0 20" fill="none" stroke="var(--border-primary)" strokeWidth="0.5" opacity="0.3" />
                  </pattern>
                </defs>
                <rect width="100%" height="100%" fill="url(#gridPattern)" />

                {/* Draw connecting temporary line */}
                {activeMode === 'line' && connectingBusId !== null && (() => {
                ...
                {lines.map(line => {
                  ...
                  return (
                    <g key={`line-${line.id}`} onClick={(e) => { e.stopPropagation(); setSelectedElement({ type: 'line', id: line.id }) }}>
                      <line
                        x1={fromBus.x}
                        y1={fromBus.y}
                        x2={toBus.x}
                        ...
                        className={cn(
                          ...
                          isSelected ? "stroke-[var(--accent-primary)]" : "stroke-[var(--text-tertiary)]"
                        )}
                      />
                  ...
                  return (
                    ...
                    >
                      ...
                      {connectingBusId === bus.id && (
                        <circle r="12" fill="none" stroke="var(--accent-primary)" strokeWidth="2" className="animate-ping" />
                      )}
                    </g>
                  )
                })}
                ...
                {generators.map(gen => {
                  ...
                  if (!bus) return null
                  const isSelected = selectedElement?.type === 'generator' && selectedElement.id === gen.id
                  return (
                    <g
                      key={`gen-${gen.id}`}
                      transform={`translate(${bus.x}, ${bus.y - 45})`}
                      onClick={(e) => { e.stopPropagation(); setSelectedElement({ type: 'generator', id: gen.id }) }}
                    ...
                    >
                      ...
                      />
                      <path d="M-8,0 Q-4,-6 0,0 T8,0" fill="none" className="stroke-2 stroke-green-400" />
                      <line x1="0" y1="14" x2="0" y2="41" stroke="var(--text-secondary)" strokeWidth="2" />
                      <text x="20" y="4" className="text-[10px] fill-[var(--text-secondary)] font-medium">
                        {gen.name}
                      </text>
                    </g>
                  )
                ...
                {loads.map(ld => {
                  ...
                  if (!bus) return null
                  const isSelected = selectedElement?.type === 'load' && selectedElement.id === ld.id
                  return (
                    <g
                      key={`load-${ld.id}`}
                      transform={`translate(${bus.x}, ${bus.y + 15})`}
                      onClick={(e) => { e.stopPropagation(); setSelectedElement({ type: 'load', id: ld.id }) }}
                      className="cursor-pointer"
                    >
                      {/* Load symbol: Arrow pointing down */}
                      <line x1="0" y1="0" x2="0" y2="25" stroke="var(--text-secondary)" strokeWidth="2" />
                      <polygon
                        points="0,25 -6,17 6,17"
                        className={cn(
                          "stroke-1",
                          isSelected ? "fill-[var(--accent-primary)] stroke-[var(--accent-primary)]" : "fill-amber-400 stroke-amber-400"
                        )}
                      />
                      <text x="12" y="16" className="text-[10px] fill-[var(--text-secondary)] font-medium">
                        {ld.name}
                      </text>
                    </g>
                  )
              ...
              {activeMode !== 'select' && (
                <div className="absolute bottom-4 left-4 bg-black/85 border border-[var(--border-primary)] px-3 py-1.5 rounded-md text-xs text-brand-400 font-medium">
                  {isRtl ? `وضع الرسم النشط: ${activeMode.toUpperCase()}` : `Active Tool: ${activeMode.toUpperCase()}`}
                </div>
              )}
            </div>
          </Card>
          ...
          <Card padding="md">
            ...
            {selectedDetails ? (
              <div className="space-y-4 text-xs">
                <div>
                  <label className="block text-[var(--text-tertiary)] mb-1">{isRtl ? 'الاسم' : 'Name'}</label>
                  <input
                    type="text"
                ...
                {selectedElement?.type === 'bus' && (() => {
                  ...
                  return (
                    <>
                      <div>
                        <label className="block text-[var(--text-tertiary)] mb-1">{isRtl ? 'نوع الناقل' : 'Bus Type'}</label>
                        <select
                          value={b.type}
                      ...
                      <div>
                        <label className="block text-[var(--text-tertiary)] mb-1">{isRtl ? 'الجهد الاسمي (kV)' : 'Nominal KV'}</label>
                        <input
                          type="number"
                          value={b.baseKv}
                          onChange={(e) => updateSelectedProperty('baseKv', parseFloat(e.target.value))}
                          className="w-full bg-[var(--bg-primary)] border border-[var(--border-primary)] rounded px-2.5 py-1.5 text-[var(--text-primary)] focus:outline-none"
                        />
                      </div>
                    </>
                ...
                {selectedElement?.type === 'line' && (() => {
                  ...
                  return (
                    <>
                      <div>
                        <label className="block text-[var(--text-tertiary)] mb-1">{isRtl ? 'المقاومة R1 (pu)' : 'Resistance R1 (pu)'}</label>
                        <input
                          type="number"
                          step="0.001"
                          value={l.r1}
                          onChange={(e) => updateSelectedProperty('r1', parseFloat(e.target.value))}
                          className="w-full bg-[var(--bg-primary)] border border-[var(--border-primary)] rounded px-2.5 py-1.5 text-[var(--text-primary)] focus:outline-none"
                        />
                      </div>
                      <div>
                        <label className="block text-[var(--text-tertiary)] mb-1">{isRtl ? 'المفاعلة X1 (pu)' : 'Reactance X1 (pu)'}</label>
                        <input
                          type="number"
                          step="0.001"
                          value={l.x1}
                          onChange={(e) => updateSelectedProperty('x1', parseFloat(e.target.value))}
                          className="w-full bg-[var(--bg-primary)] border border-[var(--border-primary)] rounded px-2.5 py-1.5 text-[var(--text-primary)] focus:outline-none"
                        />
                      </div>
                    </>
                ...
                {selectedElement?.type === 'generator' && (() => {
                  ...
                  return (
                    <>
                      <div>
                        <label className="block text-[var(--text-tertiary)] mb-1">{isRtl ? 'القدرة الحقيقية pg (MW)' : 'Real Power pg (MW)'}</label>
                        <input
                          type="number"
                          value={g.pg}
                          onChange={(e) => updateSelectedProperty('pg', parseFloat(e.target.value))}
                          className="w-full bg-[var(--bg-primary)] border border-[var(--border-primary)] rounded px-2.5 py-1.5 text-[var(--text-primary)] focus:outline-none"
                        />
                      </div>
                      <div>
                        <label className="block text-[var(--text-tertiary)] mb-1">{isRtl ? 'قيمة تنظيم الجهد' : 'Voltage Setpoint (pu)'}</label>
                        <input
                          type="number"
                          step="0.01"
                          value={g.vSetpoint}
                          onChange={(e) => updateSelectedProperty('vSetpoint', parseFloat(e.target.value))}
                          className="w-full bg-[var(--bg-primary)] border border-[var(--border-primary)] rounded px-2.5 py-1.5 text-[var(--text-primary)] focus:outline-none"
                        />
                      </div>
                    </>
                ...
                {selectedElement?.type === 'load' && (() => {
                  ...
                  return (
                    <>
                      <div>
                        <label className="block text-[var(--text-tertiary)] mb-1">{isRtl ? 'القدرة الفعالة P (MW)' : 'Real Load P (MW)'}</label>
                        <input
                          type="number"
                          value={ld.pMw}
                          onChange={(e) => updateSelectedProperty('pMw', parseFloat(e.target.value))}
                          className="w-full bg-[var(--bg-primary)] border border-[var(--border-primary)] rounded px-2.5 py-1.5 text-[var(--text-primary)] focus:outline-none"
                        />
                      </div>
                      <div>
                        <label className="block text-[var(--text-tertiary)] mb-1">{isRtl ? 'القدرة غير الفعالة Q (MVAR)' : 'Reactive Load Q (MVAR)'}</label>
                        <input
                          type="number"
                          value={ld.qMvar}
                          onChange={(e) => updateSelectedProperty('qMvar', parseFloat(e.target.value))}
                          className="w-full bg-[var(--bg-primary)] border border-[var(--border-primary)] rounded px-2.5 py-1.5 text-[var(--text-primary)] focus:outline-none"
                        />
                      </div>
                    </>
              ...
              <p className="text-xs text-[var(--text-muted)] text-center py-4">
                {isRtl ? 'اضغط على أي عنصر في اللوحة لعرض وتعديل خصائصه الهندسية.' : 'Select an element on the canvas to edit its properties.'}
              </p>
            )}
          </Card>
            ...
            {validationResults ? (
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  <Badge variant={validationResults.valid ? 'success' : 'danger'} size="sm" className="w-full text-center">
                    {validationResults.valid ? (isRtl ? 'بنية شبكة سليمة' : 'Structure Valid') : (isRtl ? 'خطأ في الربط' : 'Invalid Connection')}
                  </Badge>
                </div>
