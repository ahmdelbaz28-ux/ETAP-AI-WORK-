...
function loadAssets(): typeof assets {
  try {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {['Active', 'Maintenance', 'Faulted', 'Offline'].map(cat => (
          <div key={cat} className="bg-surface-800 rounded-xl p-4 border border-surface-700 text-center">
            ...
            <p className="text-2xl font-bold text-white mt-1">
              {cat === 'Active' ? assets.filter(a => a.status === 'active').length : 0}
              {cat === 'Maintenance' ? assets.filter(a => a.status === 'maintenance').length : 0}
              {cat === 'Faulted' ? 0 : ''}
              {cat === 'Offline' ? 0 : ''}
            </p>
          </div>
