...
function parseCurlCommand(
  curl: string,
): { baseUrl: string; apiKey: string; modelId: string } | null {
  try {
    ...
    const authMatch =
      curl.match(/Bearer\s+([a-zA-Z0-9_-]+)/i) ||
      curl.match(/Authorization:\s*Bearer\s+([a-zA-Z0-9_-]+)/i); // NOSONAR — S6594: RegExp.exec vs match; performance neutral
    let apiKey = '';
    if (authMatch) {
      apiKey = authMatch[1];
    } else {
      const genericKey =
        curl.match(/api-key:\s*([a-zA-Z0-9_-]+)/i) || curl.match(/x-api-key:\s*([a-zA-Z0-9_-]+)/i); // NOSONAR — S6594: RegExp.exec vs match; performance neutral
      if (genericKey) apiKey = genericKey[1];
    }

    const modelMatch =
      curl.match(/"model"\s*:\s*"([^"]+)"/) || curl.match(/'model'\s*:\s*'([^']+)'/); // NOSONAR — S6594: RegExp.exec vs match; performance neutral
...
function AISettingsPanel({ settings, setSettings, notify }: AISettingsPanelProps) {
  ...
  const handleParseCurl = () => {
    ...
    if (result) {
      ...
      notify(
        'error',
        'Failed to parse curl command. Ensure it has a URL and Authorization header.',
      );
    }
  };
  ...
  const handleConnectCustom = () => {
    notify(
      'success',
      `Custom provider connected successfully using ${customTab.toUpperCase()} Config (${customScope} scope)!`,
    );
  };
  ...
  return (
    <div className="space-y-6 col-span-2">
      ...
      >
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="text-base font-semibold text-[var(--text-primary)]">
                Quick Setup — Connect your AI
              </h3>
              <p className="text-xs text-[var(--text-secondary)] mt-0.5">
                Paste your API key from any provider below and click{' '}
                <span className="font-semibold text-brand-400">Test &amp; Save</span>. Your key is
                stored locally in your browser (not sent anywhere unless you test it).
              </p>
            </div>
          </div>
        ...
        <div className="mt-4 flex items-start gap-2 p-3 rounded-lg bg-[var(--bg-primary)] border border-[var(--border-primary)]">
          ...
          <p className="text-[11px] text-[var(--text-secondary)] leading-relaxed">
            <span className="font-semibold">How it works:</span> Your API key is stored locally in
            your browser and used to call the AI provider directly. Once you've connected at least
            one provider, the <span className="font-semibold text-brand-400">AI Assistant</span>{' '}
            page will use it automatically. No key is ever sent to our servers unless you explicitly
            test it.
          </p>
        </div>
      </Card>
      ...
      <Card
        padding="md"
        className="border-2 border-purple-500/30 shadow-lg shadow-purple-500/5 bg-gradient-to-br from-purple-500/[0.03] to-transparent"
      >
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5 pb-4 border-b border-[var(--border-primary)]">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-[var(--text-secondary)] mt-0.5">
                Connect any provider that uses the OpenAI API format (e.g., Ollama, vLLM, LocalAI,
                Together AI, OpenRouter). Enter your endpoint URL, API key, and model ID below.
              </p>
            </div>
          </div>
          {settings.CUSTOM_OPENAI_API_KEY && (
            ...
            >
              ...
              {testResults.custom_openai.details && (
                <details className="mt-1.5">
                  ...
                  <pre className="text-[9px] opacity-60 mt-1 whitespace-pre-wrap break-all">
                    {testResults.custom_openai.details}
                  </pre>
                </details>
              )}
        ...
        <details className="mt-3">
          ...
          <div className="mt-2 p-3 rounded-lg bg-[var(--bg-primary)] border border-[var(--border-primary)] text-[10px] space-y-1.5 font-mono">
            <div>
              <span className="text-purple-400">Ollama (local):</span> http://localhost:11434/v1 ·
              model: llama3.2
            </div>
            <div>
              <span className="text-purple-400">vLLM (local):</span> http://localhost:8000/v1 ·
              model: meta-llama/Llama-3.1-8B-Instruct
            </div>
            <div>
              <span className="text-purple-400">Together AI:</span> https://api.together.xyz/v1 ·
              model: meta-llama/Llama-3.3-70B-Instruct-Turbo
            </div>
            <div>
              <span className="text-purple-400">OpenRouter:</span> https://openrouter.ai/api/v1 ·
              model: openai/gpt-4o-mini
            </div>
            <div>
              <span className="text-purple-400">Groq:</span> https://api.groq.com/openai/v1 · model:
              llama-3.3-70b-versatile
            </div>
            <div>
              <span className="text-purple-400">LM Studio (local):</span> http://localhost:1234/v1 ·
      ...
      <details className="group">
        <summary className="flex items-center gap-2 cursor-pointer p-3 rounded-lg bg-[var(--bg-elevated)] border border-[var(--border-primary)] hover:border-brand-500/30 transition-colors list-none">
          ...
          <span className="text-sm font-medium text-[var(--text-secondary)]">
            Advanced Options (Custom Models, curl import, JSON config)
          </span>
        </summary>
            ...
            <div className="flex items-center gap-2 mb-4 border-b border-[var(--border-primary)] pb-3">
              ...
              <div>
                <h3 className="text-sm font-semibold text-[var(--text-primary)]">
                  Popular Models (Fast Connect)
                </h3>
                <p className="text-[10px] text-[var(--text-muted)]">
                  Quickly connect to leading AI providers with your API key.
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {POPULAR_PROVIDERS.map((p) => {
                ...
                return (
                  <div
                    key={p.id}
                    className="p-3 bg-[var(--bg-elevated)] border border-[var(--border-primary)] hover:border-brand-500/30 rounded-xl transition-all flex flex-col justify-between space-y-3"
                  >
                    ...
                    <div className="space-y-2">
                      <div>
                        <label
                          htmlFor={`prov-${p.id}-key`}
                          className="block text-[9px] text-[var(--text-tertiary)] mb-1"
                        >
                          API Key
                        </label>
                        <input
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-[var(--text-muted)] mt-0.5">
                    Configure custom endpoints, local models (Ollama, LM Studio), or fine-tune
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setSettings((prev) => ({ ...prev, CUSTOM_BASE_URL: e.target.value }))
                        }
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setSettings((prev) => ({ ...prev, CUSTOM_MODEL_ID: e.target.value }))
                        }
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setSettings((prev) => ({ ...prev, CUSTOM_API_KEY: e.target.value }))
                        }
                        className="w-full px-3 py-2 bg-[var(--bg-input)] border border-[var(--border-primary)] rounded-lg text-[var(--text-primary)] text-sm focus:border-brand-500 outline-none transition-colors"
                      />
                    </div>
              ...
              {customTab === 'openai' && (
                <div className="space-y-4">
                  <div className="bg-[var(--bg-primary)] p-3 rounded-lg border border-[var(--border-primary)] text-xs text-[var(--text-muted)] leading-relaxed">
                    ℹ️ Use this configuration to connect standard local developer platforms like{' '}
                    <strong>Ollama</strong> or <strong>LM Studio</strong>. For Ollama, use default
                    base URL <code>http://localhost:11434/v1</code>. For LM Studio, use{' '}
