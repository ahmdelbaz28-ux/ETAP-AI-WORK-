...
// S3358: extract nested-ternary badge variants into helper functions.
const scoreVariant = (score: number): "success" | "warning" | "default" => {
...
export default function ContextEngine() {
  ...
  const handleRetrieve = async () => {
    ...
    try {
      ...
      notify("error", err instanceof Error ? err.message : "Failed to retrieve context");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleImpact = async () => {
    ...
    try {
      ...
      notify("error", err instanceof Error ? err.message : "Failed to analyze impact");
    } finally {
      setLoading(false);
    }
  };
