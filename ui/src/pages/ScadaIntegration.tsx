...
import {
  Activity, RefreshCw, Database, Server, Play, Pause, Save,
  Radio, ShieldAlert
} from 'lucide-react'
...
export default function ScadaIntegration() {
  ...
  const [telemetryPoints, setTelemetryPoints] = useState<TelemetryPoint[]>([
    ...
    { tag: 'BUS1.F', value: 50.00, unit: 'Hz', quality: 'GOOD' },
    { tag: 'FEEDER1.I', value: 412.5, unit: 'A', quality: 'GOOD' },
    { tag: 'XF1.P', value: 2.80, unit: 'MW', quality: 'GOOD' },
    { tag: 'XF1.Q', value: 0.90, unit: 'MVAR', quality: 'GOOD' }
  ])
  ...
  useEffect(() => {
    try {
      ...
      if (stored) {
        ...
        if (parsed.SCADA_SYNC_INTERVAL_SEC) setSyncInterval(parseInt(parsed.SCADA_SYNC_INTERVAL_SEC) || 2)
      }
  ...
  const handleSaveSettings = () => {
    try {
      ...
      addLog(isRtl ? 'تم حفظ إعدادات خادم زينون في النظام.' : 'Zenon SCADA server configurations updated.')
    } catch (err: any) {
  ...
  const testConnection = async () => {
    ...
    addLog(isRtl ? 'جاري فحص الاتصال مع خادم Zenon SCADA...' : 'Testing connection to Zenon SCADA server...')
    ...
    try {
      ...
      if (response.ok) {
        ...
        notify('success', isRtl ? 'تم الاتصال بنجاح مع نظام الإسكادا!' : 'SCADA connection verified successfully!')
        addLog(isRtl ? `تم الاتصال. زمن الاستجابة: ${Math.round(endTime - startTime)} ملي ثانية.` : `Connected. Latency: ${Math.round(endTime - startTime)} ms.`)
      } else {
      ...
      setLatency(null)
      notify('error', isRtl ? 'فشل الاتصال بنظام الإسكادا. تأكد من تشغيل خادم زينون.' : 'Connection failed. Ensure Zenon service is running.')
  ...
  const startRealSync = () => {
    ...
    try {
      ...
      addLog(isRtl ? `جاري فتح اتصال WebSocket على: ${wsUrl}` : `Opening WebSocket connection to: ${wsUrl}`)
          ...
          if (parsed.measurements) {
            ...
            if (parsed.measurements.bus_voltages) {
              parsed.measurements.bus_voltages.forEach((b: any) => {
                mappedPoints.push({ tag: `${b.bus_id}.V`, value: b.voltage_kV, unit: 'kV', quality: 'GOOD' })
              })
            }
            if (parsed.measurements.generator_outputs) {
              parsed.measurements.generator_outputs.forEach((g: any) => {
                mappedPoints.push({ tag: `${g.gen_id}.P`, value: g.mw, unit: 'MW', quality: 'GOOD' })
              })
            }
      ...
      socketRef.current.onerror = () => {
        addLog(isRtl ? 'فشل اتصال WebSocket. جاري التحويل لوضع الاقتراع الدؤوب (Polling)...' : 'WebSocket connection failed. Falling back to HTTP Polling...')
        startPolling()
      }
      ...
      addLog(isRtl ? 'تعذر تشغيل WebSocket. جاري تفعيل الاقتراع الدؤوب...' : 'WebSocket client init error. Starting HTTP Polling...')
      startPolling()
    }
  }
  ...
  const startSimulation = () => {
    ...
    addLog(isRtl ? 'تفعيل محاكي إسكادا زينون المحلي.' : 'Activated local Zenon SCADA simulation feed.')
    ...
    pollIntervalRef.current = setInterval(() => {
      ...
      setTelemetryPoints(prev =>
        prev.map(p => {
          ...
          return {
            ...p,
            value: parseFloat((p.value + fluctuation).toFixed(2))
          }
        })
      )
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-[var(--text-tertiary)]">
              {isRtl ? 'مراقبة اتصال خادم إسكادا زينون ومزامنة القراءات والإنذارات الحية مع أحمد إيتاب.' : 'Monitor Zenon SCADA server connectivity and sync real-time telemetry variables.'}
            </p>
          </div>
        </div>
          ...
          <Card padding="md">
            ...
            <div className="space-y-3.5 text-xs">
              ...
              <div className="pt-2 flex flex-col gap-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <Button variant="secondary" icon={RefreshCw} loading={connectionStatus === 'connecting'} onClick={testConnection}>
                    {isRtl ? 'فحص الاتصال' : 'Ping Server'}
                  </Button>
                  <Button variant={isLive ? 'danger' : 'success'} icon={isLive ? Pause : Play} onClick={toggleLiveSync}>
                    {isLive ? (isRtl ? 'إيقاف البث' : 'Pause Sync') : (isRtl ? 'تشغيل البث' : 'Start Live')}
                  </Button>
                </div>
              </div>
            </div>
          ...
          <Card padding="md">
            ...
            {latency !== null && (
              <div className="mt-4 pt-3 border-t border-[var(--border-primary)] flex justify-between text-xs">
                <span className="text-[var(--text-muted)]">{isRtl ? 'زمن الاستجابة (Latency):' : 'Latency Response:'}</span>
          ...
          <Card padding="md">
            <div className="flex items-center justify-between mb-4 pb-2 border-b border-[var(--border-primary)]">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-[var(--text-primary)]">
                  {isRtl ? 'قراءات العدادات الحية (SCADA Telemetry Tags)' : 'SCADA Live Telemetry Tags'}
                </h3>
              </div>
              {isLive && (
                ...
                <tbody className="divide-y divide-[var(--border-primary)]">
                  {telemetryPoints.map((point) => (
                    <tr key={point.tag} className="hover:bg-[var(--bg-elevated)] transition-colors">
                      <td className="py-2.5 px-3 font-mono font-semibold text-[var(--text-primary)]">{point.tag}</td>
                      <td className="py-2.5 px-3 font-mono text-blue-400 font-bold text-sm">{point.value}</td>
            ...
            <Card padding="md" className="h-[260px] flex flex-col">
              <div className="flex items-center justify-between mb-3 pb-2 border-b border-[var(--border-primary)] shrink-0">
                <div className="flex items-center gap-1.5">
                  ...
                  <h3 className="text-xs font-bold text-[var(--text-primary)]">{isRtl ? 'إنذارات الشبكة الحالية' : 'Active Alarm Stream'}</h3>
                </div>
                {alarms.length > 0 && (
                  <Badge variant="danger" size="sm">{alarms.length}</Badge>
                )}
              </div>
              <div className="flex-1 overflow-y-auto space-y-2 pr-1 text-xs">
                {alarms.length === 0 ? (
                  <p className="text-[var(--text-muted)] text-center py-12">{isRtl ? 'لا توجد إنذارات حالية.' : 'No active alarms.'}</p>
                ) : (
                  alarms.map((a, idx) => (
                    <div key={idx} className={`p-2 rounded border text-[11px] ${
                      a.severity === 'CRITICAL' 
                        ? 'bg-red-500/10 border-red-500/20 text-red-400' 
                        : 'bg-amber-500/10 border-amber-500/20 text-amber-400'
                    }`}>
                      <div className="flex justify-between font-bold mb-0.5">
                        <span>{a.alarm_id} - {a.severity}</span>
            ...
            <Card padding="md" className="h-[260px] flex flex-col">
              <div className="flex items-center justify-between mb-3 pb-2 border-b border-[var(--border-primary)] shrink-0">
                <div className="flex items-center gap-1.5">
                  ...
                  <h3 className="text-xs font-bold text-[var(--text-primary)]">{isRtl ? 'أثر اتصالات إسكادا' : 'Connection Trace Logs'}</h3>
                </div>
                <button className="text-[10px] text-blue-400 hover:underline" onClick={() => setLogs([])}>
                  {isRtl ? 'تفريغ' : 'Clear'}
                </button>
              </div>
              <div className="flex-1 overflow-y-auto font-mono text-[10px] text-[var(--text-secondary)] space-y-1.5">
                {logs.length === 0 ? (
                  <p className="text-[var(--text-muted)] text-center py-12">{isRtl ? 'السجلات فارغة.' : 'Trace is empty.'}</p>
                ) : (
                  logs.map((log, idx) => (
                    <div key={idx} className="border-b border-[var(--border-primary)]/40 pb-1">{log}</div>
                  ))
                )}
              </div>
            </Card>
