...
function auditToLogs(metrics: MetricsResponse | null, audit: AuditEntry[]): LogEntry[] {
  ...
  if (metrics) {
    ...
    logs.push({
      ...
      level: 'info',
      source: 'metrics',
      message: `API requests: ${Object.values(apiEntries).reduce((a: number, b: number) => a + b, 0)} total`,
    });
    const providerEntries = metrics.providers as Record<
      string,
    ...
    for (const [name, p] of Object.entries(providerEntries)) {
      logs.push({
        ...
        level: 'info',
        source: 'provider',
        message: `${name}: ${p.count} calls, avg ${p.avgMs}ms, ${(p.failureRate * 100).toFixed(1)}% failures`,
      });
    }
    const circuitEntries = metrics.circuits as Record<
      string,
    ...
    for (const [name, c] of Object.entries(circuitEntries)) {
      if (c.state !== 'closed')
        logs.push({
          ...
          level: 'warn',
          source: 'circuit',
          message: `Breaker ${name}: ${c.state} (${c.consecutiveFailures} failures)`,
        });
    }
  }
