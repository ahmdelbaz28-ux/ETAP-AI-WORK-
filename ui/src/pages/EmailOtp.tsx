...
import { Ban, CheckCircle2, KeyRound, RefreshCw, Send, ShieldCheck, XCircle } from "lucide-react";
...
type OtpPurpose = "signup" | "login" | "password_reset" | "mfa" | "sensitive_action";
...
export default function EmailOtpPage() {
  ...
  const [verifyResult, setVerifyResult] = useState<VerifyOtpResponse | null>(null);
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      const res = await adminFetch<SendOtpResponse>("/api/v1/auth/email-otp/send", {
        method: "POST",
        body: JSON.stringify(body),
      });
      setSendResult(res);
      if (res.success) {
        // SECURITY: test_code is dev-only. Ref: fix/admin-pages-hardening (#3)
        const codeNote =
          import.meta.env.DEV && res.test_code ? ` (test code: ${res.test_code})` : "";
        notify(
          "success",
          t("adminPages.emailOtp.send.success", { email: sendEmail.trim(), note: codeNote }),
        );
      } else {
        notify(
          "error",
          t("adminPages.emailOtp.send.failed", {
      ...
      setSendError(msg);
      notify("error", t("adminPages.emailOtp.send.failed", { error: msg }));
    } finally {
      setSendLoading(false);
    }
  }, [sendEmail, sendPurpose, sendName, notify, t]);
  ...
  const handleVerify = useCallback(async () => {
    ...
    try {
      ...
      const res = await adminFetch<VerifyOtpResponse>("/api/v1/auth/email-otp/verify", {
        method: "POST",
        body: JSON.stringify(body),
      });
      setVerifyResult(res);
      if (res.success) {
        notify("success", t("adminPages.emailOtp.verify.success", { email: verifyEmail.trim() }));
      } else {
        notify(
          "error",
          t("adminPages.emailOtp.verify.failed", {
            error: res.error ?? t("adminPages.common.unknownError"),
          }),
        );
      }
      ...
      const msg = err instanceof Error ? err.message : String(err);
      setVerifyError(msg);
      notify("error", t("adminPages.emailOtp.verify.failed", { error: msg }));
    } finally {
      setVerifyLoading(false);
    }
  }, [verifyEmail, verifyPurpose, verifyCode, notify, t]);
  ...
  const handleInvalidate = useCallback(async () => {
    ...
    try {
      ...
      const res = await adminFetch<InvalidateResponse>(
      ...
      if (res.success) {
        notify(
          "success",
          t("adminPages.emailOtp.invalidate.success", {
            email: invEmail.trim(),
            purpose: invPurpose,
          }),
        );
        ...
        notify(
          "error",
          t("adminPages.emailOtp.invalidate.failed", {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      setInvError(msg);
      notify("error", t("adminPages.emailOtp.invalidate.failed", { error: msg }));
    } finally {
      setInvLoading(false);
    }
  }, [invEmail, invPurpose, notify, t]);
    ...
    >
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <p className="text-sm text-[var(--text-muted)] mt-1">
            Admin &amp; debug tools for the email one-time-passcode flow. Send a code to any
            email/purpose, verify it without round-tripping through the inbox, or invalidate a
            pending OTP for logout/admin flows.
          </p>
        </div>
        <Button
          variant="secondary"
      ...
      {tab === "send" && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {/* Result card */}
          <Card>
            <CardHeader title="Result" subtitle="Backend response from POST /send" />
            <CardSection className="p-4">
              ...
              {sendResult && (
                <div data-testid="send-result" className="space-y-2">
                  ...
                  {sendResult.message && (
                    <p className="text-xs text-zinc-400">{sendResult.message}</p>
                  )}
                  {sendResult.error && (
                  ...
                  {sendResult.expires_in_seconds !== undefined && (
                    <StatRow label="Expires in" value={`${sendResult.expires_in_seconds}s`} />
                  )}
                  {sendResult.cooldown_seconds !== undefined && (
                    <StatRow label="Cooldown" value={`${sendResult.cooldown_seconds}s`} />
                  )}
                  {sendResult.retry_after_seconds !== undefined && (
                    <StatRow label="Retry after" value={`${sendResult.retry_after_seconds}s`} />
                  )}
                  {sendResult.test_code && (
                    <StatRow label="Test code" value={sendResult.test_code} />
                  )}
                  {sendResult.trace_id && <StatRow label="Trace ID" value={sendResult.trace_id} />}
                </div>
              )}
            </CardSection>
          </Card>
      ...
      {tab === "verify" && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <CardSection className="p-4 space-y-4">
              ...
              <div>
                ...
                <select
                  id="verify-purpose"
                  value={verifyPurpose}
                  onChange={(e) => setVerifyPurpose(e.target.value as OtpPurpose)}
                  className={inputClass}
          ...
          <Card>
            <CardHeader title="Result" subtitle="Backend response from POST /verify" />
            <CardSection className="p-4">
              ...
              {verifyResult && (
                <div data-testid="verify-result" className="space-y-2">
                  ...
                  {verifyResult.message && (
                    <p className="text-xs text-zinc-400">{verifyResult.message}</p>
                  )}
                  {verifyResult.error && (
                    <div className="rounded-md border border-red-500/30 bg-red-500/10 p-2 text-xs text-red-300 font-mono break-all">
                      {verifyResult.error}
                  ...
                  {verifyResult.verified_email && (
                    <StatRow label="Verified email" value={verifyResult.verified_email} />
                  )}
                  {verifyResult.purpose && <StatRow label="Purpose" value={verifyResult.purpose} />}
                  {verifyResult.action_token && (
                    <StatRow label="Action token" value={verifyResult.action_token} />
                  )}
                  ...
                  {verifyResult.retry_after_seconds !== null &&
                    verifyResult.retry_after_seconds !== undefined && (
                      <StatRow label="Retry after" value={`${verifyResult.retry_after_seconds}s`} />
                    )}
      ...
      {tab === "invalidate" && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card>
            ...
            <CardSection className="p-4 space-y-4">
              <p className="text-sm text-zinc-400">
                Force-invalidate a pending OTP for the given email/purpose. Useful for logout flows
                or admin operations. The endpoint accepts{" "}
                <code className="text-xs font-mono text-zinc-300">email</code> and{" "}
                <code className="text-xs font-mono text-zinc-300">purpose</code> as query
                parameters.
              </p>
              <div>
          ...
          <Card>
            <CardHeader title="Result" subtitle="Backend response from POST /invalidate" />
            <CardSection className="p-4">
              ...
              {invResult && (
                <div data-testid="inv-result" className="space-y-2">
                  ...
                  {invResult.trace_id && <StatRow label="Trace ID" value={invResult.trace_id} />}
                </div>
              )}
            </CardSection>
          </Card>
