...
const ALERT_TYPE_LABELS: Record<string, { label: string; description: string }> = {
  ...
  load_flow_violations: {
    label: "Load Flow Violations",
    description: "Bus voltage & line loading violations",
  },
  protection_coordination: {
    label: "Protection Coordination",
    description: "Relay coordination issues",
  },
  equipment_alarms: {
    label: "Equipment Alarms",
    description: "Transformer & motor thermal alarms",
  },
    ...
    try {
      ...
      const fetched: DigestConfig = {
        alert_types: {
          ...
          short_circuit:
            data.alert_types?.short_circuit ?? DEFAULT_CONFIG.alert_types.short_circuit,
          scada_faults: data.alert_types?.scada_faults ?? DEFAULT_CONFIG.alert_types.scada_faults,
          load_flow_violations:
            data.alert_types?.load_flow_violations ??
            DEFAULT_CONFIG.alert_types.load_flow_violations,
          protection_coordination:
            data.alert_types?.protection_coordination ??
            DEFAULT_CONFIG.alert_types.protection_coordination,
          equipment_alarms:
            data.alert_types?.equipment_alarms ?? DEFAULT_CONFIG.alert_types.equipment_alarms,
          system_health:
            data.alert_types?.system_health ?? DEFAULT_CONFIG.alert_types.system_health,
        },
        email_digest: {
          enabled: data.email_digest?.enabled ?? DEFAULT_CONFIG.email_digest.enabled,
          cron_schedule:
            data.email_digest?.cron_schedule ?? DEFAULT_CONFIG.email_digest.cron_schedule,
          recipients: Array.isArray(data.email_digest?.recipients)
            ? data.email_digest.recipients
            : [],
        },
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <p className="text-sm text-[var(--text-tertiary)]">
                Alert preferences, email digests & webhooks
              </p>
        ...
        >
          <Card padding="md">
            ...
            <div className="space-y-3">
              {(
                Object.entries(ALERT_TYPE_LABELS) as [
                  keyof DigestConfig["alert_types"],
          ...
          <Card padding="md">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <Button variant="secondary" size="sm" icon={Webhook} onClick={addWebhook}>
                  Add
                </Button>
              </div>
