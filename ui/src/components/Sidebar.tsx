		...
		const interval = setInterval(() => {
			fetchHealth()
				.then((h: HealthResponse) =>
					setHealthStatus(h.ok ? "online" : "offline"),
				)
				.catch(() => setHealthStatus("offline"));
		}, 30000);
			...
			>
					...
					{sectionOrder.map((section) => {
						...
						return (
							<div key={section} className="sidebar-section-divider">
								...
								{sidebarCollapsed && (
									<hr className="border-[var(--border-primary)] mx-2" />
								)}
								{items.map((item) => {
									const isActive = location.pathname === item.to;
									return (
				...
				<div className="p-2 border-t border-[var(--border-primary)] space-y-1">
					...
					>
						{sidebarCollapsed ? (
							<ChevronRight
								className={`w-[18px] h-[18px] shrink-0 ${isRtl ? "rotate-180" : ""}`}
							/>
						) : (
							<>
								{isRtl ? (
