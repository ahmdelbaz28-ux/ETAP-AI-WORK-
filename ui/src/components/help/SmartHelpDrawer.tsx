...
const docTree: DocTreeNode[] = [
  {
    ...
    children: [
      {
        label: { en: "Overview Dashboard", ar: "نظرة عامة على لوحة التحكم" },
  ...
  },
  {
    ...
    children: [
      {
        label: { en: "Creating a New Project", ar: "إنشاء مشروع جديد" },
        topicId: "projects.create",
      },
      {
        label: { en: "Managing Projects", ar: "إدارة وتعديل المشاريع" },
        topicId: "projects.manage",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    children: [
      {
        label: { en: "Generating Reports", ar: "إنشاء وتصدير التقارير" },
  ...
  },
  {
    ...
    children: [
      {
        label: { en: "ETAP Worker Integration", ar: "تكامل بيئة عمل إيتاب" },
        topicId: "projects.create",
      },
      {
        label: { en: "SCADA System zenon Integration", ar: "ربط نظام إسكادا (زينون)" },
        topicId: "integration.scada",
      },
      {
        label: { en: "Digital Twin Overview", ar: "التوأم الرقمي للمشروع" },
        topicId: "digital-twin.overview",
      },
    ],
  },
  {
    ...
    children: [
      {
        label: { en: "FastAPI Backend Config", ar: "إعدادات خادم الخدمات الهندسية" },
        topicId: "settings.backend",
      },
    ],
  },
  {
    ...
    children: [
      {
        label: { en: "Backend Service Offline", ar: "مشاكل توقف الخادم" },
        topicId: "troubleshooting.backend",
      },
      {
        label: { en: "REST API Error Codes", ar: "أكواد أخطاء استجابة الـ API" },
        topicId: "troubleshooting.api",
      },
      {
        label: { en: "User Auth & JWT Issues", ar: "مشاكل مصادقة رموز الدخول" },
        topicId: "troubleshooting.auth",
      },
    ],
  },
];
    ...
    <div className="select-none text-left" style={{ marginLeft: `${level * 10}px` }}>
      <button
        ...
        className={cn(
          ...
          "bg-transparent border-0 text-left w-full appearance-none",
...
interface TopicDetailViewProps {
  readonly activeTopic: HelpTopic;
  readonly lang: "en" | "ar";
...
function TopicDetailView({
  ...
  const hasRelatedTopics = !!activeTopic.relatedTopics && activeTopic.relatedTopics.length > 0;
  return (
    <div className="flex-1 overflow-y-auto">
      ...
      <div className="px-5 pb-4">
        <div className="prose prose-sm max-w-none text-[var(--text-secondary)]">
          {activeTopic.content[lang].split("\n").map((line) => (
            <TopicContentLine key={`line-${line.length}-${line.substring(0, 16)}`} line={line} />
          ))}
        </div>
      </div>
      ...
      <div className="px-5 pb-4 space-y-2">
        {activeTopic.navigateTo && (
          <button
            onClick={() => {
              navigate(activeTopic.navigateTo as string);
              onClose();
              if (activeTopic.navigateTo) {
...
function TopicContentLine({ line }: { readonly line: string }) {
  ...
  if (line.startsWith("- ")) {
    return (
      <li key={lineKey} className="text-xs ml-4 mb-1">
        {line.substring(2)}
      </li>
    );
  }
  ...
  return (
    <p key={lineKey} className="text-xs leading-relaxed mb-2">
      {line}
    </p>
  );
}
...
interface BookIndexViewProps {
  readonly helpViewMode: "list" | "tree";
  readonly setHelpViewMode: (mode: "list" | "tree") => void;
  readonly lang: "en" | "ar";
  readonly expandedNodes: Set<string>;
...
export function SmartHelpDrawer({ open, onClose, initialContextId }: SmartHelpDrawerProps) {
  ...
  useEffect(() => {
    if (!open || !initialContextId) return;
    openContext(initialContextId);
    expandParentFolder(initialContextId, setExpandedNodes);
    ...
    <div className="fixed inset-0 z-[100] flex justify-end">
      ...
      >
        ...
        {activeTopic ? (
          ...
          <BookIndexView
            helpViewMode={helpViewMode}
            setHelpViewMode={setHelpViewMode}
            lang={lang}
            expandedNodes={expandedNodes}
