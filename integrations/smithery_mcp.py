...
class SmitheryClient:
    ...
    async def call_tool(
        ...
        try:
            ...
            logger.error("Smithery call_tool HTTP error: %s", e.response.status_code)  # NOSONAR: logger.error in except — see existing exception() calls
            return {"error": str(e), "result": None}
        except Exception as e:
            logger.error("Smithery call_tool error: %s", e)  # NOSONAR: logger.error in except — see existing exception() calls
            return {"error": str(e), "result": None}
