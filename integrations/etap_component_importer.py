    ...
    def _extract_cables(self, root: Any) -> list[dict[str, Any]]:
        ...
        for elem in root.iter():
            ...
            if tag in ("cable", "cabledata", "branch_cable"):
                ...
                kv = _safe_float(
                    elem.attrib.get("kV") or elem.attrib.get("RatedkV") or elem.findtext("kV"), 1.0
                )
                cores = _safe_int(
                    elem.attrib.get("Cores")
                    or elem.attrib.get("NoOfCores")
                    or elem.findtext("Cores"),
                    1,
                )
                size = _safe_float(
                    elem.attrib.get("Size") or elem.attrib.get("SizeMM2") or elem.findtext("Size"),
                    50.0,
                )
                r = _safe_float(
                    elem.attrib.get("R") or elem.attrib.get("R_per_km") or elem.findtext("R"), 0.387
                )
                x = _safe_float(
                    elem.attrib.get("X") or elem.attrib.get("X_per_km") or elem.findtext("X"), 0.093
                )
                ampacity = _safe_float(
                    elem.attrib.get("Ampacity")
                    or elem.attrib.get("CurrentRating")
                    or elem.findtext("Ampacity"),
                    150.0,
                )
        ...
        for elem in root.iter():
            ...
            if tag in ("transformer", "transformer2w", "xfmr"):
                name = (
                    ...
                    or elem.attrib.get("ID")
                    or elem.findtext("Name")
                    or f"XFMR-{uuid.uuid4().hex[:6]}"
                )
                mva = _safe_float(
                    elem.attrib.get("MVA") or elem.attrib.get("RatedMVA") or elem.findtext("MVA")
                )
                kva = _safe_float(
                    elem.attrib.get("kVA") or elem.attrib.get("RatedkVA") or elem.findtext("kVA")
                )
                rated_power = mva * 1000.0 if mva else (kva if kva else 1000.0)

                prim_kv = _safe_float(
                    elem.attrib.get("PriKV") or elem.attrib.get("PrimkV") or elem.findtext("PriKV"),
                    11.0,
                )
                sec_kv = _safe_float(
                    elem.attrib.get("SecKV") or elem.attrib.get("SeckV") or elem.findtext("SecKV"),
                    0.415,
                )
                z_pct = _safe_float(
                    elem.attrib.get("Z") or elem.attrib.get("ZPercent") or elem.findtext("Z"), 5.0
                )
                xr = _safe_float(
                    elem.attrib.get("XR") or elem.attrib.get("XRRatio") or elem.findtext("XR"), 6.0
                )
        ...
        for elem in root.iter():
            ...
            if tag in ("breaker", "circuitbreaker", "hvcbreaper", "lvcbreaper", "cb"):
                name = (
                    ...
                    or elem.attrib.get("ID")
                    or elem.findtext("Name")
                    or f"CB-{uuid.uuid4().hex[:6]}"
                )
                rated_kv = _safe_float(
                    elem.attrib.get("kV") or elem.attrib.get("RatedkV") or elem.findtext("kV"),
                    0.415,
                )
                rated_a = _safe_float(
                    elem.attrib.get("RatedA")
                    or elem.attrib.get("ContinuousA")
                    or elem.attrib.get("Amps")
                    or elem.findtext("Amps"),
                    630.0,
                )
                ka = _safe_float(
                    elem.attrib.get("BreakingkA")
                    or elem.attrib.get("kA")
                    or elem.attrib.get("InterruptingkA")
                    or elem.findtext("kA"),
                    36.0,
                )
                cb_type = (
                    ...
                    or ("VCB" if rated_kv >= 1.0 else "MCCB")
                )
        ...
        for elem in root.iter():
            ...
            if tag in ("relay", "protectivedevice", "ocr"):
                name = (
                    ...
                    or elem.attrib.get("ID")
                    or elem.findtext("Name")
                    or f"Relay-{uuid.uuid4().hex[:6]}"
                )
                function_ansi = (
                    elem.attrib.get("ANSI")
                    or elem.attrib.get("Function")
                    or elem.findtext("Function")
                    or "50/51"
                )
                curve = (
                    elem.attrib.get("Curve")
                    or elem.attrib.get("CurveType")
                    or elem.findtext("Curve")
                    or "IEC Normal Inverse"
                )
                ...
                relays.append(
                    {
                        ...
                        "specs": {
                            ...
                            "curve_type": curve,
                            "phase_pickup_pu": _safe_float(
                                elem.attrib.get("Pickup") or elem.findtext("Pickup"), 1.0
                            ),
                            "time_dial": _safe_float(
                                elem.attrib.get("TimeDial") or elem.findtext("TimeDial"), 0.1
                            ),
                        },
