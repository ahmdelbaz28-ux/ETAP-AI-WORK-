...
class ShortCircuitResult:
    ...
    fault_location: str = ""
    # Sequence currents
    I_positive: complex = complex(0, 0)  # NOSONAR: standard IEEE/IEC engineering notation (Ybus/Zbus/sequence components); renaming would harm domain readability
    I_negative: complex = complex(0, 0)  # NOSONAR: standard IEEE/IEC engineering notation (Ybus/Zbus/sequence components); renaming would harm domain readability
    I_zero: complex = complex(0, 0)  # NOSONAR: standard IEEE/IEC engineering notation (Ybus/Zbus/sequence components); renaming would harm domain readability
    # Phase currents
    Ia: complex = complex(0, 0)  # NOSONAR: standard IEEE/IEC engineering notation (Ybus/Zbus/sequence components); renaming would harm domain readability
    Ib: complex = complex(0, 0)  # NOSONAR: standard IEEE/IEC engineering notation (Ybus/Zbus/sequence components); renaming would harm domain readability
    Ic: complex = complex(0, 0)  # NOSONAR: standard IEEE/IEC engineering notation (Ybus/Zbus/sequence components); renaming would harm domain readability
...
class IEC60909Engine:
    ...
    def __init__(
        self,
        Ybus_pos: npt.NDArray[np.complexfloating],  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        Ybus_neg: npt.NDArray[np.complexfloating],  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        Ybus_zero: npt.NDArray[np.complexfloating],  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
    ...
    def _compute_zbus(self) -> None:
        ...
        try:
            self.Zbus_pos = np.linalg.inv(self.Ybus_pos)  # NOSONAR: standard IEEE/IEC engineering notation (Ybus/Zbus/sequence components); renaming would harm domain readability
        except np.linalg.LinAlgError:
            self.Zbus_pos = np.linalg.pinv(self.Ybus_pos)
        try:
            self.Zbus_neg = np.linalg.inv(self.Ybus_neg)  # NOSONAR: standard IEEE/IEC engineering notation (Ybus/Zbus/sequence components); renaming would harm domain readability
        except np.linalg.LinAlgError:
            self.Zbus_neg = np.linalg.pinv(self.Ybus_neg)
        try:
            self.Zbus_zero = np.linalg.inv(self.Ybus_zero)  # NOSONAR: standard IEEE/IEC engineering notation (Ybus/Zbus/sequence components); renaming would harm domain readability
        except np.linalg.LinAlgError:
    ...
    def _calculate_mu(self, Ik_initial_pu: float, t_min: float | None = None) -> float:  # NOSONAR: physics/engineering notation
        """
    ...
    def _calculate_thermal_factor(
        self, Ik_initial: float, ip: float, t_k: float = 1.0, m_factor: float = 1.0,  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
    ) -> float:
        """
        ...
        Ith = Ik_initial * np.sqrt(m + n)  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        return Ith
    ...
    def calculate_three_phase_fault(
        ...
        V_pre = c_factor * 1.0  # c * Un/Un = c in per-unit  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        ...
        Ik_pu = V_pre / Z1  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability

        # Convert to kA
        Ik_kA = abs(Ik_pu) * self.base_i / 1000.0  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        ...
        Ib = mu * Ik_kA  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        ...
        # Thermal current
        Ith = self._calculate_thermal_factor(Ik_kA, ip, t_k)  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        ...
        Ib_phase = Ik_pu * np.exp(1j * (-2 * np.pi / 3))  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        Ic_phase = Ik_pu * np.exp(1j * (2 * np.pi / 3))  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
    ...
    def calculate_line_to_ground_fault(
        ...
        V_pre = c_factor * 1.0  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        ...
        Ia = 3 * I1  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability

        # Convert to kA
        Ik_kA = abs(Ia) * self.base_i / 1000.0  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        ...
        # Phase currents
        Ib_phase = complex(0, 0)  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        Ic_phase = complex(0, 0)  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
    ...
    def calculate_line_to_line_fault(
        ...
        Ia = complex(0, 0)  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        a_op = np.exp(1j * 2 * np.pi / 3)
        a2_op = np.exp(-1j * 2 * np.pi / 3)
        Ib_phase = a2_op * I1 + a_op * I2 + I0  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        Ic_phase = a_op * I1 + a2_op * I2 + I0  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability

        # Magnitude of fault current
        Ik_pu = abs(Ib_phase)  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        Ik_kA = Ik_pu * self.base_i / 1000.0  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
    ...
    def calculate_double_line_to_ground_fault(
        ...
        # Sequence currents
        Z2_Z0_parallel = (Z2 * Z0) / (Z2 + Z0) if (Z2 + Z0) != 0 else complex(0, 0)  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        ...
        Ia = I1 + I2 + I0  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        Ib_phase = a2 * I1 + a * I2 + I0  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        Ic_phase = a * I1 + a2 * I2 + I0  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability

        # Use the larger of Ib and Ic for magnitude
        Ik_kA = max(abs(Ib_phase), abs(Ic_phase)) * self.base_i / 1000.0  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
