...
class FaultAnalyzer:
    def __init__(self, ybus_pos, ybus_neg=None, ybus_zero=None, base_mva=100.0, base_kv=115.0):
        ...
        self.Ybus_neg = ybus_neg if ybus_neg is not None else ybus_pos  # NOSONAR: standard IEEE/IEC engineering notation (Ybus/Zbus/sequence components); renaming would harm domain readability
        self.Ybus_zero = ybus_zero if ybus_zero is not None else ybus_pos  # NOSONAR: standard IEEE/IEC engineering notation (Ybus/Zbus/sequence components); renaming would harm domain readability
    ...
    def _invert_ybus(self, Ybus):  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
    ...
    def three_phase_fault(self, bus_index):
        ...
        """
        Vpre = complex(1.0, 0.0)  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        Zth = self._z(bus_index, "pos")  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        If = complex(float("inf"), 0) if abs(Zth) < 1e-12 else Vpre / Zth  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        return {
            "fault_current": If,
    ...
    def line_to_ground_fault(self, bus_index):
        ...
        If = complex(float("inf"), 0) if abs(denominator) < 1e-12 else 3 * Vpre / denominator  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        return {
            "fault_current": If,
            "fault_current_magnitude": np.abs(If),
            "fault_current_ka": self._pu_to_ka(If),
    ...
    def line_to_line_fault(self, bus_index):
        ...
        if abs(denominator) < 1e-12:
            If = complex(float("inf"), 0)  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        else:
    ...
    def double_line_to_ground_fault(self, bus_index):
        ...
        If1 = Vpre / (Z1 + Z20)  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        If0 = -If1 * (Z2 / (Z2 + Z0)) if abs(Z2 + Z0) > 1e-12 else complex(0, 0)  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        If2 = -If1 - If0  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        a = complex(-0.5, np.sqrt(3) / 2)
        a2 = complex(-0.5, -np.sqrt(3) / 2)
        Ia = If1 + If2 + If0  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        Ib = a2 * If1 + a * If2 + If0  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        Ic = a * If1 + a2 * If2 + If0  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        return {
            "fault_current": Ia,
            "fault_current_a": Ia,
            "fault_current_b": Ib,
