...
def check_agent_file_structure(filepath: str) -> List[str]:
    ...
    try:
        ...
        for node in ast.walk(tree):
            ...
            elif isinstance(node, ast.Import) and any(alias.name == "numpy" for alias in node.names):
                issues.append(f"Direct import of numpy: {ast.unparse(node)}")
        ...
        if 'import numpy as np' in content:
            ...
            if numpy_usage_found and numpy_import_line != -1:
                ...
                issues.append(f"File {filepath} uses numpy without proper error handling in some contexts")
                    ...
                    if 'prompt_handle =' in line:
                        ...
                        if len(parts) > 1:
                            ...
                            if handle_value not in available_handles:
                                print(f"⚠️  {agent_file} has prompt_handle '{handle_value}' which is not available")
                            else:
