...
# ---------------------------------------------------------------------------
INFO "1/4: helm lint"
for VALUES in "" "-f 01-helm-chart/etap-ai/values-production.yaml"; do
    # shellcheck disable=SC2086 # intentional word splitting for the -f flag
    if helm lint 01-helm-chart/etap-ai $VALUES > /tmp/lint.log 2>&1; then
...
if helm unittest 01-helm-chart/etap-ai > /tmp/unittest.log 2>&1; then
...
if helm template etap-ai 01-helm-chart/etap-ai -n etap \
    -f 01-helm-chart/etap-ai/values-production.yaml > /tmp/etap-rendered.yaml 2>/tmp/template.log; then
