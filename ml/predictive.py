...
class LoadForecaster:
    ...
    def _train_linear(self, data: np.ndarray) -> dict[str, Any]:
        ...
        X_flat = X.reshape(X.shape[0], X.shape[1])  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability

        XtX = X_flat.T @ X_flat  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        Xty = X_flat.T @ y  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        try:
    ...
    def _create_sequences(self, data: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        for i in range(len(data) - w):
            X.append(data[i : i + w])
            y.append(data[i + w])
        X_arr = np.array(X)  # NOSONAR: physics/engineering notation (I=current, V=voltage, P/Q=power, Ybus/Zbus matrices); snake_case would harm domain readability
        y_arr = np.array(y)
    ...
    def predict(self, horizon_hours: int = 24) -> np.ndarray:
        ...
        if self.model is None and self._fallback_weights is None:
            raise RuntimeError("Model has not been trained yet. Call train() first.")  # NOSONAR: intentional repetition (audit constant)
    ...
    def _predict_lstm(self, horizon_hours: int) -> np.ndarray:
        ...
        ) * np.random.rand(self._window_size)  # NOSONAR: numpy.random.Generator migration; API change required
...
class FaultPredictor:
    ...
    def train(self, features: np.ndarray, labels: np.ndarray) -> dict[str, Any]:  # NOSONAR: cognitive complexity; scheduled for refactoring sprint (extract helpers / early returns)
    ...
    def explain(self, features: np.ndarray) -> dict[str, Any]:  # NOSONAR: cognitive complexity; scheduled for refactoring sprint (extract helpers / early returns)
...
def get_ml_capabilities() -> dict[str, Any]:
    ...
    return {
        ...
        "forecasting_methods": {
            ...
            "best_available": "lstm"
            if _HAS_TENSORFLOW
            else ("prophet" if _HAS_PROPHET else "linear"),  # NOSONAR: nested conditional; extract to named variable (tech debt)
        },
        "fault_prediction_methods": {
            ...
            if _HAS_XGBOOST
            else ("random_forest" if _HAS_SKLEARN else "none"),  # NOSONAR: nested conditional; extract to named variable (tech debt)
        },
