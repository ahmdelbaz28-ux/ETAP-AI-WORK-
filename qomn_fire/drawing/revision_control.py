...
def draw_revision_cloud(doc, vertices: List[Tuple[float, float]]):
    ...
    msp.add_lwpolyline(
        bulge_vertices, format="xyb", close=True, dxfattribs={"layer": "A-FIRE-REVC", "color": 1}
    )
...
def draw_revision_table(doc, revisions: List[Revision]):
    ...
    layout.add_text(
        "REVISIONS LOG", dxfattribs={"insert": (610.0, 235.0), "height": 3.0, "color": 7}
    )
    ...
    for rev in revisions:
        ...
        layout.add_text(
            rev_str, dxfattribs={"insert": (610.0, y_offset), "height": 2.2, "color": 7}
        )
        y_offset -= 15.0
