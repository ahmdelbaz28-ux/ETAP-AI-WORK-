...
def _check_allow_list(allow_list: Any, user_id: str | None, roles: list[str] | None) -> bool:
    if not isinstance(allow_list, (list, tuple, set)):
        return False
    if user_id is not None and str(user_id).strip() and user_id in allow_list:
        return True
    if roles:
        return any(role in allow_list for role in roles)
    return False


def _check_rollout_pct(rollout: Any, user_id: str | None) -> bool:
    if rollout is None or rollout == "" or user_id is None or not str(user_id).strip():
        return False
    try:
        pct = int(rollout)
...
def evaluate_flag_with_rollout(
    ...
    if _check_allow_list(cfg.get("allow_list"), user_id, roles):
        return True

    return _check_rollout_pct(cfg.get("rollout_percentage"), user_id)
