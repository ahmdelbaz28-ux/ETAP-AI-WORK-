...
# ---------------------------------------------------------------------------
# Document Ingestion (chunkr-style hierarchical chunking)
# ---------------------------------------------------------------------------
# This endpoint ingests PDF/DOCX/PPTX files, extracts structured segments,
# and chunks them using chunkr's hierarchical chunking algorithm (ported to
# Python in ai_context_engine/text_chunker.py). The resulting chunks can be
# stored in the RAG knowledge base for retrieval.
#
# This is an ADDITIVE endpoint — it does not modify any existing endpoint.
# The chunkr integration is self-contained in ai_context_engine/ and degrades
# gracefully if optional dependencies (python-docx, python-pptx) are missing.


@router.get("/rag/supported-formats")
async def rag_supported_formats(request: Request):
    """Return which document formats are available for ingestion.

    Useful for the UI to show which file types can be uploaded.
    """
    trace_id = getattr(request.state, "trace_id", "unknown")
    try:
...
async def rag_ingest_document(request: Request):
    """Ingest a PDF/DOCX/PPTX document and return hierarchical chunks.

    Accepts a JSON body with:
    - file_path: Path to the document on the server (required)
    - target_length: Target word count per chunk (optional, default 500)
    - ignore_headers_and_footers: Skip page headers/footers (optional, default True)

    Returns the extracted chunks as JSON. Each chunk has:
    - content: concatenated text from its segments
    - word_count: total words in the chunk
    - segment_types: list of segment types in this chunk
    - page_numbers: pages referenced by the chunk

    Note: This endpoint does NOT store the chunks — it only returns them.
    Storage into the RAG knowledge base is a separate step (future endpoint).
    """
    trace_id = getattr(request.state, "trace_id", "unknown")
    try:
        ...
        target_length = int(body.get("target_length", 500))
        ignore_headers = bool(body.get("ignore_headers_and_footers", True))

        if not file_path:
            raise HTTPException(status_code=400, detail="file_path is required")

        if target_length <= 0:
            raise HTTPException(status_code=400, detail="target_length must be > 0")
        ...
        chunks = ingest_document(
            ...
            ignore_headers_and_footers=ignore_headers,
        )
        ...
        logger = getLogger("engineering_service")
        logger.error("rag_ingest_failed error=%s", str(e), extra={"trace_id": trace_id})
        return JSONResponse(
            status_code=500, content={"success": False, "errors": [str(e)], "trace_id": trace_id}
        )
