...
Generated: 2026-07-25 21:01

## Summary

| Confidence | Count | Action |
|------------|-------|--------|
| HIGH | 87 | Consolidate immediately |
| MEDIUM | 64 | Investigate further |
| LOW | 14 | Review if time permits |
...
### Validate domain-specific agent results using the same errors-list→extend→return-len(errors)==0 skeleton

**Category:** agent

**Functions:**
- `validate_result` in `ETAP-AI-WORK/agents/anomaly_agent.py:613` - Checks anomaly methods (SPC/CUSUM/EWMA), counts, severity. errors list → extend → return len==0.
- `validate_result` in `ETAP-AI-WORK/agents/arc_flash_agent.py:421` - Checks incident energy, arc boundary, PPE category, arc current. errors list → extend → return len==0.
- `validate_result` in `ETAP-AI-WORK/agents/battery_storage_agent.py:925` - Checks BESS power/energy capacity, efficiency, NPV, cycle utilization. errors list → extend → return len==0.
- `validate_result` in `ETAP-AI-WORK/agents/cable_sizing_agent.py:709` - Checks derated ampacity, voltage drop compliance, short-circuit adequacy. errors list → extend → return len==0.
- `validate_result` in `ETAP-AI-WORK/agents/earth_grid_agent.py:826` - Checks mesh/step/touch voltage safety per IEEE 80. errors list → extend → return len==0.
- `validate_result` in `ETAP-AI-WORK/agents/goal_planner_agent.py:476` - Checks task count, hours, composite scores, duplicate names. errors list → extend → return len==0.
- `validate_result` in `ETAP-AI-WORK/agents/motor_starting_agent.py:501` - Checks starting current, voltage dip, acceleration time. errors list → extend → return len==0.
- `validate_result` in `ETAP-AI-WORK/agents/predictive_agent.py:644` - Checks forecast non-negativity, MAPE, failure probability range, priority. errors list → extend → return len==0.
- `validate_result` in `ETAP-AI-WORK/agents/renewable_agent.py:816` - Checks PV/wind capacity factor, IEEE 1547 compliance, hosting capacity. errors list → extend → return len==0.
- `validate_result` in `ETAP-AI-WORK/agents/scada_agent.py:933` - Checks connection status, invalid measurements ratio, bus quality, anomaly count. errors list → extend → return len==0.
- `validate_result` in `ETAP-AI-WORK/agents/stability_agent.py:612` - Checks transient stability, small-signal eigenvalues, CCT. errors list → extend → return len==0.
- `validate_result` in `ETAP-AI-WORK/agents/weather_agent.py:470` - Checks derating factor range, dynamic rating, risk level. errors list → extend → return len==0.
- `validate_result` in `ETAP-AI-WORK/agents/orchestrator.py:363` - LoadFlowAgent — checks convergence + voltage limits. Returns len(errors)==0 instead of bool directly.
- `validate_result` in `ETAP-AI-WORK/agents/orchestrator.py:486` - ShortCircuitAgent — checks fault current positivity. Returns len(errors)==0.
- `validate_result` in `ETAP-AI-WORK/agents/orchestrator.py:605` - HarmonicAnalysisAgent — checks violations list. Returns True/False directly (not len(errors)==0 pattern).
- `validate_result` in `ETAP-AI-WORK/agents/orchestrator.py:716` - OptimalPowerFlowAgent — checks convergence + power balance. Returns True/False directly.
- `validate_result` in `ETAP-AI-WORK/agents/orchestrator.py:818` - ProtectionCoordinationAgent — checks coordination margins. Returns True/False directly.
- `validate_result` in `ETAP-AI-WORK/agents/orchestrator.py:929` - ETAPExecutionAgent — checks success flag + errors. Returns True/False directly.

**Differences:** All 18 functions share the same structural skeleton: (1) create errors: list[str] = [], (2) extract domain-specific data from result.data, (3) validate domain-specific constraints appending to errors, (4) result.validation_errors.extend(errors), (5) return len(errors) == 0. The domain-specific checks differ per agent (anomaly methods vs arc flash physics vs cable sizing criteria, etc.). Some orchestrator subclasses (HarmonicAnalysisAgent, OPFAgent, ProtectionAgent, ETAPExecutionAgent) return True/False directly instead of len(errors)==0, creating a subtle inconsistency. The BaseAgent.validate_result (line 232) provides a default with basic sanity checks (status, non-empty data) that subclasses override.

**Recommendation:** Keep `BaseAgent.validate_result in orchestrator.py (line 232)` - Create a template-method pattern: BaseAgent.validate_result runs common checks (status, non-empty data) then calls self._validate_domain(result) which each subclass overrides. This eliminates the repeated errors-list→extend→return skeleton across 18 functions while preserving domain-specific logic. Also standardize return style to always use len(errors)==0 for consistency.

---

### Validate load-flow, short-circuit, harmonic, and OPF results — duplicated as both agent.validate_result and ValidationAgent._validate_* methods

**Category:** agent

**Functions:**
- `validate_result` in `ETAP-AI-WORK/agents/orchestrator.py:363` - LoadFlowAgent.validate_result — checks convergence + bus voltage limits per self.voltage_limits.
- `_validate_load_flow` in `ETAP-AI-WORK/agents/orchestrator.py:1032` - ValidationAgent._validate_load_flow — checks convergence + bus voltage limits per self.standards dict. Returns {status, issues} dict instead of bool.
- `validate_result` in `ETAP-AI-WORK/agents/orchestrator.py:486` - ShortCircuitAgent.validate_result — checks fault current positivity, returns bool.
- `_validate_short_circuit` in `ETAP-AI-WORK/agents/orchestrator.py:1051` - ValidationAgent._validate_short_circuit — checks fault current > 100kA threshold, returns {status, issues} dict. Slightly different threshold logic.
- `validate_result` in `ETAP-AI-WORK/agents/orchestrator.py:605` - HarmonicAnalysisAgent.validate_result — checks violations list.
- `_validate_harmonic` in `ETAP-AI-WORK/agents/orchestrator.py:1068` - ValidationAgent._validate_harmonic — checks violations + resonance detection. More comprehensive than HarmonicAgent.validate_result.
- `validate_result` in `ETAP-AI-WORK/agents/orchestrator.py:716` - OptimalPowerFlowAgent.validate_result — checks convergence + power balance.
- `_validate_opf` in `ETAP-AI-WORK/agents/orchestrator.py:1082` - ValidationAgent._validate_opf — checks convergence only (less thorough than OPFAgent.validate_result which also checks power balance).

**Differences:** Each pair (validate_result vs _validate_*) validates the same study type but with: (1) different return types (bool vs dict{status, issues}), (2) slightly different threshold values (voltage_limits dict vs self.standards dict, fault current <=0 vs >100kA), (3) different completeness levels (_validate_harmonic adds resonance detection; _validate_opf omits power balance). The _validate_* methods are used by ValidationAgent.execute for cross-study comprehensive validation, while validate_result is the per-agent post-execution check.

**Recommendation:** Keep `A shared validation registry module (e.g., agents/validation_checks.py)` - Create a single canonical validation function per study type that returns both a bool and a issues list. Each agent's validate_result calls it; ValidationAgent._validate_* calls the same function. Eliminates 4 pairs of near-identical logic with different thresholds/return types. Thresholds should be configurable parameters, not hardcoded in each copy.

---

### Validate proposed user guide operation steps against official documentation via term-matching RAG

**Category:** agent

**Functions:**
- `validate_etap_operation` in `ETAP-AI-WORK/etap_user_guide/etap_guide_rag.py:266` - Fetches official ETAP procedure, checks found, then term-matches proposed steps against official text. Returns {valid, operation, official_steps, proposed_steps, compliance, issues, warnings}.
- `validate_zenon_operation` in `ETAP-AI-WORK/zenon_user_guide/zenon_guide_rag.py:172` - Fetches official Zenon procedure, checks found, then term-matches proposed steps against official text. Returns identical {valid, operation, official_steps, proposed_steps, compliance, issues, warnings} structure.

**Differences:** Nearly identical implementation: both (1) call get_X_procedure(operation) to fetch official steps, (2) check if found, (3) return {valid:False, reason} if not found, (4) build validation dict with identical keys, (5) use the exact same term-matching algorithm (step.lower().split() → intersection with official_text.split() → classify as compliant/not_found_in_guide). The only differences are: method names (get_etap_procedure vs get_zenon_procedure), guide name in error message ('ETAP User Guide' vs 'Zenon User Guide'), and support referral ('ETAP support' vs 'Zenon support').

**Recommendation:** Keep `A base UserGuideRAG class with validate_operation(operation, proposed_steps, guide_name, get_procedure_fn)` - The term-matching algorithm and validation result structure are identical. Create a shared UserGuideRAG base class with a generic validate_operation method that accepts the guide name and procedure-fetcher as parameters. ETAP and Zenon subclasses only need to provide their own get_procedure() and guide_name. Eliminates ~90% duplicated code.
...
### Make an HTTP POST request to an external API (with httpx/urllib fallback variants)

**Category:** http-api-database

**Functions:**
- `_make_request` in `ETAP-AI-WORK/integrations/anthropic_vision.py:286` - Dispatching wrapper that tries httpx then falls back to urllib for Anthropic vision API POST.
- `_make_request_httpx` in `ETAP-AI-WORK/integrations/anthropic_vision.py:294` - POST to Anthropic API using httpx async client. Handles image encoding + JSON payload.
- `_make_request_urllib` in `ETAP-AI-WORK/integrations/anthropic_vision.py:305` - POST to Anthropic API using urllib.request. Fallback when httpx unavailable.
- `_make_request` in `ETAP-AI-WORK/integrations/openai_vision.py:329` - Dispatching wrapper that tries httpx then falls back to urllib for OpenAI vision API POST. Identical structure to anthropic_vision._make_request.
- `_make_request_httpx` in `ETAP-AI-WORK/integrations/openai_vision.py:342` - POST to OpenAI API using httpx async client. Same pattern as anthropic_vision version but different endpoint/payload schema.
- `_make_request_urllib` in `ETAP-AI-WORK/integrations/openai_vision.py:355` - POST to OpenAI API using urllib.request. Fallback when httpx unavailable. Same structure as anthropic_vision version.
- `_http_post_json` in `ETAP-AI-WORK/integrations/resend_email.py:133` - Async HTTP POST with JSON payload to Resend email API via httpx. Same fundamental pattern (httpx POST + JSON).
- `_sync_post` in `ETAP-AI-WORK/integrations/resend_email.py:154` - Synchronous HTTP POST for Resend email API via httpx. Same pattern, blocking variant.

**Differences:** anthropic_vision and openai_vision have an identical 3-layer pattern (_make_request dispatcher → _make_request_httpx → _make_request_urllib fallback). The resend_email functions are simpler single-implementation POSTs. All share the same core logic: build headers, encode JSON body, send via httpx (or urllib), parse response. Differences: target endpoints, auth headers, payload schemas, and image-encoding logic in the vision modules.

**Recommendation:** Keep `Extract a shared http_client module with generic POST helpers` - The anthropic_vision and openai_vision modules duplicate the exact same 3-tier httpx/urllib fallback pattern. A shared http_client.py module could provide: (1) a generic async_post_json(url, headers, payload, timeout) using httpx, (2) a generic sync_post_json using httpx, (3) a urllib fallback variant, and (4) a make_request dispatcher. Each integration would then call these helpers with provider-specific config instead of reimplementing the entire HTTP stack. The resend_email helpers are already simple enough to become direct callers of the shared module.

---

### Extract the real client IP address from reverse-proxy/CDN request headers

**Category:** http-api-database

**Functions:**
- `get_client_ip` in `ETAP-AI-WORK/api/akamai_protection.py:109` - Checks True-Client-IP (Akamai) → X-Forwarded-For → X-Real-IP → request.client.host. Returns string.
- `get_client_ip` in `ETAP-AI-WORK/api/cloudflare_protection.py:103` - Checks CF-Connecting-IP (Cloudflare) → X-Forwarded-For → X-Real-IP → request.client.host. Returns string. Identical logic structure, different preferred header.

**Differences:** Same function name, same 4-step cascade (CDN-specific header → X-Forwarded-For → X-Real-IP → fallback). Only difference is which CDN header is checked first: Akamai uses True-Client-IP, Cloudflare uses CF-Connecting-IP. The X-Forwarded-For, X-Real-IP, and client.host fallback chain is identical.

**Recommendation:** Keep `Extract a shared get_client_ip(request, preferred_header=None) in api/dependencies.py or a CDN utils module` - The two functions are structurally identical except for the CDN-specific header name. A single function that takes a configurable preferred header parameter eliminates the duplication while preserving the ability to prioritize different CDN headers. The akamai_protection and cloudflare_protection modules would each call the shared function with their respective header name.

---

### Extract CDN-specific metadata from request headers (country, region, threat scores, etc.)

**Category:** http-api-database

**Functions:**
- `get_akamai_metadata` in `ETAP-AI-WORK/api/akamai_protection.py:125` - Extracts Akamai-specific headers (True-Client-IP, X-Akamai-Country, Akamai-Bot-Score, etc.) into a dict.
- `get_cloudflare_metadata` in `ETAP-AI-WORK/api/cloudflare_protection.py:124` - Extracts Cloudflare-specific headers (CF-Connecting-IP, CF-IPCountry, CF-Bot-Score, etc.) into a dict. Same pattern, different header names.

**Differences:** Same pattern: iterate over a mapping of metadata fields to CDN-specific header names, extract values from request.headers, return a dict. Only difference is the specific header name mapping (Akamai headers vs Cloudflare headers). The output dict structure (country, bot_score, etc.) is conceptually the same.

**Recommendation:** Keep `Extract a shared get_cdn_metadata(request, header_map) function` - Both functions follow the exact same pattern of extracting header values via a name mapping. A generic get_cdn_metadata function that accepts a header-to-field mapping dict would eliminate duplication. Each CDN module would define its own mapping and call the shared extractor. This also makes it trivial to add support for future CDN providers.

---

### Get/create a Redis client instance for module-specific storage operations

**Category:** http-api-database

**Functions:**
- `_get_redis` in `ETAP-AI-WORK/services/email_send_log.py:49` - Returns a Redis client instance for email send log storage. Creates new client per call or uses cached instance.
- `_get_redis` in `ETAP-AI-WORK/services/otp_store.py:53` - Returns a shared async Redis client for OTP storage. Caches the client in a module-level variable.
- `get_redis_state_client` in `ETAP-AI-WORK/core/redis_state.py:69` - Returns the shared async Redis client or None if unavailable. Module-level singleton pattern.
- `_get_redis_client` in `ETAP-AI-WORK/api/auth.py:126` - Returns the shared async Redis client or None if Redis unavailable. Same singleton-or-None pattern as redis_state.get_redis_state_client.
- `_get_sync_redis` in `ETAP-AI-WORK/etap_integration/worker_registry.py:90` - Returns a synchronous Redis client for heartbeat thread. Separate sync client instance.
- `_get_async_redis` in `ETAP-AI-WORK/etap_integration/worker_registry.py:101` - Returns an async Redis client for FastAPI endpoints. Module-level async singleton.
- `_get_redis` in `ETAP-AI-WORK/etap_integration/worker_registry.py:166` - Get Redis client for worker registry operations. Returns the async client from _get_async_redis.
- `_get_redis` in `ETAP-AI-WORK/etap_integration/worker_registry.py:251` - Get async Redis client for worker pool operations. Second _get_redis in the same file (different class).

**Differences:** All follow the pattern: check if a Redis client exists → create one if not → cache it module-level → return it. Variations: (1) some return None on failure (auth, redis_state), others raise or create new; (2) some are sync clients (worker_registry._get_sync_redis), most are async; (3) worker_registry has TWO different _get_redis functions at lines 166 and 251 (different classes); (4) email_send_log and otp_store have identical _get_redis naming but in different modules with potentially different client configs.

**Recommendation:** Keep `Create a shared Redis client factory in core/redis_client.py` - 8 functions across 4 modules all implement the same Redis client acquisition pattern (singleton + fallback). A centralized RedisClientFactory in core/redis_client.py could provide: get_async_client(), get_sync_client(), get_client_or_none(). Each module would call the factory instead of reimplementing the connection/ caching logic. This also ensures consistent error handling (None vs raise), connection pool sharing, and makes it easy to switch Redis configurations globally.

---

### Initialize database tables/schema on first use (create table if not exists)

**Category:** http-api-database

**Functions:**
- `_init_db` in `ETAP-AI-WORK/services/api_key_store.py:199` - Creates api_keys SQLite table if not exists. Called lazily on first key operation. Simple CREATE TABLE IF NOT EXISTS.
- `_init_db` in `ETAP-AI-WORK/core/database.py:60` - Initialize database tables and schema on first connection. Creates multiple tables + loads initial data. More comprehensive.
- `init_db` in `ETAP-AI-WORK/api/database.py:255` - Create all database tables that do not yet exist (migration bootstrap). Uses SQLAlchemy create_all on async engine.
- `_initialize_db` in `ETAP-AI-WORK/knowledge/rag_engine.py:222` - Initialize the vector database for knowledge storage. Dispatches to _init_chroma, _init_faiss, or _init_memory based on config.

**Differences:** All perform 'ensure tables exist' logic. api_key_store._init_db and core/database._init_db use raw SQLite CREATE TABLE IF NOT EXISTS. api/database.init_db uses SQLAlchemy metadata.create_all on async engine. rag_engine._initialize_db initializes a vector store (ChromaDB/FAISS/in-memory). Different database backends and different table schemas, but the same lazy-initialization pattern.

**Recommendation:** Keep `N/A` - The structural pattern (lazy init, create-if-not-exists) is duplicated 4 times across different database backends (SQLite, SQLAlchemy async, vector stores). The SQL-based versions (api_key_store and core/database) could share a SQLite init helper. The SQLAlchemy version (api/database.init_db) is necessarily different because it uses ORM metadata. The vector store init is entirely different. Partial consolidation possible for the two SQLite versions, but full consolidation is blocked by fundamentally different DB technologies.

---

### Fetch a database record by ID or raise HTTP 404 if not found

**Category:** http-api-database

**Functions:**
- `_get_category_by_id` in `ETAP-AI-WORK/api/equipment.py:282` - db.get(Category, id) → raise HTTPException(404) if None. Standard SQLAlchemy get-or-404 pattern.
- `_get_equipment_by_id` in `ETAP-AI-WORK/api/equipment.py:296` - db.get(Equipment, id) → raise HTTPException(404) if None. Same pattern, different model.
- `_get_role_by_id` in `ETAP-AI-WORK/api/rbac.py:310` - db.get(Role, id) → raise HTTPException(404) if None. Same pattern, different model.
- `_get_permission_by_id` in `ETAP-AI-WORK/api/rbac.py:322` - db.get(Permission, id) → raise HTTPException(404) if None. Same pattern, different model.
- `_get_study_result` in `ETAP-AI-WORK/api/study_versions.py:107` - db.get(StudyResult, id) → raise HTTPException(404) if None. Same pattern, different model.
- `_get_template` in `ETAP-AI-WORK/api/templates.py:125` - db.get(Template, id) → raise HTTPException(404) if None. Same pattern, different model.

**Differences:** All 6 functions follow the exact same pattern: (1) db.get(ModelClass, id) → (2) if result is None → raise HTTPException(status_code=404, detail='...not found'). Only differences: the SQLAlchemy model class and the detail message text. The logic is 100% structural duplication.

**Recommendation:** Keep `Extract a shared get_or_404(db, model, id, detail=None) helper in api/dependencies.py` - 6 identical get-or-404 helper functions across 4 files. A single generic get_or_404(db, model_class, item_id, detail=f'{model_class.__name__} not found') function eliminates all 6 private helpers. FastAPI's newer versions even provide this as a built-in (db.get_or_404), so migrating to that API would be the cleanest solution. Until then, a shared helper is trivial to implement and removes significant duplication.

---

### Broadcast/notification: send a message to all connected WebSocket clients simultaneously

**Category:** http-api-database

**Functions:**
- `_broadcast` in `ETAP-AI-WORK/api/cua_confirmation_ws.py:141` - Iterates over active WebSocket connections set, calls send_json on each. Gathers all sends concurrently via asyncio.
- `_notify_clients` in `ETAP-AI-WORK/api/dual_control.py:124` - Iterates over registered WebSocket clients dict, calls send_json on each. Same broadcast pattern for approval updates.

**Differences:** Both iterate over a set/dict of WebSocket connections and call send_json on each. cua_confirmation_ws uses asyncio.gather for concurrent sends; dual_control may iterate sequentially. Both handle disconnected clients by removing them from the active set on failure.

**Recommendation:** Keep `Extract a shared ws_broadcast(clients, message) utility in api/ws_utils.py` - Both functions implement the same WebSocket broadcast pattern (iterate connections, send JSON, remove disconnected). A shared broadcast utility with asyncio.gather and error handling would eliminate the duplication. Each module would maintain its own client registry but call the shared broadcast function.

---

### Search/retrieve from a vector database by semantic similarity (embeddings or query string)

**Category:** http-api-database

**Functions:**
- `retrieve` in `ETAP-AI-WORK/ai_context_engine/retriever.py:109` - Query ChromaDB vector store and return matching code chunks. Uses ChromaDB query API with embedding.
- `search` in `ETAP-AI-WORK/knowledge/rag_engine.py:300` - Dispatching search function that delegates to _search_chroma, _search_faiss, or _search_memory based on backend config.
- `_search_chroma` in `ETAP-AI-WORK/knowledge/rag_engine.py:318` - Search ChromaDB for similar documents by embedding. Uses ChromaDB query API — same underlying API as retriever.retrieve.
- `_search_faiss` in `ETAP-AI-WORK/knowledge/rag_engine.py:341` - Search FAISS index for similar documents by embedding. Alternative vector backend.
- `_search_memory` in `ETAP-AI-WORK/knowledge/rag_engine.py:361` - Search in-memory storage by cosine similarity. Fallback vector backend.
- `retrieve_knowledge` in `ETAP-AI-WORK/knowledge/rag_engine.py:684` - High-level retrieval: takes query string → generates embedding → calls search → returns formatted results. Wraps the search function.
- `retrieve_context` in `ETAP-AI-WORK/api/context_engine.py:20` - API endpoint that retrieves shared context from the context engine. Calls retriever.retrieve under the hood.

**Differences:** retriever.retrieve and rag_engine._search_chroma both query ChromaDB but for different domains (code chunks vs engineering documents). rag_engine.search is a dispatcher over multiple backends. retrieve_knowledge is a higher-level wrapper that adds embedding generation. retrieve_context is the HTTP endpoint that calls retriever.retrieve. The core similarity: all take an embedding/query, search a vector store, return ranked results.

**Recommendation:** Keep `N/A` - Two separate vector retrieval subsystems (ai_context_engine/retriever for code context, knowledge/rag_engine for engineering documents) that query the same type of backend (ChromaDB). Could potentially share a VectorStore abstraction layer that normalizes search across ChromaDB/FAISS/in-memory, with each domain defining its own collection/schema. However, the two systems serve fundamentally different content types and may need independent tuning, so partial consolidation (shared backend abstraction, separate retrieval logic) is more appropriate than full merge.
...
### Sidebar navigation component — render a collapsible nav sidebar with health polling, theme toggle, grouped nav links, and RTL support

**Category:** ui-helpers

**Functions:**
- `Sidebar` in `ETAP-AI-WORK/ui/src/components/Sidebar.tsx:87` - Full-featured sidebar with desktop+mobile drawer, BrandLogo header, health status dot, navItems array, grouped sections, theme/collapse buttons. 442 lines.
- `Sidebar` in `ETAP-AI-WORK/ui/src/components/layout/Sidebar.tsx:78` - Stripped-down sidebar with same navItems, sectionOrder, sectionLabels, sectionIcons arrays, same health polling logic, same theme/collapse toggle pattern. Uses StatusIndicator component instead of inline dot. 253 lines. No mobile drawer.

**Differences:** Both export identically-named Sidebar with the same NavItem interface, navItems array (slight item differences: components/Sidebar has GridEditor and ScadaIntegration; layout/Sidebar omits them), identical sectionOrder/sectionLabels/sectionIcons configs, and identical fetchHealth polling (30s interval). components/Sidebar adds a mobile drawer overlay with backdrop; layout/Sidebar only renders a single aside. components/Sidebar uses inline health status dots; layout/Sidebar uses the reusable StatusIndicator component. Both have RTL chevron handling and theme toggle.

**Recommendation:** Keep `ETAP-AI-WORK/ui/src/components/layout/Sidebar.tsx` - Two Sidebar components with the same name, same purpose, and 80%+ identical logic. layout/Sidebar is already cleaner (uses StatusIndicator, no mobile duplication). Merge the mobile-drawer feature from components/Sidebar into layout/Sidebar, delete components/Sidebar.tsx, and ensure all imports resolve to the single canonical module.

---

### API client — authenticate requests, set Bearer token, attach provider headers, handle errors, and make HTTP calls to the backend

**Category:** ui-helpers

**Functions:**
- `request<T>` in `ETAP-AI-WORK/ui/src/lib/api.ts:13` - Centralized request function: Bearer token, Content-Type, provider headers from getCachedSettings(), 15s timeout, structured error extraction, 204 handling. ~82 lines core + ~300 lines of fetch* endpoint wrappers.
- `apiClient.get / .post / .put / .delete` in `ETAP-AI-WORK/ui/src/hooks/useApi.ts:21` - React-Query-based API client: each method independently reads Bearer token, sets Content-Type, does error handling with the same pattern. No provider header injection. GET 30s timeout, POST 60s, PUT 30s, DELETE 30s. ~133 lines. Wraps fetch calls identically to request<T> but without provider header forwarding.
- `fetchStatus` in `ETAP-AI-WORK/ui/src/pages/DigitalTwin.tsx:394` - Inline fetch: reads Bearer token from localStorage, constructs headers, 8s timeout, error extraction. Bypasses both centralized clients.
- `fetchKillSwitch / fetchAuditLog / activateKill / deactivateKill` in `ETAP-AI-WORK/ui/src/pages/CuaMonitor.tsx:40` - Four inline fetch callbacks that each manually read Bearer token, construct headers, make raw fetch calls. No timeout, no Content-Type on GETs. Bypasses both centralized clients.
- `fetchAssets` in `ETAP-AI-WORK/ui/src/pages/AssetManagement.tsx:119` - Inline fetch callback with same Bearer token + fetch pattern. Bypasses both centralized clients.
- `fetchProjects` in `ETAP-AI-WORK/ui/src/pages/Projects.tsx:43` - Inline fetch callback with same Bearer token + fetch pattern. Bypasses both centralized clients.

**Differences:** lib/api.ts's request<T> is the most complete: injects provider headers, handles 204, extracts structured errors. useApi.ts's apiClient methods duplicate the auth+headers+error pattern per HTTP method but lack provider header forwarding. The inline page fetches (DigitalTwin, CuaMonitor, AssetManagement, Projects) all reinvent the same token+headers+fetch+error pattern at different quality levels (missing timeouts, missing Content-Type, missing provider headers). useApi.ts also duplicates every endpoint that lib/api.ts already exposes as a standalone function (fetchHealth, fetchAgents, etc.) but wraps them in React-Query hooks that nobody actually imports — all pages import from lib/api.ts directly.

**Recommendation:** Keep `ETAP-AI-WORK/ui/src/lib/api.ts (enhance request<T> to support all HTTP methods)` - lib/api.ts is the single authoritative API client with provider header injection and proper error handling. useApi.ts adds no value — its apiClient methods duplicate request<T> without provider headers, and its React-Query hooks duplicate the same endpoint wrappers that lib/api.ts already provides. The 4+ inline page fetches should be replaced by adding the missing endpoints to lib/api.ts (digital-twin/status, admin/cua/kill-switch, etc.). Delete useApi.ts entirely; optionally add a thin React-Query wrapper layer that calls request<T> instead of reimplementing fetch.

---

### Empty state placeholder — render an icon, title, description, and optional action when no data is available

**Category:** ui-helpers

**Functions:**
- `EmptyState` in `ETAP-AI-WORK/ui/src/components/ui/EmptyState.tsx:12` - Simple empty state: icon (ReactNode), title, description, action (ReactNode), className. Minimal styling — centered flex column, muted colors.
- `PremiumEmptyState` in `ETAP-AI-WORK/ui/src/components/ui/Visual.tsx:117` - Premium empty state: icon (ElementType), title, description, action ({label, onClick, icon}), variant (default/illustration). Gradient backgrounds, larger padding, richer typography.

**Differences:** Both serve the same semantic purpose (no-data placeholder with icon+title+description+action). EmptyState is minimal/styled; PremiumEmptyState adds gradient icon backgrounds, illustration variant, and structured action button. Icon prop differs: ReactNode vs ElementType. Action prop differs: ReactNode vs structured object.

**Recommendation:** Keep `ETAP-AI-WORK/ui/src/components/ui/EmptyState.tsx (add variant and premium styling as optional modes)` - Two components for the same UX pattern. Merge PremiumEmptyState's variant and richer action prop into EmptyState with a variant prop ('default' | 'premium' | 'illustration'). Delete PremiumEmptyState from Visual.tsx. This gives one canonical empty-state component with a styling toggle.

---

### Loading/skeleton placeholder — render visual placeholders or spinners while data is being fetched

**Category:** ui-helpers

**Functions:**
- `Skeleton / SkeletonCard / SkeletonTable` in `ETAP-AI-WORK/ui/src/components/ui/Skeleton.tsx:16` - Generic skeleton primitives: Skeleton (single bar), SkeletonCard (card-shaped placeholder with lines), SkeletonTable (grid placeholder). Reusable and composable.
- `PremiumLoading` in `ETAP-AI-WORK/ui/src/components/ui/Visual.tsx:168` - Premium loading component with 3 variants: spinner (animated circle), skeleton (hardcoded 3-row card list), progress (bar). Uses inline 'skeleton' CSS class that duplicates Skeleton/SkeletonCard.
- `DashboardSkeleton` in `ETAP-AI-WORK/ui/src/pages/Dashboard.tsx:255` - Page-specific skeleton layout that manually recreates skeleton card patterns (bg-[var(--bg-card)] rounded-xl + skeleton divs) instead of using SkeletonCard.
- `LoaderIcon` in `ETAP-AI-WORK/ui/src/pages/AIAssistant.tsx:633` - Animated SVG spinner icon (4 circle segments with stagger animation). Duplicates the spinner concept from PremiumLoading's spinner variant.

**Differences:** Skeleton/SkeletonCard/SkeletonTable are the canonical reusable primitives. PremiumLoading's skeleton variant renders a fixed layout that could be composed from SkeletonCard. DashboardSkeleton builds its own skeleton cards from raw div+skeleton classes. LoaderIcon is a custom SVG spinner vs PremiumLoading's CSS-animated border spinner. All serve the same UX purpose: indicate loading state.

**Recommendation:** Keep `ETAP-AI-WORK/ui/src/components/ui/Skeleton.tsx (add Spinner and ProgressBar variants)` - Skeleton.tsx already provides composable primitives. Add Spinner (animated SVG or CSS) and ProgressBar variants to it. Delete PremiumLoading from Visual.tsx (its skeleton variant should use SkeletonCard; its spinner becomes Skeleton.Spinner). Replace DashboardSkeleton's manual skeleton divs with SkeletonCard components. Replace LoaderIcon with Skeleton.Spinner. This gives one canonical loading/skeleton module.

---

### Card container — render a styled card/panel with border, background, optional header, and content

**Category:** ui-helpers

**Functions:**
- `Card / CardHeader / CardSection` in `ETAP-AI-WORK/ui/src/components/ui/Card.tsx:26` - Generic card system: Card with 5 variants (default/interactive/bordered/flat/glass), 4 padding sizes, hover option. CardHeader (icon+title+subtitle+action). CardSection (bordered content divider).
- `GlowCard` in `ETAP-AI-WORK/ui/src/components/ui/Visual.tsx:331` - Card with animated glow border on hover: 4 color presets (primary/success/warning/danger). Same base structure (bg-card, rounded-xl, border) as Card.
- `GlassPanel` in `ETAP-AI-WORK/ui/src/components/ui/Visual.tsx:11` - Glassmorphism panel: 3 variants (default/elevated/subtle) with backdrop-blur. Overlaps Card's 'glass' variant.
- `StatCard` in `ETAP-AI-WORK/ui/src/pages/Dashboard.tsx:118` - Dashboard-specific stat card with icon, value, label, trend badge, sparkline chart, and color accent. Could extend Card.

**Differences:** Card is the canonical card component with a 'glass' variant that already covers GlassPanel's purpose. GlowCard adds animated glow borders, which could be a Card variant. StatCard is a composition: Card + CardHeader + sparkline, but it's built inline instead of composing the Card primitives. GlassPanel uses backdrop-blur which Card's glass variant already supports.

**Recommendation:** Keep `ETAP-AI-WORK/ui/src/components/ui/Card.tsx (add glow and glass panel variants)` - Card already has a variant system (5 variants + padding + hover). Add 'glow' variant (with color prop) to absorb GlowCard. Card's existing 'glass' variant already covers GlassPanel's default/elevated/subtle modes — enhance it with backdrop-blur styling. StatCard should compose Card + CardHeader + Sparkline instead of building its own card divs. Delete GlowCard and GlassPanel from Visual.tsx.

---

### Animated auth-page background — render a decorative SVG circuit/power-system diagram with grid overlay and ambient glow for login/register pages

**Category:** ui-helpers

**Functions:**
- `LoginBackground` in `ETAP-AI-WORK/ui/src/components/LoginBackground.tsx:19` - Interactive circuit board SVG with breaker toggle, tooltip on hover, mouse coordinate HUD, language toggle. 625 lines. Uses viewBox 1440x900, grid CSS background, ambient glow divs.
- `LoginHeroAnimation` in `ETAP-AI-WORK/ui/src/components/LoginHeroAnimation.tsx:3` - Decorative SVG animation with floating node shapes, particle streams, central pulsing node. Non-interactive. 253 lines. Same viewBox 1440x900, same grid CSS pattern, same ambient glow divs.
- `RegisterBackground` in `ETAP-AI-WORK/ui/src/pages/Register.tsx:460` - Static decorative circuit SVG for registration page. Simplified version of LoginBackground (no interactivity, no tooltips). Same viewBox 1440x900, same grid CSS background, same gradient defs, similar bus line + transformer + flow animations.

**Differences:** All three render the same core visual pattern: CSS grid background (blue lines, 40px spacing), SVG viewBox 1440x900 with circuit/power-system elements (bus lines, transformer circles, animated dash flows, particle dots), and ambient background glow divs (large rounded blurred circles). LoginBackground is interactive (breaker toggle, tooltips, HUD); LoginHeroAnimation is decorative-only with different shapes (floating nodes vs circuit elements); RegisterBackground is a simplified static version of the circuit diagram. The grid CSS, viewBox, gradient defs, and ambient glow markup are nearly identical across all three.

**Recommendation:** Keep `ETAP-AI-WORK/ui/src/components/AuthBackground.tsx (new unified module)` - Three backgrounds for auth pages share 60-70% markup (grid overlay, SVG viewBox, gradient defs, ambient glows). Create a unified AuthBackground component with variant prop: 'interactive' (LoginBackground features), 'decorative' (LoginHeroAnimation), 'static' (RegisterBackground). Extract shared grid overlay and ambient glow into a shared sub-component or CSS class. Delete LoginBackground.tsx, LoginHeroAnimation.tsx, and RegisterBackground inline function.

---

### Modal overlay — render a backdrop overlay with escape-key handling, close-on-click, and optional header with close button

**Category:** ui-helpers

**Functions:**
- `Modal` in `ETAP-AI-WORK/ui/src/components/ui/Modal.tsx:24` - Complete modal: backdrop (bg-black/60 + backdrop-blur), escape key listener, close-on-overlay, header (title+subtitle+close button), body, footer. 115 lines.
- `ModalBackdrop` in `ETAP-AI-WORK/ui/src/components/ModalBackdrop.tsx:18` - Standalone backdrop component with escape key handling, close-on-click, accessibility. Duplicates Modal's backdrop div and escape listener.
- `ModalHeader` in `ETAP-AI-WORK/ui/src/components/ModalHeader.tsx:29` - Standalone header component with icon, title, close button. Duplicates Modal's inline header section.

**Differences:** Modal already includes backdrop rendering (line 68-71) and header rendering (lines 85-99) inline. ModalBackdrop re-implements the same backdrop pattern (escape key, close-on-overlay, z-index, bg-black/blur). ModalHeader re-implements the same header pattern (icon slot, title, close button). The standalone components exist because older code used them separately, but Modal now provides a unified solution.

**Recommendation:** Keep `ETAP-AI-WORK/ui/src/components/ui/Modal.tsx` - Modal already renders both backdrop and header. ModalBackdrop and ModalHeader are standalone versions of functionality that Modal includes inline. Delete ModalBackdrop.tsx and ModalHeader.tsx. If any code needs just a backdrop or just a header outside a Modal context, export those sub-components from Modal.tsx (Modal.Backdrop, Modal.Header) rather than maintaining separate files.

---

### Health status polling — periodically fetch /health endpoint and map the response to a displayable status (online/offline/checking)

**Category:** ui-helpers

**Functions:**
- `(inline useEffect in Sidebar)` in `ETAP-AI-WORK/ui/src/components/Sidebar.tsx:113` - Calls fetchHealth() on mount + 30s interval. Maps h.ok to 'online'/'offline'/'checking'. Identical pattern repeated twice in mobile drawer section.
- `(inline useEffect in Sidebar)` in `ETAP-AI-WORK/ui/src/components/layout/Sidebar.tsx:87` - Same fetchHealth polling: mount + 30s interval, same status mapping.
- `(inline useEffect in StatusBar)` in `ETAP-AI-WORK/ui/src/components/layout/StatusBar.tsx:17` - Same fetchHealth polling pattern in StatusBar component.
- `(inline useEffect in Dashboard)` in `ETAP-AI-WORK/ui/src/pages/Dashboard.tsx:319` - fetchHealth + fetchAgents combined with Promise.all, same catch pattern.

**Differences:** All four locations implement the same pattern: useEffect with fetchHealth() on mount + setInterval at 30s, mapping h.ok to online/offline, catching errors to set offline. Sidebar components also store status in useState; Dashboard stores health data directly. StatusBar does the same. The polling logic, interval duration, and error handling are identical.

**Recommendation:** Keep `ETAP-AI-WORK/ui/src/hooks/useHealthStatus.ts (new hook)` - The same fetchHealth polling pattern is duplicated in 4 components. Create a useHealthStatus() hook that encapsulates: mount fetch, 30s interval, status mapping, cleanup. Each component calls the hook and gets { healthStatus, healthData }. This eliminates ~40 lines of duplicated useEffect logic per consumer.
...
### Check runtime dependencies for CUA/desktop automation execution

**Category:** agent

**Functions:**
- `check_dependencies` in `ETAP-AI-WORK/agents/cua_executor.py:240` - Checks pyautogui, pytesseract, tesseract binary, gemini_vision. Returns dict with all_available + per-dep booleans.
- `check_dependencies` in `ETAP-AI-WORK/agents/browser_cua_executor.py:170` - Checks playwright, chromium binary, gemini_vision. Returns dict with all_available + per-dep booleans + missing list.
- `_check_chromium_installed` in `ETAP-AI-WORK/agents/browser_cua_executor.py:82` - Module-level helper: searches multiple filesystem paths for chromium binary. Used by BrowserCUAExecutor.check_dependencies.
- `_check_gui_deps` in `ETAP-AI-WORK/agents/etap_gui_agent.py:70` - Checks pyautogui, PIL, google.generativeai, display server, pytesseract. Returns (bool, missing_list) tuple instead of dict.

**Differences:** All four functions check runtime dependency availability but for different automation stacks: cua_executor (desktop pyautogui+tesseract), browser_cua_executor (playwright+chromium), etap_gui_agent (pyautogui+PIL+Gemini). Return types differ: cua_executor and browser_cua_executor return dicts; etap_gui_agent returns a tuple. The _check_chromium_installed helper is unique to browser automation. cua_executor and etap_gui_agent overlap significantly (both check pyautogui + gemini + tesseract), but cua_executor uses lazy import pattern while etap_gui_agent uses try/except directly.

**Recommendation:** CONSOLIDATE - Extract a shared dependency checking framework with a unified return format (dict with all_available + per-dep booleans + missing list). Each executor calls the shared checker with its own dependency list. The pyautogui/PIL/Gemini/tesseract checks overlap between cua_executor and etap_gui_agent and should live in one place. The chromium-specific check stays as a specialization but shares the framework. Eliminates the inconsistent tuple vs dict return types.

---

### Run engineering validation checks on a model (voltage, overcurrent, coordination, cable sizing, model integrity)

**Category:** agent

**Functions:**
- `_handle_validate_design` in `ETAP-AI-WORK/copilot/mcp/server.py:573` - MCP tool: orchestrates voltage/overcurrent/coordination checks on self._model. Returns {success, passed, checks}.
- `_handle_run_engineering_checks` in `ETAP-AI-WORK/copilot/mcp/server.py:590` - MCP tool: more comprehensive — adds model_integrity + cable_sizing. Takes model_json arg, builds UnifiedEngineeringModel. Returns {success, passed, checks}.
- `_check_voltage_levels` in `ETAP-AI-WORK/copilot/mcp/server.py:613` - Checks panel voltage_nominal_v > 0 on self._model. Returns {passed, issues}.
- `_check_overcurrent` in `ETAP-AI-WORK/copilot/mcp/server.py:620` - Checks main breaker <= bus rating. Returns {passed, issues}.
- `_check_coordination` in `ETAP-AI-WORK/copilot/mcp/server.py:631` - Checks feeder rated_current < main breaker. Returns {passed, issues}.
- `_check_cable_sizing` in `ETAP-AI-WORK/copilot/mcp/server.py:642` - Checks cable length > 0 on model arg. Returns {passed, issues}.
- `_check_model_integrity` in `ETAP-AI-WORK/copilot/mcp/server.py:653` - Checks model has panels or buildings. Returns {passed, issues}.

**Differences:** The MCP server validation checks operate on UnifiedEngineeringModel (panel-based data structure) while the agent validate_result methods operate on AgentResult.data (dict-based results). They validate overlapping concepts (voltage limits, overcurrent, coordination, cable sizing) but on fundamentally different data representations. The two handler functions (_handle_validate_design and _handle_run_engineering_checks) are themselves near-duplicates: both orchestrate subsets of the same _check_* methods and return the same {success, passed, checks} format, differing only in which checks they include and whether they accept an external model_json.

**Recommendation:** CONSOLIDATE - Merge _handle_validate_design into _handle_run_engineering_checks: the 'validate_design' variant is just a subset of the same checks. Add a check_types filter parameter so callers can select which checks to run. The individual _check_* methods should stay as they operate on a different data model than the agent validate_result methods — they are not true duplicates, just conceptually overlapping validators on different representations.
...
### Multiple functions named to_json across 7 different files

**Category:** exact-name-uncovered

**Functions:**
- `to_json` in `ETAP-AI-WORK/acp_runtime/acp/observability/structured_logger.py:71` - method in observability
- `to_json` in `ETAP-AI-WORK/acp_runtime/acp/observability/tracer.py:101` - method in observability
- `to_json` in `ETAP-AI-WORK/acp_runtime/acp/security/audit.py:62` - method in security
- `to_json` in `ETAP-AI-WORK/ai_context_engine/knowledge_graph.py:222` - method in ai_context_engine
- `to_json` in `ETAP-AI-WORK/api/error_debugger.py:899` - method in api
- `to_json` in `ETAP-AI-WORK/digital_twin/state_store.py:226` - method in digital_twin
- `to_json` in `ETAP-AI-WORK/security/siem.py:103` - method in security

**Differences:** Same function name but different implementations across multiple modules - need manual verification

**Recommendation:** INVESTIGATE - Same-name functions in different modules may or may not be duplicates - verify each manually

---

### Multiple functions named reset across 5 different files

**Category:** exact-name-uncovered

**Functions:**
- `reset` in `ETAP-AI-WORK/acp_runtime/acp/observability/metrics.py:100` - method in observability
- `reset` in `ETAP-AI-WORK/acp_runtime/acp/observability/metrics.py:179` - method in observability
- `reset` in `ETAP-AI-WORK/acp_runtime/acp/observability/metrics.py:266` - method in observability
- `reset` in `ETAP-AI-WORK/api/_rate_limit.py:55` - method in api
- `reset` in `ETAP-AI-WORK/digital_twin/event_bus.py:479` - method in digital_twin
- `reset` in `ETAP-AI-WORK/engine/numerical_safety.py:267` - method in engine
- `reset` in `ETAP-AI-WORK/engine/resilience.py:381` - method in engine
...
### Multiple functions named __post_init__ across 4 different files

**Category:** exact-name-uncovered

**Functions:**
- `__post_init__` in `ETAP-AI-WORK/core/models.py:267` - method in core
- `__post_init__` in `ETAP-AI-WORK/core/models.py:307` - method in core
- `__post_init__` in `ETAP-AI-WORK/core_model/motor_model.py:44` - method in core_model
- `__post_init__` in `ETAP-AI-WORK/core_model/zip_load.py:38` - method in core_model
- `__post_init__` in `ETAP-AI-WORK/digital_twin/event_bus.py:98` - method in digital_twin
- `__post_init__` in `ETAP-AI-WORK/digital_twin/event_bus.py:113` - method in digital_twin
- `__post_init__` in `ETAP-AI-WORK/digital_twin/event_bus.py:128` - method in digital_twin
- `__post_init__` in `ETAP-AI-WORK/digital_twin/event_bus.py:142` - method in digital_twin
- `__post_init__` in `ETAP-AI-WORK/digital_twin/event_bus.py:157` - method in digital_twin
- `__post_init__` in `ETAP-AI-WORK/digital_twin/event_bus.py:172` - method in digital_twin
...
### Multiple functions named read_message across 4 different files

**Category:** exact-name-uncovered

**Functions:**
- `read_message` in `ETAP-AI-WORK/acp_runtime/acp/transport/base.py:29` - method in transport
- `read_message` in `ETAP-AI-WORK/acp_runtime/acp/transport/stdio.py:40` - method in transport
- `read_message` in `ETAP-AI-WORK/acp_runtime/acp/transport/uds.py:68` - method in transport
- `read_message` in `ETAP-AI-WORK/acp_runtime/acp/transport/websocket.py:52` - method in transport
...
### Multiple functions named write_message across 4 different files

**Category:** exact-name-uncovered

**Functions:**
- `write_message` in `ETAP-AI-WORK/acp_runtime/acp/transport/base.py:38` - method in transport
- `write_message` in `ETAP-AI-WORK/acp_runtime/acp/transport/stdio.py:55` - method in transport
- `write_message` in `ETAP-AI-WORK/acp_runtime/acp/transport/uds.py:79` - method in transport
- `write_message` in `ETAP-AI-WORK/acp_runtime/acp/transport/websocket.py:61` - method in transport
...
### Multiple functions named execute_study across 3 different files

**Category:** exact-name-uncovered

**Functions:**
- `execute_study` in `ETAP-AI-WORK/etap_integration/etap_adapter.py:55` - method in etap_integration
- `execute_study` in `ETAP-AI-WORK/etap_integration/etap_adapter.py:97` - method in etap_integration
- `execute_study` in `ETAP-AI-WORK/etap_integration/etap_adapter.py:137` - method in etap_integration
- `execute_study` in `ETAP-AI-WORK/etap_integration/etap_provider.py:78` - method in etap_integration
- `execute_study` in `ETAP-AI-WORK/etap_integration/etap_provider.py:120` - method in etap_integration
- `execute_study` in `ETAP-AI-WORK/etap_integration/etap_provider.py:196` - method in etap_integration
- `execute_study` in `ETAP-AI-WORK/etap_integration/etap_provider.py:431` - method in etap_integration
- `execute_study` in `ETAP-AI-WORK/etap_integration/etap_provider.py:489` - method in etap_integration
- `execute_study` in `ETAP-AI-WORK/etap_integration/etap_worker_service.py:103` - module in etap_integration
...
### Multiple functions named info across 3 different files

**Category:** exact-name-uncovered

**Functions:**
- `info` in `ETAP-AI-WORK/acp_runtime/acp/observability/structured_logger.py:121` - method in observability
- `info` in `ETAP-AI-WORK/core/bootstrap.py:252` - method in core
- `info` in `ETAP-AI-WORK/scripts/verify_services.py:53` - module in scripts
...
### Multiple functions named flush across 3 different files

**Category:** exact-name-uncovered

**Functions:**
- `flush` in `ETAP-AI-WORK/acp_runtime/acp/observability/structured_logger.py:147` - method in observability
- `flush` in `ETAP-AI-WORK/integrations/langfuse_integration.py:315` - method in integrations
- `flush` in `ETAP-AI-WORK/security/siem.py:367` - method in security
...
### Multiple functions named dispatch across 3 different files

**Category:** exact-name-uncovered

**Functions:**
- `dispatch` in `ETAP-AI-WORK/api/csrf.py:161` - method in api
- `dispatch` in `ETAP-AI-WORK/api/routes.py:153` - method in api
- `dispatch` in `ETAP-AI-WORK/integrations/langfuse_middleware.py:121` - method in integrations
...
### Multiple functions named connect across 3 different files

**Category:** exact-name-uncovered

**Functions:**
- `connect` in `ETAP-AI-WORK/api/cua_confirmation_ws.py:121` - method in api
- `connect` in `ETAP-AI-WORK/api/notifications.py:182` - method in api
- `connect` in `ETAP-AI-WORK/api/websocket.py:31` - method in api
...
### Multiple functions named disconnect across 3 different files

**Category:** exact-name-uncovered

**Functions:**
- `disconnect` in `ETAP-AI-WORK/api/cua_confirmation_ws.py:134` - method in api
- `disconnect` in `ETAP-AI-WORK/api/notifications.py:189` - method in api
- `disconnect` in `ETAP-AI-WORK/api/websocket.py:45` - method in api
...
### Multiple functions named run_study across 3 different files

**Category:** exact-name-uncovered

**Functions:**
- `run_study` in `ETAP-AI-WORK/api/studies.py:317` - module in api
- `run_study` in `ETAP-AI-WORK/engine/engine.py:314` - method in engine
- `run_study` in `ETAP-AI-WORK/etap_integration/etap_com.py:261` - method in etap_integration
...
### Multiple functions named shutdown across 3 different files

**Category:** exact-name-uncovered

**Functions:**
- `shutdown` in `ETAP-AI-WORK/engine/async_executor.py:340` - method in engine
- `shutdown` in `ETAP-AI-WORK/etap_integration/etap_com.py:1471` - method in etap_integration
- `shutdown` in `ETAP-AI-WORK/integrations/langfuse_integration.py:324` - method in integrations
...
### Multiple functions named scan across 3 different files

**Category:** exact-name-uncovered

**Functions:**
- `scan` in `ETAP-AI-WORK/guards/base.py:154` - method in guards
- `scan` in `ETAP-AI-WORK/guards/code_guard.py:46` - method in guards
- `scan` in `ETAP-AI-WORK/guards/docs_guard.py:42` - method in guards
...
### Return health/availability status of a subsystem (database, Redis, WebSocket manager, API store)

**Category:** http-api-database

**Functions:**
- `health_check` in `ETAP-AI-WORK/api/cua_confirmation_ws.py:315` - Returns dict with WebSocket manager stats: active connections, confirmation state. Specific to CUA WebSocket subsystem.
- `health_check` in `ETAP-AI-WORK/services/api_key_store.py:374` - Returns dict with API key storage backend health: db_accessible, key_count. Specific to API key SQLite store.
- `check_db_health` in `ETAP-AI-WORK/api/database.py:229` - Pings the PostgreSQL/SQLite database and returns {status: ok/error, details}. General DB health check.
- `etap_gui_health` in `ETAP-AI-WORK/api/agents.py:426` - Returns GUI system health status with API key authentication. Broader system health check.
- `etap_gui_safety_health` in `ETAP-AI-WORK/api/agents.py:531` - Returns safety system health status. Domain-specific subsystem health.
- `etap_gui_siem_health` in `ETAP-AI-WORK/api/agents.py:580` - Returns SIEM system health status. Domain-specific subsystem health.

**Differences:** All return a dict with a health/status indicator. The DB/Redis health checks test actual connectivity (ping, query). The WebSocket and API key store checks return subsystem stats. The agents.py health checks are broader system-level checks with auth requirements. Different scopes but same structural pattern: {status: ok/unhealthy, ...details}.

**Recommendation:** INVESTIGATE - The health-check pattern is intentionally duplicated per subsystem — each module owns its own health reporting, which is good microservice design. However, the response format should be standardized (consistent key names: status, healthy, details, latency_ms). A shared HealthCheckResponse schema/model could enforce consistency without consolidating the actual check logic. The three agents.py health endpoints (gui, safety, siem) could potentially be unified into a single /health endpoint that returns all subsystem statuses.

---

### Persist and restore state snapshots to/from a storage backend (Redis or in-memory store)

**Category:** http-api-database

**Functions:**
- `save` in `ETAP-AI-WORK/core/redis_state.py:130` - Persist circuit breaker state to Redis with best-effort tolerance. JSON-serializes state dict → Redis SET.
- `restore` in `ETAP-AI-WORK/core/redis_state.py:149` - Restore circuit breaker state from Redis with fallback on failure. Redis GET → JSON-deserialize → apply defaults.
- `save_workflow_state` in `ETAP-AI-WORK/core/redis_state.py:274` - Persist workflow execution state to Redis keyed by task ID. Same save pattern for different state type.
- `load_workflow_state` in `ETAP-AI-WORK/core/redis_state.py:308` - Load workflow execution state from Redis by task ID. Same restore pattern for different state type.
- `delete_workflow_state` in `ETAP-AI-WORK/core/redis_state.py:333` - Delete workflow execution state from Redis by task ID. CRUD delete operation.
- `commit_snapshot` in `ETAP-AI-WORK/digital_twin/digital_twin_core.py:188` - Persist a state snapshot to the digital twin in-memory state store. Same save-snapshot pattern, different backend.
- `get_current_snapshot` in `ETAP-AI-WORK/digital_twin/digital_twin_core.py:201` - Retrieve the most recently committed state snapshot. Same restore-latest pattern, different backend.

**Differences:** redis_state uses Redis as backend with best-effort tolerance (silently handles failures). digital_twin uses an in-memory dict store. The save/restore pattern is structurally identical: serialize state → store with key → retrieve by key → deserialize. redis_state has three state types (circuit breaker, workflow, workflow delete) while digital_twin has one (twin snapshot).

**Recommendation:** INVESTIGATE - The save/restore pattern is duplicated across two storage backends and multiple state types. A generic StateStore abstraction (save(key, state), restore(key, default), delete(key)) could be implemented for both Redis and in-memory backends, with each domain (circuit breaker, workflow, digital twin) using the same interface. However, the best-effort tolerance in redis_state (silently ignoring Redis failures) is intentional for resilience and must be preserved in any abstraction.

---

### Route an incoming chat/execution request to the appropriate agent by matching query to agent capabilities

**Category:** http-api-database

**Functions:**
- `etap_expert_chat` in `ETAP-AI-WORK/api/agents.py:224` - Handle expert chat request: validate input → match query to agent → call agent.chat(). Returns ChatResponse.
- `etap_gui_chat` in `ETAP-AI-WORK/api/agents.py:283` - Handle GUI chat request: validate input → match query to agent → call agent.chat(). Returns ChatResponse. Same flow as expert_chat.
- `etap_gui_execute` in `ETAP-AI-WORK/api/agents.py:359` - Execute GUI command: validate input → match to agent → call agent.execute(). Returns ExecuteResponse. Same routing logic, different agent method.

**Differences:** etap_expert_chat and etap_gui_chat follow the identical flow: parse request → determine target agent → call agent.chat() → return ChatResponse. etap_gui_execute calls agent.execute() instead of chat(). The routing/matching logic is the same across all three. Minor differences in request validation and response formatting.

**Recommendation:** INVESTIGATE - expert_chat and gui_chat have nearly identical implementation — the routing logic could be extracted into a shared _route_to_agent(request, method='chat') helper. gui_execute differs only in calling execute() instead of chat(). A single routing function that accepts the agent method name as a parameter would eliminate the duplicated validation and matching logic. However, the different response types (ChatResponse vs ExecuteResponse) and different auth/validation requirements need careful handling.

---

### Get or create a database connection/engine and provide it to callers (thread-local or async session)

**Category:** http-api-database

**Functions:**
- `_get_conn` in `ETAP-AI-WORK/core/database.py:48` - Get or create a thread-local SQLite connection. Caches in threading.local(). Raw SQLite connection pattern.
- `get_db` in `ETAP-AI-WORK/api/database.py:207` - FastAPI dependency that yields an async SQLAlchemy session from the session factory. Standard FastAPI get_db pattern.
- `_build_postgres_engine` in `ETAP-AI-WORK/api/database.py:103` - Create async SQLAlchemy PostgreSQL engine with pool defaults. Engine factory for PostgreSQL.
- `_build_sqlite_engine` in `ETAP-AI-WORK/api/database.py:125` - Create async SQLAlchemy SQLite engine without pooling. Engine factory for SQLite.

**Differences:** _get_conn uses raw SQLite with thread-local caching (synchronous). get_db uses async SQLAlchemy session factory (async). _build_postgres_engine and _build_sqlite_engine are engine factories for different DB types. The core pattern is 'obtain a DB connection/session', but the implementations are fundamentally different due to sync vs async and raw vs ORM.

**Recommendation:** KEEP_SEPARATE - The two database access patterns serve different architectural layers: core/database._get_conn is for the legacy synchronous BIM/element store; api/database.get_db is the modern async SQLAlchemy FastAPI dependency. Merging them would require converting the entire BIM store to async SQLAlchemy, which is a major refactoring effort with high risk. The engine factories (_build_postgres_engine, _build_sqlite_engine) are necessarily different per DB dialect. Keeping separate is appropriate.
...
### Keyboard shortcut handling — register global keydown listeners, map key combos to actions (navigation, help, shortcuts panel)

**Category:** ui-helpers

**Functions:**
- `KeyboardShortcutsHandler` in `ETAP-AI-WORK/ui/src/App.tsx:57` - Component wrapper that calls useKeyboardShortcuts() and returns null. Exists solely to mount the hook inside the React tree.
- `useKeyboardShortcuts` in `ETAP-AI-WORK/ui/src/hooks/useKeyboardShortcuts.ts:20` - The actual hook: registers keydown listener, handles all shortcut combos, manages shortcutsPanelOpen state. 215 lines.

**Differences:** KeyboardShortcutsHandler is a 3-line component whose only job is to call useKeyboardShortcuts() and return null. It adds no value over calling the hook directly in the App component.

**Recommendation:** REMOVE_WRAPPER - KeyboardShortcutsHandler is an unnecessary wrapper — a component that renders null and delegates entirely to a hook. Call useKeyboardShortcuts() directly inside App's render body and delete KeyboardShortcutsHandler. This is a minor cleanup, not a deep semantic duplicate.

---

### Status-color mapping helper — take a state/type/category and return a CSS color class string for visual status indication

**Category:** ui-helpers

**Functions:**
- `getVariantColor` in `ETAP-AI-WORK/ui/src/pages/AssetManagement.tsx:100` - Maps variant type (success/warning/danger/default) → Tailwind text color class. Used for asset status display.
- `getTerminalLogColor` in `ETAP-AI-WORK/ui/src/pages/Login.tsx:50` - Maps log content (WARNING/TRIP/SUCCESS/converged/SEC-AUTH) → Tailwind text color class. Used for terminal log styling.
- `providerCardClass` in `ETAP-AI-WORK/ui/src/pages/Settings.tsx:48` - Maps provider key state (hasKey/isFree) → Tailwind border/background class for card styling.
- `providerButtonClass` in `ETAP-AI-WORK/ui/src/pages/Settings.tsx:55` - Maps button state (hasKey/isTesting/status) → Tailwind button styling class.

**Differences:** Each function maps a domain-specific state to a CSS class, but they all follow the same pattern: conditional logic → Tailwind color/utility class string. getVariantColor maps semantic severity; getTerminalLogColor maps log content keywords; providerCardClass/providerButtonClass map key+status combinations. The actual mappings are domain-specific and wouldn't collapse into a single function, but the pattern itself (state→class) is repeated.

**Recommendation:** EXTRACT_PATTERN - These are domain-specific and can't fully merge, but the pattern can be unified: create a statusColorMap utility in helpers.ts that provides common semantic-to-color mappings (success→green, warning→amber, danger→red, default→muted), then let each page extend or customize it for domain specifics. getVariantColor becomes a trivial call to statusColorMap. getTerminalLogColor can map keywords to semantic statuses first, then use statusColorMap.

---

### One-line diagram SVG — render a power system one-line diagram visualization with buses, branches, and equipment symbols

**Category:** ui-helpers

**Functions:**
- `DigitalTwinDiagram` in `ETAP-AI-WORK/ui/src/pages/DigitalTwin.tsx:45` - SVG one-line diagram for the Digital Twin page: buses (115kV, 13.8kV), transformer, generator, circuit breaker, feeders. Interactive with simulation data overlay.
- `OneLineDiagram` in `ETAP-AI-WORK/ui/src/pages/StudyRun.tsx:35` - SVG one-line diagram for the Study Run page: buses, branches, voltage annotations. Used for study result visualization.

**Differences:** Both render SVG power-system one-line diagrams with bus bars, transformer symbols, and branch lines. DigitalTwinDiagram is interactive (simulation status overlay, editable). OneLineDiagram is simpler (static display with voltage annotations). Both use similar SVG coordinate systems and component symbols (bus bar lines, transformer circles).

**Recommendation:** CONSOLIDATE - Two SVG one-line diagrams with overlapping visual elements. Create a shared OneLineDiagram component with props for: data (buses, branches, equipment), interactive mode, annotations. DigitalTwinDiagram and StudyRun's OneLineDiagram both compose this shared component with their page-specific overlays. Reduces ~200 lines of duplicated SVG markup.
...
### Serialize dataclass/data objects to dict for JSON/API output

**Category:** agent

**Functions:**
- `to_dict` in `ETAP-AI-WORK/agents/cua_executor.py:189`
- `to_dict` in `ETAP-AI-WORK/agents/life_safety.py:294`
- `to_dict` in `ETAP-AI-WORK/agents/scada_agent.py:157`

**Notes:** All three to_dict methods serialize different data structures with different fields. life_safety uses asdict() (automatic), while cua_executor and scada_agent manually construct dicts. The cua_executor version has complex nested serialization (steps→to_audit_dict). The scada_agent version does datetime→isoformat() conversion. The fields are entirely different per class, so these are not semantic duplicates — just the same method name pattern on different data objects.

---

### Return health/status information for /health endpoints

**Category:** agent

**Functions:**
- `health_check` in `ETAP-AI-WORK/agents/life_safety.py:937`
- `health_check` in `ETAP-AI-WORK/copilot/mcp/server.py:663`

**Notes:** Both return dicts for /health endpoints but the content is entirely domain-specific: life_safety returns safety/guard status while server.py returns CAD/ETAP integration connectivity. No overlap in fields or logic.
...
### Send a JSON response with a status code and body (HTTP response helpers)

**Category:** http-api-database

**Functions:**
- `_send_json` in `ETAP-AI-WORK/api/cron/digest.py:69`
- `health_check` in `ETAP-AI-WORK/api/cua_confirmation_ws.py:315`

**Notes:** _send_json manually writes headers and body for BaseHTTPRequestHandler (Vercel Cron style). FastAPI endpoints use automatic JSON response serialization. These are fundamentally different HTTP frameworks with different response mechanisms.
