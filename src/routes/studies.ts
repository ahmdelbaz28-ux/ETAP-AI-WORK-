...
const VALID_STUDY_TYPES = [
  'load_flow', 'short_circuit', 'fault', 'arc_flash', 'harmonic_analysis',
  'optimal_power_flow', 'protection_coordination', 'coordination', 'motor_starting',
  'etap_load_flow', 'etap_short_circuit', 'etap_arc_flash', 'etap_harmonic_analysis',
  'etap_optimal_power_flow', 'etap_motor_starting', 'etap_protection_coordination',
];
...
export async function handleStudyRun(
  ...
  task.status = 'running'; task.startedAt = Date.now(); await setTask(env, taskId, task);
  const svcRequest: EngineeringServiceRequest = { study_type: studyType, parameters, task_id: taskId, use_etap: body.useEtap === true, etap_project_path: body.etapProjectPath || undefined };
  if (body.system) svcRequest.system = body.system;
