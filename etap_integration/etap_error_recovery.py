...
try:
    from engine.resilience import CircuitBreaker, CircuitBreakerOpenError, RetryHandler
...
class ETAPErrorRecovery:
    ...
    def recover_from_com_error(self, error: Exception) -> RecoveryAttempt:
        ...
        if self._com_breaker is not None:
            try:
                ...
                logger.warning("Circuit breaker probe raised %s: %s", type(cb_err).__name__, cb_err)
    ...
    def get_error_diagnosis(self, error: Exception) -> ErrorDiagnosis:
        ...
        if isinstance(error, pythoncom.com_error) if "pythoncom" in sys.modules else False:
            return ErrorDiagnosis(
                ErrorCategory.COM_CONNECTION_LOST,
                "ETAP COM interface disconnected or process crashed.",
                "Restart ETAP and re-establish the COM connection.",
                True,
                True,
            )
        ...
        if any(
            k in msg
            for k in ["com_error", "rpc", "server", "disconnected", "0x800706ba", "0x80010108"]
        ):
            return ErrorDiagnosis(
                ErrorCategory.COM_CONNECTION_LOST,
                "ETAP COM interface disconnected or process crashed.",
        ...
        if any(k in msg for k in ["study", "calculation", "converge", "diverg", "iteration"]):
            return ErrorDiagnosis(
                ErrorCategory.STUDY_EXECUTION_FAILED,
                f"Study execution failed: {error}",
                "Verify study parameters and project configuration, then retry.",
                True,
            )

        if any(
            k in msg for k in ["file", "project", "open", "save", "edb", "permission", "access"]
        ):
            return ErrorDiagnosis(
                ErrorCategory.PROJECT_IO_ERROR,
                f"Project I/O error: {error}",
                "Check file permissions, path validity, and disk space.",
                False,
            )
        ...
        if any(k in msg for k in ["license", "activation", "expir", "dongle"]):
            return ErrorDiagnosis(
                ...
                "Verify a valid ETAP license is installed and activated.",
                False,
            )
        ...
        if any(k in msg for k in ["version", "compatible", "incompatible", "not supported"]):
            return ErrorDiagnosis(
                ...
                "Install a supported ETAP version (12.0 or later).",
                False,
            )

        if any(
            k in msg for k in ["parameter", "invalid", "out of range", "validation", "argument"]
        ):
            return ErrorDiagnosis(
                ErrorCategory.INVALID_PARAMETER,
                f"Invalid parameter: {error}",
                "Review parameter values against ETAP requirements.",
                True,
            )
    ...
    def _kill_etap_processes(  # S3776 cognitive complexity intentional; logic validated by tests
        self,
        ...
        if PSUTIL_AVAILABLE:
            ...
            try:
                r = subprocess.run(
                    ["taskkill", "/F", "/IM", ETAP_PROCESS_NAME],
                    capture_output=True,
                    text=True,
                    timeout=10,
                )
