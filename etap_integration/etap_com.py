...
STUDY_TYPE_PARAMETER_SCHEMAS: Dict[ETAPStudyType, Dict[str, Dict[str, Any]]] = {
    ETAPStudyType.LOAD_FLOW: {
        "method": {"type": "string", "allowed": ["newton_raphson", "fast_decoupled", "gauss_seidel"],
                   "required": False},
    ...
    ETAPStudyType.OPTIMAL_POWER_FLOW: {
        ...
        "objective": {"type": "string", "allowed": ["min_cost", "min_losses", "min_violations"],
                      "required": False},
        "include_reactive": {"type": "boolean", "required": False},
    },
    ETAPStudyType.MOTOR_STARTING: {
        "starting_method": {"type": "string",
                           "allowed": ["across_the_line", "soft_starter", "vfd", "autotransformer"],
                           "required": False},
        "voltage_dip_limit_percent": {"type": "numeric", "min": 0.0, "max": 100.0,
                                      "required": False},
    },
    ETAPStudyType.MOTOR_ACCELERATION: {
        "acceleration_method": {"type": "string",
                                "allowed": ["full_voltage", "reduced_voltage"],
                                "required": False},
        "load_model": {"type": "string", "allowed": ["constant_torque", "quadratic", "linear"],
                       "required": False},
    },
    ETAPStudyType.PROTECTION_COORDINATION: {
        "curve_type": {"type": "string",
                       "allowed": ["standard_inverse", "very_inverse", "extremely_inverse",
                                   "long_time_inverse"],
                       "required": False},
    ...
    ETAPStudyType.TRANSIENT_STABILITY: {
        "simulation_duration_sec": {"type": "numeric", "min": 0.1, "max": 3600.0,
                                    "required": False},
    ...
    ETAPStudyType.CABLE_AMACITY: {
        "installation_method": {"type": "string",
                                ...
                                "required": False},
        "ambient_temperature_c": {"type": "numeric", "min": -40.0, "max": 80.0,
                                  "required": False},
        "voltage_kv": {"type": "numeric", "min": 0.1, "max": 500.0, "required": False},
    },
    ETAPStudyType.GROUND_GRID: {
        "soil_resistivity_ohm_m": {"type": "numeric", "min": 0.1, "max": 10000.0,
                                   "required": False},
        "surface_layer_thickness_m": {"type": "numeric", "min": 0.0, "max": 10.0,
                                      "required": False},
    },
...
class ETAPProject:
    ...
    def _run_arc_flash(self, **kwargs) -> Dict[str, Any]:
        ...
        working_distance = ETAPAutomation._validate_input(
            ...
            min_val=WORKING_DISTANCE_MIN, max_val=WORKING_DISTANCE_MAX
        )
        try:
    ...
    def _run_motor_starting(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            for motor in getattr(self._com_project, 'Motors', []):
                ...
                if motor_id:
                    motors[motor_id] = {
                        'starting_current_multiplier': float(getattr(motor, 'StartingCurrentMult', 0.0)),
    ...
    def save(self, file_path: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if path is not None and len(str(path)) > MAX_PROJECT_PATH_LENGTH:
                raise ValueError(f"File path length {len(str(path))} exceeds maximum {MAX_PROJECT_PATH_LENGTH}")
...
class ETAPAutomation:
    ...
    def _validate_input(value, value_type: str,
                        ...
                        max_length: Optional[int] = None) -> Union[int, float, str, bool]:
        """
    ...
    @staticmethod
    def _validate_study_parameters(study_type: ETAPStudyType,
        ...
        if not isinstance(params, dict):
            raise ValueError(
                f"Study parameters must be a dict, got {type(params).__name__}"
            )

        if not isinstance(study_type, ETAPStudyType):
            raise ValueError(
                f"study_type must be ETAPStudyType, got {type(study_type).__name__}"
            )
        ...
        for key in params:
            ...
            if len(key) > 256:
                raise ValueError(
                    f"Study parameter key too long ({len(key)} chars): {key[:50]}..."
                )
            ...
            if expected_type == "numeric":
                ...
                if min_val is not None and value < min_val:
                    raise ValueError(
                        f"Parameter '{key}' value {value} below minimum {min_val}"
                    )
                if max_val is not None and value > max_val:
                    raise ValueError(
                        f"Parameter '{key}' value {value} above maximum {max_val}"
                    )
    ...
    def _validate_bus_id(bus_id: str) -> str:
        ...
        if len(bus_id) > MAX_BUS_NAME_LENGTH:
            raise ValueError(
                f"Bus ID length {len(bus_id)} exceeds maximum {MAX_BUS_NAME_LENGTH}"
            )
        return bus_id

    @staticmethod
    def _check_result_size(result_dict: Dict[str, Any],
                           max_entries: int = MAX_RESULT_ENTRIES) -> Dict[str, Any]:
        """
        ...
        for key, value in result_dict.items():
            ...
            if total > max_entries:
                raise ValueError(
                    f"Result size ({total} entries) exceeds maximum ({max_entries})"
                )
        return result_dict
    ...
    def _validate_project_path(self, file_path: str) -> bool:
        ...
        if self._allowed_project_dirs:
            is_allowed = any(
                ...
                for allowed_dir in self._allowed_project_dirs
            )
            if not is_allowed:
    ...
    def open_project(self, file_path: str) -> Optional[ETAPProject]:
        ...
        try:
            ...
            if com_project:
                project = ETAPProject(com_project, file_path,
            ...
            logger.error(f"COM error opening project {file_path} (timeout={self.com_timeout_seconds}s): {e}")
            return None
        except Exception as e:
    ...
    def create_project(self, project_name: str = "NewProject") -> Optional[ETAPProject]:
        ...
        try:
            ...
            if com_project:
                ...
                project = ETAPProject(com_project, temp_path,
                                      com_timeout=self.com_timeout_seconds)
    ...
    def get_active_project(self) -> Optional[ETAPProject]:
        ...
        try:
            ...
            if com_project:
                ...
                project = ETAPProject(com_project, "ActiveProject",
                                      com_timeout=self.com_timeout_seconds)
                return project
        except pythoncom.com_error as e:
            logger.error(f"COM error getting active project (timeout={self.com_timeout_seconds}s): {e}")
        except Exception as e:
