...
def _compute_optical_density(
    ...
    """
    Delta_H_c = PHYSICAL_CONSTANTS[
        "EFFECTIVE_HEAT_OF_COMBUSTION_MJ_KG"
    ]  # 20 MJ/kg  # NOSONAR - python:S117
...
def estimate_co_concentration(
    ...
    CO_mass_concentration = total_CO_mass / max(
        effective_volume, 1e-6
    )  # kg/m³  # NOSONAR - python:S117
...
def calculate_aset(  # NOSONAR — S3776: cognitive complexity is inherent to the safety-critical algorithm
    ...
    layer_height_at_aset = (
        scenario.room_height_m
    )  # NOSONAR — S1854: assignment kept for debug / future use
