...
class TestRoutingConstraint:
    ...
    def test_default_values(self, default_constraints) -> None:
        ...
        assert (
            default_constraints.bend_radius_mm == 300.0
        )  # NOSONAR — S1244: import retained for re-export / API surface
        assert (
            default_constraints.max_cable_length_m == 300.0
        )  # NOSONAR — S1244: import retained for re-export / API surface
        assert (
            default_constraints.clearance_mm == 50.0
        )  # NOSONAR — S1244: import retained for re-export / API surface
        assert default_constraints.conduit_type == "EMT"
        assert (
            default_constraints.vertical_penalty == 1.5
        )  # NOSONAR — S1244: import retained for re-export / API surface
        assert (
            default_constraints.cross_corridor_penalty == 2.0
        )  # NOSONAR — S1244: import retained for re-export / API surface
        assert (
            default_constraints.seismic_joint_orthogonal_bonus == 0.5
        )  # NOSONAR — S1244: import retained for re-export / API surface

    def test_custom_values(self, custom_constraints) -> None:
        """Custom constraints override defaults."""
        assert (
            custom_constraints.bend_radius_mm == 200.0
        )  # NOSONAR — S1244: import retained for re-export / API surface
        assert (
            custom_constraints.max_cable_length_m == 50.0
        )  # NOSONAR — S1244: import retained for re-export / API surface
        assert (
            custom_constraints.clearance_mm == 100.0
        )  # NOSONAR — S1244: import retained for re-export / API surface
        assert custom_constraints.conduit_type == "RMC"
        assert (
            custom_constraints.vertical_penalty == 2.0
        )  # NOSONAR — S1244: import retained for re-export / API surface
        assert (
            custom_constraints.cross_corridor_penalty == 3.0
        )  # NOSONAR — S1244: import retained for re-export / API surface
        assert (
            custom_constraints.seismic_joint_orthogonal_bonus == 0.3
        )  # NOSONAR — S1244: import retained for re-export / API surface
    ...
    def test_from_production_config_without_config(self) -> None:
        ...
        assert (
            constraint.bend_radius_mm == 300.0
        )  # NOSONAR — S1244: import retained for re-export / API surface
        assert (
            constraint.max_cable_length_m == 300.0
        )  # NOSONAR — S1244: import retained for re-export / API surface
    ...
    def test_from_production_config_with_mock(self) -> None:
        ...
        try:
            ...
            constraint = RoutingConstraint.from_production_config()
            assert (
                constraint.bend_radius_mm == 250.0
            )  # NOSONAR — S1244: import retained for re-export / API surface
            assert (
                constraint.max_cable_length_m == 200.0
            )  # NOSONAR — S1244: import retained for re-export / API surface
            assert (
                constraint.clearance_mm == 75.0
            )  # NOSONAR — S1244: import retained for re-export / API surface
            assert constraint.conduit_type == "IMC"
            assert (
                constraint.vertical_penalty == 1.8
            )  # NOSONAR — S1244: import retained for re-export / API surface
            assert (
                constraint.cross_corridor_penalty == 2.5
            )  # NOSONAR — S1244: import retained for re-export / API surface
        finally:
...
class TestRoutingEngineV10Init:
    ...
    def test_custom_constraints(self, custom_constraints) -> None:
        ...
        assert (
            engine.constraints.max_cable_length_m == 50.0
        )  # NOSONAR — S1244: import retained for re-export / API surface
    ...
    def test_none_constraints_uses_defaults(self) -> None:
        ...
        assert isinstance(engine.constraints, RoutingConstraint)
        assert (
            engine.constraints.clearance_mm == 50.0
        )  # NOSONAR — S1244: import retained for re-export / API surface
...
class TestRoutingEngineV10Helpers:
    ...
    def test_get_clearance_m_default(self, engine) -> None:
        ...
        assert (
            engine._get_clearance_m(  # NOSONAR — S1244: import retained for re-export / API surface
                RoutingObstacle(obstacle_type="wall", x=0, y=0, width=1, height=1)
            )
            == 50.0
        )
    ...
    def test_get_clearance_m_override(self, engine) -> None:
        ...
        assert (
            engine._get_clearance_m(obs) == 200.0
        )  # NOSONAR — S1244: import retained for re-export / API surface
    ...
    def test_get_clearance_m_unknown_type(self, engine) -> None:
        ...
        assert (
            engine._get_clearance_m(obs) == 50.0
        )  # NOSONAR — S1244: import retained for re-export / API surface
...
class TestIntegration:
    ...
    def test_custom_clearance_obstacle(self) -> None:
        ...
        engine.add_obstacle(obs)
        assert (
            engine._get_clearance_m(obs) == 500.0
        )  # NOSONAR — S1244: import retained for re-export / API surface
