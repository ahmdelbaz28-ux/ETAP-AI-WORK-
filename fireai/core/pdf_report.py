...
def _base_table_style(
    _col_count: int,
) -> TableStyle:  # NOSONAR — S1172: parameter retained for API stability
    return TableStyle(
        [
...
def generate_building_report(
    ...
    try:
        ...
        if not is_safe:
            _logger.error(
                "Path traversal / invalid output path detected in generate_pdf_report: %s",
                output_path,
            )
