...
calculations. All responses include a ``source: "zenmux"`` field and a
``disclaimer`` reminding the engineer that AI output must be verified against
the published code.
"""
    ...
    if not svc.available:
        raise HTTPException(
            ...
            detail={
                ...
                "message": "ZENMUX_API_KEY is not configured.",
            },
        )
    try:
        ...
        raise HTTPException(
            ...
            detail={
                ...
                "message": "The LLM provider returned an error. Check server logs.",
            },
        )

    return {
        "success": True,
        "data": LLMResponseModel(
...
async def llm_explain(request: Request, req: ExplainRequest) -> Dict[str, Any]:
    ...
    svc = get_llm_service()
    if not svc.available:
        raise HTTPException(503, detail="LLM service not configured")
    ...
    try:
        ...
        raise HTTPException(502, detail="LLM request failed")
...
async def llm_models(request: Request) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(502, detail="Failed to list models from provider")
