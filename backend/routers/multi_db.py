...
async def set_in_redis(
    key: str, value: str, ttl: Optional[int] = Query(None, description="Time to live in seconds")
...
async def find_similar_elements(
    query_embedding: List[float], limit: int = Query(5, ge=1, le=20)
):  # NOSONAR - python:S8410
...
async def create_element_relationships(
    ...
    relationship_type: str = Query(
        "CONNECTED_TO", description="Type of relationship"
    ),  # NOSONAR - python:S8410
):
...
async def execute_neo4j_query(
    query: str, parameters: Optional[str] = Query(None)
):  # NOSONAR - python:S8410
