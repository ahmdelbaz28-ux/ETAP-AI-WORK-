...
On first startup, creates an admin key from API_KEY env var.
"""
...
KEYS_FILE = os.getenv("API_KEYS_FILE", "db/api_keys.json")
...
def _get_keys_file_path() -> str:
    ...
    Reads API_KEYS_FILE at call time to support runtime configuration
    ...
    """
    return os.getenv("API_KEYS_FILE", KEYS_FILE)
...
def _get_server_secret_file_path() -> str:
    ...
    Reads API_KEYS_SECRET_FILE at call time to support runtime
    ...
    """
    return os.getenv("API_KEYS_SECRET_FILE", _SERVER_SECRET_FILE)
...
_VALIDATED_KEY_CACHE_TTL = float(os.getenv("KEY_CACHE_TTL", "300"))
...
def _ensure_default_admin_key() -> None:
    ...
    env_key = os.getenv("API_KEY")
    if env_key:
        add_api_key(env_key, Role.ADMIN, "Default admin key (from API_KEY)")
        logger.info("Created default admin API key from API_KEY env var")
