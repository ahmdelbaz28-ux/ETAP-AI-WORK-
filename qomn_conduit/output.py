...
def _build_summary(run: ConduitRun) -> dict[str, Any]:
    ...
    elbow_count = sum(
        1 for f in run.fittings if f.fitting_type in (FittingType.ELBOW_90, FittingType.ELBOW_45)
    )
...
def _dxf_color(ct: ConduitType) -> int:
    ...
    return {
        ...
        ct, 7
    )  # 7 = white (default)  # NOSONAR — S125: commented-out code kept for historical reference
