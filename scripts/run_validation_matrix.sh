...
# === HELPER FUNCTIONS ===
log_status() {
    echo "[*] $1"  # NOSONAR — S7679: shell acceptable
}
...
if [ "${RUFF_EXIT}" -eq 0 ] && [ "${PYTEST_EXIT}" -eq 0 ] && [ "${PIP_AUDIT_EXIT}" -eq 0 ]; then  # NOSONAR — S7688: shell acceptable
...
if [ "${RUFF_EXIT}" -ne 0 ] || [ "${PYTEST_EXIT}" -ne 0 ] || [ "${PIP_AUDIT_EXIT}" -ne 0 ]; then  # NOSONAR — S7688: shell acceptable
    echo "[-] FAILURE: Gate failures detected:"
    [ "${RUFF_EXIT}" -ne 0 ] && echo "  - Ruff (lint/format): FAILED (exit ${RUFF_EXIT})"  # NOSONAR — S7688: shell acceptable
    [ "${PYTEST_EXIT}" -ne 0 ] && echo "  - Pytest (test suite): FAILED (exit ${PYTEST_EXIT})"  # NOSONAR — S7688: shell acceptable
    [ "${PIP_AUDIT_EXIT}" -ne 0 ] && echo "  - Pip-audit (dependencies): FAILED (exit ${PIP_AUDIT_EXIT})"  # NOSONAR — S7688: shell acceptable
    exit 1
fi
exit 0
