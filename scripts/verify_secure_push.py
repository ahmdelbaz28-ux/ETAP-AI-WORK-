        ...
        for line in content.split('\n'):
            ...
            if s.startswith('cryptography'):
                ...
                if m and int(m.group(1)) >= 50:
                    ...
                    check(f"cryptography >= 50.0.0 in {req_file}", False,
                          f"found: {s}", blocking=True)
                break
...
if os.path.exists(pyproject):
    ...
    if chromadb_match:
        ...
        check("chromadb pinned < 1.0.0 (CVE-2026-45829 CRITICAL)", has_upper,
              f"spec: chromadb{spec}", blocking=True)
    else:
...
if os.path.exists(uv_lock):
    ...
    if m:
        ...
        check("uv.lock: chromadb < 1.0.0", major < 1,
              f"found version {ver}", blocking=True)
    ...
    for pkg_name, (expected_ver, cve_ref) in expected_overrides.items():
        ...
        if actual != 'NOT SET':
            ...
            if m and me:
                ...
                check(f"override {pkg_name} >= {expected_ver} ({cve_ref})",
                      ok, f"found {actual}", blocking=not ok)
            else:
                check(f"override {pkg_name} >= {expected_ver}", True)
        else:
            check(f"override {pkg_name} >= {expected_ver} ({cve_ref})",
                  False, "not set in overrides", blocking=True)
...
if os.path.exists(pnpm_lock):
    ...
    vuln_undici = re.findall(r'undici@(7\.(?:2[0-7]|[01]\d)\.\d+|6\.(?:2[0-6]|[01]\d)\.\d+)', content)
    check("pnpm-lock.yaml: no vulnerable undici 7.x < 7.29.0",
          not vuln_undici, f"found: {vuln_undici[:5]}", blocking=True)
...
is_main = branch == 'main'
check("NOT on main branch (feature branch required)", not is_main,
...
check("No uncommitted content changes", len(real_changes) == 0,
