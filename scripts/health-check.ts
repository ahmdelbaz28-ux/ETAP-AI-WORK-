...
`);
  ...
  if (jsonOutput) {
    ...
    const safeJson = JSON.stringify(report, (key, value) => {
      if (typeof value === 'string') {
        // Remove ANSI escapes + all C0/C1 control chars (log-injection guard).
    ...
    console.log(safeJson);  // NOSONAR — S5145: sanitised above
  } else {
    printReport(report);
  }
