...
`);
  ...
  if (jsonOutput) {
    ...
    // derived from API responses (e.g. error messages). Sanitise by
    // stripping ANSI escape sequences and control chars before logging.
    const safeJson = JSON.stringify(report, (key, value) => {
      if (typeof value === 'string') {
        // Remove ANSI escapes + all C0/C1 control chars (log-injection guard).
        return value.replace(/\x1b\[[0-9;]*[A-Za-z]/g, '').replace(/[\x00-\x1f\x7f-\x9f]/g, '');
      }
      return value;
    }, 2);
