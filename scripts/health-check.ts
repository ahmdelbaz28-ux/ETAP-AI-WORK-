  ...
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: {
        ...
        ...(headers ?? {}),  // NOSONAR — S7744: spread is safe even with empty object
...
`);
  ...
  if (jsonOutput) {
    // NOSONAR — tssecurity:S5145: report is generated by this script from
    // internal health-check results, not from user input. JSON.stringify
    // also escapes any control chars that could be used for log injection.
    console.log(JSON.stringify(report, null, 2));  // NOSONAR — S5145: report is internally generated, not user input
  } else {
    printReport(report);
  }
