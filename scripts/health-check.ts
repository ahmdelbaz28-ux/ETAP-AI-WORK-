 ...
 *
 * SonarCloud tssecurity:S5145: every string field is run through a replacer
 * that strips:
 *   1. ANSI escape sequences (e.g. `\x1b[2J` clears the terminal screen,
 *      `\x1b[8;40;41m` changes background colour — both can be injected by
 *      an attacker via API response strings echoed into the report).
 *   2. C0 control chars (U+0000–U+001F) and DEL (U+007F) — can forge fake
 *      log lines when a naive log processor splits on `\n` without first
 *      parsing the JSON structure.
 *   3. C1 control chars (U+0080–U+009F) — same risk in some legacy
 *      terminal emulators.
 *
 * Both `console.log` output AND the `health-check-report.json` CI artifact
 * must use this helper so a log-injection attempt cannot bypass the
 * console guard by going through the file artifact instead.
 *
 * @param report — the HealthReport object to serialise
 * @returns a 2-space-indented JSON string with all string fields sanitised
 */
function stringifyReportSanitised(report: HealthReport): string {
  return JSON.stringify(report, (_key, value) => {
    if (typeof value === 'string') {
      // eslint-disable-next-line no-control-regex
      return value
        // Strip ANSI CSI sequences: ESC [ ... <final letter>
        .replace(/\x1b\[[0-9;]*[A-Za-z]/g, '')
        // Strip remaining C0 (U+0000–U+001F) + DEL (U+007F) + C1 (U+0080–U+009F)
        // eslint-disable-next-line no-control-regex
...
`);
  ...
  if (jsonOutput) {
    ...
    // stripping ANSI escape sequences and C0/C1 control chars before
    // logging, so even if an attacker injected `\x1b[2J` (clear screen)
    // or `\n` (newline forgery) into an API response, it cannot affect
    // log consumers downstream.
    const safeJson = stringifyReportSanitised(report);
    // NOSONAR — tssecurity:S5145: safeJson is fully sanitised above (every
    // string field has ANSI escapes + control chars stripped). The original
    // `report` object's data was also generated by this script's own check
