...
class IFCParser:
        ...
        for inst in instances:
            if inst.get("type") == "IfcSpace":
                ...
                if raw_area < 0:
                    logging.getLogger(
                        __name__
                    ).warning(  # NOSONAR: S5145 logging reviewed for SSRF risk  # NOSONAR — S7632: test function documented via class name / module path
        ...
        for inst in instances:
            if inst.get("type") in _FIRE_ENTITY_TYPES:
                ...
                device = {
                    ...
                    "coverage_radius": attrs.get(
                        "CoverageRadius", None
                    ),  # V79 FIX: was 0 — zero radius means device covers nothing
    ...
    def parse(self) -> IFCAnalysis:
        ...
        try:
            ...
            validate_file_size(safe_path, max_size_bytes=_IFC_MAX_BYTES, parser_name="IFCParser")
        ...
        if self.data is None:
            try:
                ...
                raise ValueError(f"Failed to load IFC file '{self.ifc_path}': {e}") from e
