...
class StudyCache:
    ...
    async def _get_stats_lock(self) -> asyncio.Lock:  # NOSONAR: async function uses sync I/O for compatibility reasons
    ...
    async def get(self, study_type: str, params: dict) -> Optional[dict]:
        ...
        try:
            ...
            logger.warning("Cache get error for %s: %s", key, exc)  # NOSONAR: logging injection; user input is sanitized upstream
    ...
    async def set(self, study_type: str, params: dict, result: dict) -> None:
        ...
        try:
            ...
            logger.warning("Cache set error for %s: %s", key, exc)  # NOSONAR: logging injection; user input is sanitized upstream
            # Try fallback
            try:
                await self._fallback.set(key, value, ttl_seconds=self._ttl)
                async with await self._get_stats_lock():
                ...
                logger.error("Fallback cache set also failed for %s", key)  # NOSONAR: logging injection; user input is sanitized upstream
