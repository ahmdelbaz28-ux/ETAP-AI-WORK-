...
class SparseMatrixManager:
    ...
    def estimate_memory_savings(self, dense_size: int, sparse_size: int) -> Dict[str, Any]:
        if dense_size == 0:
            return {'dense_bytes': 0, 'sparse_bytes': 0, 'ratio': 1.0,
                    'saved_bytes': 0, 'saved_mb': 0.0, 'percent_saved': 0.0}
        saved = max(0, dense_size - sparse_size)
        return {'dense_bytes': dense_size, 'sparse_bytes': sparse_size,
                'ratio': dense_size / sparse_size if sparse_size else float('inf'),
                'saved_bytes': saved, 'saved_mb': saved / 1048576,
                'percent_saved': (1 - sparse_size / dense_size) * 100}


BUS_DTYPE = np.dtype([
    ('bus_id', np.int64), ('vmag', np.float64), ('vang', np.float64),
    ('pL', np.float64), ('qL', np.float64), ('pG', np.float64), ('qG', np.float64),
    ('kv', np.float64), ('bt', np.int8), ('qmin', np.float64), ('qmax', np.float64),
    ('vms', np.float64),
])
...
class MemoryOptimizedSystem:
    ...
    __slots__ = (
        'base_mva', 'bus_count', 'lines', 'transformers', 'generators', 'loads',
        'Ybus_seq', '_inc_gen_z', '_use_arr',
        '_ids', '_vmag', '_vang', '_pL', '_qL', '_pG', '_qG', '_kv', '_bt',
        '_qmin', '_qmax', '_vms', '_buses', '_sm',
    )
    THRESH = 100
    ...
    def from_system(self, system: System) -> 'MemoryOptimizedSystem':
        ...
        for i, bid in enumerate(bids):
            b = system.buses[bid]
            a[i] = (bid, b.voltage_magnitude, b.voltage_angle,
                    b.load_power.real, b.load_power.imag,
                    b.generation_power.real, b.generation_power.imag,
                    b.base_kv if b.base_kv else np.nan,
                    BTYPE_MAP.get(b.bus_type, 2), b.q_min, b.q_max,
                    b.voltage_magnitude_scheduled if b.voltage_magnitude_scheduled is not None else np.nan)
        for attr, f in [('_ids', 'bus_id'), ('_vmag', 'vmag'), ('_vang', 'vang'),
                        ('_pL', 'pL'), ('_qL', 'qL'), ('_pG', 'pG'), ('_qG', 'qG'),
                        ('_kv', 'kv'), ('_bt', 'bt'), ('_qmin', 'qmin'),
                        ('_qmax', 'qmax'), ('_vms', 'vms')]:
    ...
    def get_bus_data(self, bus_id: int) -> Dict[str, Any]:
        if not self._use_arr:
            ...
            return {'bus_id': b.bus_id, 'voltage_magnitude': b.voltage_magnitude,
                    'voltage_angle': b.voltage_angle, 'voltage': b.voltage,
                    'load_power': b.load_power, 'generation_power': b.generation_power,
                    'base_kv': b.base_kv, 'bus_type': b.bus_type,
                    'q_min': b.q_min, 'q_max': b.q_max}
        i = self._b_idx(bus_id)
        return {'bus_id': int(self._ids[i]), 'voltage_magnitude': float(self._vmag[i]),
                ...
                'q_min': float(self._qmin[i]), 'q_max': float(self._qmax[i])}
    ...
    def to_system(self) -> System:
        ...
        if self._use_arr:
            for i in range(self.bus_count):
                bus = Bus(bus_id=int(self._ids[i]),
    ...
    def estimate_memory_usage(self) -> Dict[str, Any]:
        ...
        if self._use_arr:
            bus_mem = sum(getattr(self, a).nbytes for a in
                          ['_ids', '_vmag', '_vang', '_pL', '_qL', '_pG', '_qG',
                           '_kv', '_bt', '_qmin', '_qmax', '_vms']
                          if getattr(self, a) is not None)
        else:
            bus_mem = sum(sys.getsizeof(b) for b in (self._buses or {}).values())
            bus_mem += sys.getsizeof(self._buses or {})
        extra = sum(sum(sys.getsizeof(x) for x in lst)
        ...
        return {'total_bytes': total, 'total_mb': total / 1048576,
                'bus_data_bytes': bus_mem, 'ybus_bytes': ym,
...
class BatchProcessor:
    ...
    def __init__(self, batch_size: int = 1000):
        ...
        self._stats = {'total_items': 0, 'num_batches': 0,
                       'batch_sizes': [], 'batch_times': [], 'total_time': 0.0}
    def process_buses(self, buses: Union[Dict, List], fn: Callable) -> List[Any]:
        return self._batch_process(list(buses.items()) if isinstance(buses, dict) else list(buses), fn)
    def process_lines(self, lines: List, fn: Callable) -> List[Any]:
        return self._batch_process(list(lines), fn)
    def process_faults(self, buses: Union[Dict, List], ftypes: List[str],
    ...
    def get_batch_statistics(self) -> Dict[str, Any]:
        ...
        s.update(avg_batch_time=float(np.mean(t)) if t else 0.0,
...
class DataCompressor:
    ...
    def compress_system_state(self, system: Union[System, MemoryOptimizedSystem]) -> Dict[str, Any]:
        ...
        comp = {'base_mva': np.float32(system.base_mva), 'bus_count': len(bids),
                'bus_ids': np.array(bids, dtype=np.int32), 'bus_vmag': vmag,
                'bus_vang': vang, 'bus_types': btypes,
                'lines': [{'id': l.line_id, 'fr': l.from_bus.bus_id, 'to': l.to_bus.bus_id,
                           'z1r': l.z1.real, 'z1i': l.z1.imag} for l in s.lines],
                'transformers': [{'id': t.transformer_id, 'fr': t.from_bus.bus_id,
                                  'to': t.to_bus.bus_id, 'z1r': t.z1.real, 'z1i': t.z1.imag,
                                  'tap': t.tap_ratio, 'ph': t.phase_shift} for t in s.transformers],
                'generators': [{'id': g.generator_id, 'bus': g.bus.bus_id,
                                'v1r': g.internal_voltage.get('1', 0j).real,
                                'v1i': g.internal_voltage.get('1', 0j).imag} for g in s.generators]}
    ...
    def decompress_system_state(self, compressed: Dict[str, Any]) -> System:
        ...
        for td in compressed.get('transformers', []):
            s.add_transformer(Transformer(td['id'], s.buses[td['fr']], s.buses[td['to']],
                                          z1=complex(td['z1r'], td['z1i']),
                                          tap_ratio=td['tap'], phase_shift=td['ph']))
        for gd in compressed.get('generators', []):
            s.add_generator(Generator(gd['id'], s.buses[gd['bus']],
                                       internal_voltage={'1': complex(gd['v1r'], gd['v1i']),
                                                         '2': 0j, '0': 0j}))
        return s
    ...
    def get_compression_ratio(self) -> Dict[str, Any]:
        ...
        return {'original_bytes': self._orig, 'compressed_bytes': self._comp,
...
class PerformanceProfiler:
    ...
    def profile_function(self, fn: Callable, *args: Any, **kwargs: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            if gc_was:
                gc.enable()
        p = {'function': fn.__name__, 'elapsed_seconds': elapsed,
             'memory_before_mb': mb, 'memory_after_mb': ma,
             'memory_delta_mb': ma - mb, 'result_type': type(result).__name__}
    ...
    def profile_memory(self):
        ...
        try:
            ...
            self._profiles.append(dict(function='memory_block', memory_before_mb=before,
...
class LargeSystemAdapter:
    ...
    def __init__(self, system: Union[System, MemoryOptimizedSystem], memory_limit_mb: int = 1024):
        ...
        self.optimized_system = MemoryOptimizedSystem(system) if isinstance(system, System) else system
    ...
    def run_load_flow_optimized(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        p = params or {}
        Y = self.sparse_manager.build_sparse_ybus(self.optimized_system.to_system(), p.get('seq', '1'))
        r = {'Ybus': Y, 'Ybus_format': 'sparse_csr', 'bus_count': self._n,
             'memory_limit_mb': self.memory_limit_mb}
        if self._large:
            r.update(LU_factor=splu(Y), solver='sparse_lu',
                     initial_mismatch=Y.dot(self.optimized_system.get_all_bus_voltages())
                     if self._n <= 5000 else np.zeros(self._n, dtype=complex))
        else:
        ...
        if fb is not None:
            ...
            def ab(batch):
                ...
                for e in batch:
                    ...
                    zt = float(abs(r['Zbus_pos'][bid - 1, bid - 1])) if isinstance(r.get('Zbus_pos'), np.ndarray) else 1.0
                    br.append({'bus': bid, 'fault_type': ft, 'thevenin_impedance': zt})
                return br

            r['fault_analyses'] = self.batch_processor._batch_process(
                [(b, ft) for b in bl for ft in p.get('fault_types', ['3ph', 'slg', 'll', 'llg'])], ab)
        return r

    def get_optimization_strategy(self) -> Dict[str, Any]:
        n = self._n
        if n >= 10000:
            flags = dict(use_sparse=True, use_array_storage=True, use_batch_processing=True,
            ...
            flags = dict(use_sparse=True, use_array_storage=True,
            ...
            flags = dict(use_sparse=True, use_array_storage=True, use_batch_processing=False,
            ...
            flags = {k: False for k in ['use_sparse', 'use_array_storage', 'use_batch_processing',
                                        'use_compression', 'need_memory_monitoring']}
